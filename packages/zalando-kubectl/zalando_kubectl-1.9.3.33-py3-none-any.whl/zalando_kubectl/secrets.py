...
# Prefer Manual/Emergency roles to allow decrypt to work
_AWS_ROLES = ('Administrator', 'Manual', 'Emergency', 'PowerUser', 'Deployer', 'ReadOnly')
...
def find_aws_role(token, account_id):
    ...
    matching_roles = [role for role in user_roles if role.account_id == account_id and role.role_name in _AWS_ROLES]

    # Order the roles in the order of preference
    matching_roles.sort(key=lambda role: _AWS_ROLES.index(role.role_name))
...
def kms_for_cluster(cluster_metadata):
...
def encrypt(cluster_metadata, kms_keyid, plain_text_blob):
    kms = kms_for_cluster(cluster_metadata)
