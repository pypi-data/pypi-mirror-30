...
### logging ###################################################################
logging.getLogger('hexer').addHandler(logging.NullHandler())
...
class Hexer:
    def __init__(self, filename, **kwargs):
        ...
        for key, value in kwargs.items():
            if key == 'blocks':
                ...
                config = read_block_config(value)
    ...
    def printHeader(self):
        # sorted_keys = sorted(hexer.header.value_dict)
        for key in sorted(self.header.value_dict):
            value = self.header.value_dict[key]
            print('%20s' % key, value)
...
class HeaderBlock:
    def __init__(self, parent, block_dict):
        self.logger = logging.getLogger('hexer')
        self.dtype = ''
        self.fmt = tuple([])
        self.length = 0
        self.offset = 0
        ...
        if self.length and self.dtype:
            ...
            elif len(self.dtype) == 2:
                self.fmt = self.dtype[0] + str(self.length) + self.dtype[1]
    ...
    def readBlock(self):
        for offset, name, fmt in self.structure:
            ...
            if (fmt == 'chunk'):
                if name not in self.parent.chunks.keys():
                    self.logger.warning('%s not found in chunk dict', name)
                    continue

                chunk_dict = self.parent.chunks[name]
                chunk_dict['offset'] = offset
                chunk = HeaderChunk(self, chunk_dict)
                values = chunk.readChunk()
            else:
...
###############################################################################
class HeaderChunk:
    def __init__(self, parent, chunk_dict):
        self.chunk_dict = chunk_dict
        self.parent = parent
        
    def readChunk(self):
        N = self.chunk_dict['loop']
        offset_0 = self.chunk_dict['offset']
        size = self.chunk_dict['size']
        structure = self.chunk_dict['structure']
        ...
        for i in range(N):
            ...
            for offset, name, fmt in structure:
                total_offset = offset_0 + i * size + offset

                values = struct.unpack_from(
                        fmt,
                        self.parent.buffer[total_offset:])
