...
class TestBlocks(unittest.TestCase):
    ...
    def test_00_float_block(self):
        config_fullfile = os.path.join('hexer', 'config', 'bruker_opus.cfg')
        data_fullfile = os.path.join('data', 'w699.0')
        hexer = Hexer(data_fullfile, config_file=config_fullfile)
        hexer.readBlocks()

        self.assertEqual(hexer.header.magic_numbers, (2570, 65278))
        self.assertEqual(hexer.header.block_offset, 24)
        self.assertEqual(hexer.header.no_of_blocks, 7)
