...
.. glossary::
    ...
    filter_trans_homopolymers
        :class:`bool` - Filter all single bp ins/del/dup events that are in a homopolymer region of at least 5 bps and are not paired to a genomic event. The corresponding environment variable is ``MAVIS_FILTER_TRANS_HOMOPOLYMERS`` and the default value is ``True``
    ...
    min_call_complexity
        :class:`~mavis.constants.float_fraction` - The minimum complexity score for a call sequence. is an average for non-contig calls. see :term:`contig_complexity`. The corresponding environment variable is ``MAVIS_MIN_CALL_COMPLEXITY`` and the default value is ``0.21``
