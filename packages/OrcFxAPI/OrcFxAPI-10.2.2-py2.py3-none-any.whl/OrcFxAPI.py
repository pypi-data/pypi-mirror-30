...
_useNumpy = OrcFxAPIConfig.getUseNumpy()
if _useNumpy:
    try:
        import numpy
...
if _useNumpy:
    def _allocateArray(length): return numpy.empty(length, dtype=numpy.float64)
    ...
    def _finaliseArray(array): return array

    def _allocateIntArray(length): return numpy.empty(length, dtype=numpy.int32)

    _finaliseIntArray = _finaliseArray

    def _getArrayData(array): return array.ctypes.data_as(ctypes.POINTER(ctypes.c_double))
    ...
    def _allocateArray(length): return (ctypes.c_double * length)()
    ...
    def _finaliseArray(array): return tuple(array)

    def _allocateIntArray(length): return (ctypes.c_int * length)()

    _finaliseIntArray = _finaliseArray

    def _getArrayData(array): return ctypes.cast(ctypes.byref(array), ctypes.POINTER(ctypes.c_double))

    _array = tuple

    IntArrayType = ctypes.POINTER(ctypes.c_int)
    ...
    # the intention is that this type is never actually used, because we
    # enforce numpy when operating on complex valued data
    ComplexArrayType = ctypes.POINTER(complex)
    del(complex)


def _requiresNumpy():
    if not _useNumpy:
        raise MissingRequirementError('This function requires the numpy module to be available')
...
propSolidFrictionCoefficientsHandle = 5
...
class DLLError(Exception):
    ...
    def __init__(self, status):
        if isinstance(status, str):
            ...
            _GetLastErrorString(ErrorString)
