...
class Request(object):
    ...
    def form(self):
        ...
        """
        try:
            return self.environ['fresco.request.form']
        except KeyError:
            if self.environ['REQUEST_METHOD'] in ('PUT', 'POST'):
                form = MultiDict(
                    parse_post(self.environ,
                               ...
                               ie_workaround=
                                self.IE_CONTENT_DISPOSITION_WORKAROUND)
                )
            else:
                data = environ_to_unicode(self.environ.get('QUERY_STRING', ''))
                form = MultiDict(parse_querystring(data, self.charset))
                self.environ['fresco.request.query'] = form
            self.environ['fresco.request.form'] = form
        return form

    @property
    def content_type_encoding(self):
        try:
            return self.environ['fresco.request.parsed_content_type'].encoding
        except KeyError:
            r = self.environ['fresco.request.parsed_content_type'] = \
                get_content_type_info(self.environ)
        return r.encoding
    ...
    def content_type(self):
        try:
            return \
                self.environ['fresco.request.parsed_content_type'].content_type
        except KeyError:
            r = self.environ['fresco.request.parsed_content_type'] = \
                get_content_type_info(self.environ)
    ...
    def body_bytes(self):
        try:
            return self.environ['fresco.request.body_bytes']
        except KeyError:
            r = self.environ['fresco.request.body_bytes'] = \
                get_body_bytes(self.environ, self.MAX_SIZE)
        return r
    ...
    def files(self):
        ...
        """
        try:
            return self.environ['fresco.request.files']
        except KeyError:
            r = self.environ['fresco.request.files'] = \
                    MultiDict((k, v)
    ...
    def query(self, environ_to_unicode=environ_to_unicode):
        ...
        """
        try:
            return self.environ['fresco.request.parsed_query']
        except KeyError:
            query = environ_to_unicode(self.environ.get('QUERY_STRING', ''))
            r = self.environ['fresco.request.parsed_query'] = \
                    MultiDict(parse_querystring(query, self.charset))
            return r
    ...
    def cookies(self):
        ...
        """
        try:
            return self.environ['fresco.request.cookies']
        except KeyError:
            r = self.environ['fresco.request.cookies'] = MultiDict(
    ...
    def parsed_url(self):
        ...
        """
        try:
            return self.environ['fresco.request.parsed_url']
        except KeyError:
            pass
        ...
        r = self.environ['fresco.request.parsed_url'] = ParseResult(
            scheme,
            netloc,
