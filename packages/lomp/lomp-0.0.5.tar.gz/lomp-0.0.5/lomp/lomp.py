...
def parallel_map(data,
    ...
    # If data is a generator, we need to consume it so we can pass it by reference to all threads. This unfortunately
    # requires that at least one copy of the data be able to fit into memory.
    data = tuple(data)
    if len(data) == 0:
        # There was no data
        raise StopIteration
