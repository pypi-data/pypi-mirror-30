...
def parallel_map(data,
    ...
    if not isinstance(data, collections.Sequence):
        # The data is a generator, so we need to consume it so we can pass it by reference to all threads. This
        # unfortunately requires that at least one copy of the data be able to fit into memory. If the data object is
        # already a sequence, then it's already in memory and we want to avoid copying it into a tuple.
        data = tuple(data)
    if len(data) == 0:
        # There was no data
        raise StopIteration
    ...
    work_item_indexes = [[] for _ in range(process_count)]
    for i in range(len(data)):
        work_item_indexes[i % process_count].append(i)
    ...
    for n in range(process_count):
        if not work_item_indexes[n]:
            break

        process = multiprocessing.Process(target=func, args=(data, work_item_indexes[n], args), kwargs=kwargs)
...
def parallel_iterator(data,
    ...
    bins = [[] for _ in range(process_count)]
    i = 0
    for i, item in enumerate(data):
        bins[i % process_count].append(item)
    ...
    for n in range(process_count):
        ...
        process = multiprocessing.Process(target=func, args=(finished_signal, bins[n], args), kwargs=kwargs)
        processes.append(process)
        process.start()
