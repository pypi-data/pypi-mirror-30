...
class GEE_pt(object):
    ...
    def extr_S1(self,
        ...
        def computeLIA(image):
            # function for the computation of the local incidence angle, based on azimuth angle and srtm
        ...
        for track_nr in available_tracks:
            ...
            #  filter for track
            if dual_pol == True:
                gee_s1_track_fltd = gee_s1_filtered.filterMetadata('relativeOrbitNumber_start', 'equals',
            ...
            if dual_pol == True:
                out_dict[str(int(track_nr))] = pd.DataFrame({'sig0': vv_sig0, 'sig02': vh_sig0, 'lia': lia}, index=ge_dates)
            else:
                out_dict[str(int(track_nr))] = pd.DataFrame({'sig0': vv_sig0, 'lia': lia}, index=ge_dates)
    ...
    def extr_SM(self,
        ...
        for track_id in s1_ts.keys():
            ...
            if ts_length < 10:
                print('Time series is too short to compute reliable statistics')
                continue
