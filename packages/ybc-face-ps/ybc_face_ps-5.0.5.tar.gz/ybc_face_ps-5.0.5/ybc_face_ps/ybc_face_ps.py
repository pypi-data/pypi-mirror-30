...
def meizhuang(filename='', meitype='日系妆-芭比粉'):
    ...
    if res['ret'] == 0 and res['data']:
        new_file = os.path.splitext(filepath)[0] + '_meizhuang.jpg'
...
def bianzhuang(filename='', biantype='灰姑娘妆'):
    ...
    res = r.json()
    if res['ret'] == 0 and res['data']:
        new_file = os.path.splitext(filepath)[0] + '_bianzhuang.jpg'
        with open(new_file,'wb') as f:
            f.write(base64.b64decode(res['data']['image']))
        return new_file
    else:
...
def ronghe(filename='', rongtype='孙尚香'):
    ...
    res = r.json()
    if res['ret'] == 0 and res['data']:
        new_file = os.path.splitext(filepath)[0] + '_ronghe.png'
        with open(new_file,'wb') as f:
            f.write(base64.b64decode(res['data']['image']))
        return new_file
    else:
