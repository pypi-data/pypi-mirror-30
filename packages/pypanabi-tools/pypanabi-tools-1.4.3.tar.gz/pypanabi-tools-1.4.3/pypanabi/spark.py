...
class Spark(object):
    ...
    def aggregate(self, operation='count', aggregation_column_name='total', sum_column=None):
        """
        Aggregate current dataframe
        :param operation: count or sum [String]
        ...
        try:
            ...
            if operation == 'sum':
                if not sum_column:
                    self._logger.error('Please, inform the column name to summarize')
                else:
    ...
    def concatenate(self, columns, concatenation_column_name='concat'):
        """
        Concatenate columns
        :param columns: list of columns [List<String>]
