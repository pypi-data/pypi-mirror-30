...
def DFconcatmdb(cur,table,df):
    ...
    fn=["["+v.replace('.',' ')+"]" for v in fn]#this eliminates SQL conflict errors
    ind=list(df.axes[0])
...
def createTable(cur,table,cols,debug=False):
    '''creates a table
    could insert cols for blank table or insert dataframe
    for full table'''
    import pyodbc
    from pandas import DataFrame
    if type(cols)==type(DataFrame()):
        df=cols
        cols=list(df.columns.values)
    
    ct="CREATE TABLE ["+table+"] (["+"] MEMO, [".join(cols)+"] MEMO )"
    ct=ct.replace("."," ")
    try: cur.execute(ct)
    except: 
        if debug: print('table already exists')
    #try: cur=DFtomdb(cur,table,df)
    #except: pass
    return cur