...
def auto_config(config, pool_name, prefix='', section=None):
    ...
    # FIXME : Переписать загрузку драйвера с использованием __import__ или
    # importlib
    ...
    catalog[pool_name] = Driver(**{param: config.get(prefix + param, value) for param, value in defaults.items()})
...
class ThreadedPool(object):
    ...
    def _refill(self):
        for _ in range(len(self.pool) + len(self.used), self.min_connections):
    ...
    def clean(self):
        now = time.time()
        for conn in self.pool + list(self.used.values()):
