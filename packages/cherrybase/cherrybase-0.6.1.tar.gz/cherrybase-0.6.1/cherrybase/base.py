...
class Application(object):
    ...
    def __init__(self, name=None, config=None, vhosts=None, routes=None):
        ...
        self.config = config
        self.app = cherrypy.Application(None, '', self.config)
        if routes:
...
class Server(object):
    ...
    def __init__(self, applications=None, config=None, debug=True):
        ...
        if debug:
            cherrypy.log._get_builtin_handler(
                cherrypy.log.error_log, 'screen').setLevel(logging.DEBUG)
            cherrypy.log._get_builtin_handler(
                cherrypy.log.access_log, 'screen').setLevel(logging.DEBUG)

    def scan_applications(self):
        import sys

        if not _server_conf.pkg_path or not os.path.exists(_server_conf.pkg_path):
            cherrypy.log.error('Invalid server packages path (cherrybase.pkg_path): {}'.format(
                _server_conf.pkg_path), 'SERVER', logging.FATAL)
        ...
        basename = cherrypy.config.get(
            'server.basename', cherrypy.config.get('server.socket_host', '127.0.0.1'))
        ...
        for pkg_name in [_server_conf.packages] if isinstance(_server_conf.packages, basestring) else _server_conf.packages:
            cherrypy.log.error(
                'Importing package "{}"'.format(pkg_name), 'SERVER')
            module = __import__(pkg_name)
            applications = module.get_applications(
                'debug' if self.debug else 'production', basename)
            if isinstance(applications, Application):
                applications = [applications]
            cherrypy.log.error('Applications found: {}'.format(
                [app.name for app in applications]), 'SERVER')
    ...
    def start(self):
        ...
        for app in self.applications:
            ...
            cherrypy.log.error('Application {} virtual hosts: {}'.format(
                app.name, app.vhosts), 'SERVER')

        self.daemonize()

        cherrypy.tree.graft(
            _cpwsgi.VirtualHost(
                cherrypy.Application(_server_conf.main_handler),
                domains=vhosts
            )
        )
            ...
            try:
                ...
                cherrypy.log.error(
                    'Cannot find user "{0}" or group "{1}"'.format(
                        _daemon_conf.user, _daemon_conf.group),
                    'SERVER',
                    logging.FATAL
                )
                raise
