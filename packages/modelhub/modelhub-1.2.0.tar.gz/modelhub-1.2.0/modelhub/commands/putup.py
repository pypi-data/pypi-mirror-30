...
API_MODEL_TEMPLATE = """
# -*- coding: utf-8 -*-
from modelhub.framework import BaseModel
...
# -*- coding: utf-8 -*-
from modelhub.framework import TFBaseModel
...
class Command(BaseCommand):
    ...
    def run(self, project_name, tf=False):
        ...
        self._create_file(root / "setup.py", SETUP_PY_TEMPLATE, project_name=project_name, conf=conf)
        self._create_file(root / ".gitignore", GITIGNORE_TEMPLATE, project_name=project_name, conf=conf)
        self._create_file(root / "README.md", README_MD_TEMPLATE, project_name=project_name, conf=conf)
        src_dir = root / project_name
        src_dir.mkdir()
        self._create_file(src_dir / "__init__.py", INIT_PY_TEMPLATE, project_name=project_name, conf=conf)
        self._create_file(src_dir / "api_model.py", TF_API_MODEL_TEMPLATE if tf else API_MODEL_TEMPLATE, project_name=project_name, conf=conf)
