...
class VariableKiosk(dict):
    ...
    def flush_rates(self):
        ...
        """
        for key in list(self.published_rates.keys()):
            self.pop(key, None)

    def flush_states(self):
        """flush the values of all state variable from the kiosk.
        """
        for key in list(self.published_states.keys()):
            self.pop(key, None)
...
class SimulationObject(HasTraits, DispatcherObject):
    ...
    def _find_SubSimObjects(self):
        ...
        for attr in list(defined_traits.values()):
...
class WeatherDataContainer(SlotPickleMixin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(kwargs) > 0:
            ...
            logging.warning(msg, list(kwargs.keys()))
...
class WeatherDataProvider(object):
    ...
    def export(self):
        ...
        if self.supports_ensembles:
            ...
            weather_data = []
            days = sorted([r[0] for r in list(self.store.keys())])
            for day in days:
                wdc = self(day)
...
class ParameterProvider(MutableMapping):
    ...
    def _test_uniqueness(self):
        ...
        for mapping in [self._sitedata, self._timerdata, self._soildata, self._cropdata]:
            parnames.extend(list(mapping.keys()))
        unique = Counter(parnames)
        for parname, count in list(unique.items()):
            if count > 1:
    ...
    def __str__(self):
        ...
        return msg % (len(self), len(self._override), list(self._override.keys()))
