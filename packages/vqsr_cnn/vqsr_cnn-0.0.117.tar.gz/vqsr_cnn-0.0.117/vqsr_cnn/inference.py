...
def score_and_write_batch(args, model, file_out, fifo, batch_size, python_batch_size, tensor_dir):
    ...
    for i in range(batch_size):
        if 'SNP' == variant_types[i]:
            file_out.write(variant_data[i]+'\t{0:.2f}'.format(snp_scores[i])+'\n')
        elif 'INDEL' == variant_types[i]:
            file_out.write(variant_data[i]+'\t{0:.2f}'.format(indel_scores[i])+'\n')
        else:
            file_out.write(variant_data[i]+'\t{0:.2f}'.format(max(snp_scores[i], indel_scores[i]))+'\n')
