...
def score_and_write_batch(args, model, file_out, fifo, batch_size, python_batch_size, tensor_dir):
    ...
    for i in range(batch_size):
        ...
        at = annotation_string_to_tensor(args, fifo_data[5])
        rt = reference_string_to_tensor(fifo_data[4])
        print('At pos:', fifo_data[1], 'Ref string is:', fifo_data[4])
        print('Reference tensor sum:', np.sum(rt, axis=0))
        print(' type:', fifo_data[6],'annotation tensor:', at, '\n anno string:', fifo_data[5])
        print('is snp?:', fifo_data[6]=='SNP')
        print('Stripped is snp?:', fifo_data[6].strip()=='SNP')
        print(' prediction:', model.predict([np.expand_dims(rt, axis=0),np.expand_dims(at, axis=0)]))
