...
def score_and_write_batch(args, model, file_out, fifo, batch_size, python_batch_size, tensor_dir):
    ...
    for _ in range(batch_size):
        ...
        print('At variant:', fifo_data[0] + '\t' + fifo_data[1] + '\t' + fifo_data[2] + '\t' + fifo_data[3])
        print(' AT:', annotation_string_to_tensor(args, fifo_data[5]))
        ...
        if args.tensor_name in defines.TENSOR_MAPS_2D and len(fifo_data) > fidx:
            ...
            print(' Insert dict:', insert_dict, 'ref start:', ref_start)
            print(' RT sums:', np.sum(np.sum(tensor, axis=0), axis=0))
