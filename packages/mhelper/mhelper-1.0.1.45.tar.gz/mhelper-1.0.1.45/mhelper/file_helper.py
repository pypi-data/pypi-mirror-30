...
def read_all_text( file_name: str, default = NOT_PROVIDED ) -> Optional[str]:
    """
    ...
    if not os.path.isfile( file_name ):
        if default is NOT_PROVIDED:
            raise FileNotFoundError( "Cannot read_all_text from file because the file doesn't exist: «{}».".format( file_name ) )
...
def is_windows() -> bool:
    """
    Rerturns if the current platform is Windows.
    """
    return os.name == "nt"
...
def recycle_file( file_name ):
    try:
        ...
        send2trash = None
        warnings.warn( "I could not load the `send2trash` module so instead of sending the file '{}' to the recycle bin I am just going to delete it. Please install `send2trash` to avoid this warning in future.".format( file_name ), UserWarning )
        os.remove( file_name )
