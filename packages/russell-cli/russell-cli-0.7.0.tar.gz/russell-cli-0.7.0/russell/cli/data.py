...
@click.pass_context
def upload(ctx, r, message, nopack, file):
    """
    ...
    if size > 5 * 1024 * 1024 * 1024:
        sys.exit("Total size: {}. Data size too large to sync, please keep it under 5GB.".format(sizeof_fmt(size)))
    # if len(files) > 100:
    ...
    if file:
        ...
        # finally:
            # rm temp dir
            # shutil.rmtree(_TEMP_UPLOAD_DIR)
