...
def extract_account_breakdown(df, account, silent, json):
    ...
    for row in acc_df.iterrows():
        series = row[1]
        breakdown.append({
            'Site Name': series[SITE_NAME],
            'Site ID': series[SITE_ID.strip()],
            'Total Requests': series[REQ_TOTAL],
            'TLS v1.0': series[VERSIONS.get('TLS 1.0')],
            'TLS v1.1': series[VERSIONS.get('TLS 1.1')]
        })
