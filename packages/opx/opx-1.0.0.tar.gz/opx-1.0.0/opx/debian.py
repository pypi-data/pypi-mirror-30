...
PACKAGES = 'dists/unstable/opx/binary-amd64/Packages.gz'
...
def resolve_dependencies(repo: str, control, packages):
    ...
    L.debug(f'Need to build in this order: {rv}')
...
def opx_build_dependencies(repo: str):
    ...
    if not Path(f'{repo}/debian/control').exists():
        return [repo]

    controlfile = Path(f'{repo}/debian/control')
    control = parse_control_fields(deb822_from_string(controlfile.read_text()))

    # TODO: Cache this file in config directory
    packages = remote_packages(f'http://deb.openswitch.net/{PACKAGES}')

    return resolve_dependencies(repo, control, packages)


def release_versions(url: str) -> Dict[str, str]:
    """Given a URL to a Packages.gz file, return a dict of packages and
    versions.
    """
    return {k: v.get('Version') for k, v in remote_packages(url).items()}
