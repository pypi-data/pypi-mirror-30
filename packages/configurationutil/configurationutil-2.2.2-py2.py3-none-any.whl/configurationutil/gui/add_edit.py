...
class AddEditConfigFrame(with_metaclass(ABCMeta, BaseFrame)):
    ...
    def _draw(self):
        ...
        self._inherits = Combobox(value=self.selected_record.get(DefaultInheritableConstant.inherits, u'')
                                  if self.edit else u'',
                                  values=self._inheritable_items,
                                  row=Position.CURRENT,
                                  column=Position.NEXT,
                                  sticky=EW)
    ...
    @property
    def defaults(self):
        return self.selected_record.get(DefaultInheritableConstant.defaults, {}) if self.edit else {}
    ...
    def _save(self):
        ...
        try:
            ...
            new_host = {
                DefaultInheritableConstant.defaults: self.save_defaults()
            }

            if self._inherits.value:
                new_host[DefaultInheritableConstant.inherits] = self._inherits.value

            if self._hidden.value:
                new_host[DefaultInheritableConstant.hidden] = self._hidden.value
