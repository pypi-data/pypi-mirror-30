...
_kube_spark_deploy_template_registry = {'spark': (['yaml/spark-deploy.yaml.template'], [])}

_kube_spark_svc_template_registry = {'spark': (['yaml/spark-svc.yaml.template'], [])}
...
def spark_server_init(model_name,
                      model_tag,
                      model_type,
                      new_model_path=None):
    ...
    templates_base_path = _os.path.normpath('%s/spark/%s' % (_templates_path(), model_type))
...
## TODO:  spark_server_build
## TODO:  spark_server_start
## TODO:  spark_kube_start
...
#
# ie. http://localhost:6970/train-local-start/mnist/a/GARBALLY0GOOP==
...
#                       start_cmd='docker'
    ...
    # This convention must be supported in <model_path>/pipeline.yaml file.
    # Specifically, the model (name) attribute.
    args = "%s-%s" % (model_name, model_tag) 
