...
#     gunicorn --bind 0.0.0.0:6970 --pythonpath <cli_pipeline_module_path> -k gevent cli_pipeline:app http <debug=True|False> <_new_model_base_path>
def _main():
    global _http_mode
    if len(_sys.argv) == 1:
        return help()
    if len(_sys.argv) >= 2 and _sys.argv[1] == 'http':
        ...
        cmd = 'gunicorn --bind 0.0.0.0:%s --pythonpath %s -k gevent cli_pipeline:app http %s %s' % (port, cli_pipeline_module_path, debug, _new_model_base_path)
        _subprocess.call(cmd, shell=True)
