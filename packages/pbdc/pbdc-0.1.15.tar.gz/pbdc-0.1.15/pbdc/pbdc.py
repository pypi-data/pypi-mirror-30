...
def check_keys(kwargs, keys=[]):
    for k in keys:
        v = kwargs.get(k, None)
        if v is None:
            raise Exception('not found the key:"%s" in define context:"%s:%s" pdE:"%s"...' % (k, str(gtx.stack[-1].name), str(gtx.stack[-1].type),str(kwargs)))
...
def pdE(*args, **kwargs):
    ...
    if current_ctype == 'enum':
        ...
        if(len(args) > 1):
            kwargs['v']=args[1]
        if kwargs.get('v',None) is None:
            if len(gtx.stack[-1].fields) == 0:
               kwargs['v'] = 0 
            else:
               kwargs['v'] = int(gtx.stack[-1].fields[-1]['v'])+1
