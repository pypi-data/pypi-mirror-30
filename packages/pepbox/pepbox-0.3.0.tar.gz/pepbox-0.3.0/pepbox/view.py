...
def get_2by2_exvivo_table(tet_pval_exp_clonotype_ids, rel_pep_stim_exp_clonotype_ids, rel_pep_stim_not_exp_clonotype_ids, exvivo):
    ...
    tetp_rel_pep_stim_exp_cln_ids_exvivo_freq_by_clonotypes = exvivo.aa_clonotypes.get_cell_frequencies_by_aa_clonotype_ids(aa_clonotype_ids=tetp_rel_pep_stim_exp_cln_ids)
    tetp_rel_pep_stim_exp_cln_ids_exvivo_freq = sum(tetp_rel_pep_stim_exp_cln_ids_exvivo_freq_by_clonotypes)

    tetn_rel_pep_stim_exp_cln_ids_exvivo_freq_by_clonotypes = exvivo.aa_clonotypes.get_cell_frequencies_by_aa_clonotype_ids(aa_clonotype_ids=tetn_rel_pep_stim_exp_cln_ids)
    tetn_rel_pep_stim_exp_cln_ids_exvivo_freq = sum(tetn_rel_pep_stim_exp_cln_ids_exvivo_freq_by_clonotypes)

    tetp_rel_pep_stim_not_exp_cln_ids_exvivo_freq_by_clonotypes = exvivo.aa_clonotypes.get_cell_frequencies_by_aa_clonotype_ids(aa_clonotype_ids=tetp_rel_pep_stim_not_exp_cln_ids)
    tetp_rel_pep_stim_not_exp_cln_ids_exvivo_freq = sum(tetp_rel_pep_stim_not_exp_cln_ids_exvivo_freq_by_clonotypes)

    tetn_rel_pep_stim_not_exp_cln_ids_exvivo_freq_by_clonotypes = exvivo.aa_clonotypes.get_cell_frequencies_by_aa_clonotype_ids(aa_clonotype_ids=tetn_rel_pep_stim_not_exp_cln_ids)
...
def determine_pep_specificity_thresholds_for_multiple_subjects(path_tables_and_figures_folder, processed_file_aa_pval_names,
...
def validation_bar_plot_aa(path_tables_and_figures_folder, exvivo, medium, tetramer, rel_pep_stim, irr_pep_stim,
    ...
    total_exvivo_tetp_freq_by_clonotypes = exvivo.aa_clonotypes.get_cell_frequencies_by_aa_clonotype_ids(aa_clonotype_ids=tet_pval_exp_clonotype_ids)
    ...
    exposed_super_confident_clonotypes_freq_by_clonotypes = exvivo.aa_clonotypes.get_cell_frequencies_by_aa_clonotype_ids(aa_clonotype_ids=df_temp.index.values)
    exposed_super_confident_clonotypes = sum(exposed_super_confident_clonotypes_freq_by_clonotypes)
...
def add_thres_lines(list_of_axis, mylimits, rel_pepstim_threshold, myfake_value, myfake_min, myfake_max, negctrl_threshold):
    for myaxis_index in range(len(list_of_axis)):
        ...
        if myaxis_index == 1:
            ...
            myaxis.plot([rel_pepstim_threshold, rel_pepstim_threshold], [ylimits[0], ylimits[1]], '--', linewidth=1.0,  color='r')#marker='.',
            myaxis.annotate('{:.2e}'.format(rel_pepstim_threshold), xy=(rel_pepstim_threshold, ylimits[1] + (0.06* ylimits[1])), size=15)
    return list_of_axis
...
def plot_comparison_negctrl_vs_pepstim_pvalues(path_tables_and_figures_folder, exvivo, tetramer, medium, irrpepstim, pepstim,
    ...
    if double_positive:
        ...
        # tet_df.index
        tet_aa_clonotypes_exp = tet_aa_clonotypes_exp.get
    else:
        tet_pval_exp_clonotype_index = tet_aa_clonotypes_exp.get_clonotype_index()
...
def produce_validation_bar_plot_aa(path_tables_and_figures_folder, filename, tetramer_sp=['exvivo, tetramer stain'], culture_thresholds=[1e-20, 1e-3]):
    if path.isfile(filename):
        df = read_csv(filename, header=None)
...
def produce_negctrl_vs_pepstim_pval_plot(path_tables_and_figures_folder, filename, tetramer_sp=['exvivo, tetramer stain'],
