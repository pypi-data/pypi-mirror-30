...
def program_setting():
    ...
    parser.add_argument('--version', action='version', version='pepbox 0.3.0')
...
def write_file3(samples, infos_df, output_path, subject_id, aa_file_sfx, clonotype_level="aacdr3", pvalues=None):
    ...
    myfileabspath = output_path['path_processed_data_folder'] + str(subject_id) + '_' + aa_file_sfx + '.csv'
...
def add_pvalues_to_file_with_cell_counts(paths, filename, sequence_level, fisher_test, infos_to_extract, clonotype_level="aacdr3"):
    if path.isfile(filename):
        ...
        for mygroup_samples in grouped_samples_by_subject_id:
            ...
            for subject_id in samples.get_subject_ids():
                if sequence_level == 'nucleotide':
                    add_pvalues(mygroup_samples, mygroup_samples.get_pvalues_df(), infos_df, paths, subject_id, sequence_level, 'nucleotide_clonotypes', clonotype_level)
                else:
                    add_pvalues(mygroup_samples, mygroup_samples.get_pvalues_df(), infos_df, paths, subject_id, sequence_level, 'CDR3_amino_acid_clonotypes', clonotype_level)
    else:
...
def pepbox():
    print("--- Program starts at: %s ---" % asctime(localtime(time())))
    ...
    if args.merge:
        print('++++++++++++++++++++++++++++')
        print(' Producing cell count files ')
        print('++++++++++++++++++++++++++++')

        parameters_for_sample_selection = None

        infos_to_extract = ['file name', 'subject id', 'experiment', 'peptide', 'peptide sequence', 'sequencing level', 'repeat', 'time point', 'cell type']

        for k in paths.keys():
            mypath = paths[k]
            if not path.isdir(mypath):
                print("Folder " + mypath + " does not exist !!")
                print("Creating it !!")
                mkdir(mypath)
        ...
        if options.keys() in ['additional_parameters']:
            ...
            produce_nt_aa_source_files(level_of_output_detail=level_of_output_detail, clonotype_level=clonotype_level,
                                       parameters_for_sample_selection=parameters_for_sample_selection, infos_to_extract=infos_to_extract, paths=paths)
    ...
    if args.expansion:
        print('+++++++++++++++++++++++++++++++++++++++++++++++++++++')
        print('Adding pvalues to previously created cell count files')
        print('+++++++++++++++++++++++++++++++++++++++++++++++++++++')

        # processed_file_nt_names = glob(path_processed_data_folder + '*nucleotide_clonotypes.csv')
        processed_file_aa_names = glob(path_processed_data_folder + '*CDR3_amino_acid_clonotypes.csv')
        ...
        infos_to_extract = ['file name', 'subject id', 'experiment', 'peptide', 'peptide sequence', 'sequencing level', 'repeat']
        ...
        if len(processed_file_aa_names) > 0 :
            for filename_aa in processed_file_aa_names:
                ...
                process = Process(getpid())
                print('After mem flush: ', sizeof_fmt(process.memory_info().rss))
        elif len(glob(path_processed_data_folder + '*CDR3_amino_acid_clonotypes_pval.csv')) > 0:
            raise Exception("Summary file already contains p-values.")
        else:
            raise Exception("Requested file(s) are not in \"" + path_processed_data_folder +"\" or do not have the right identifiers.")

    if args.geneusage:
        print('++++++++++++++++++++++++++++++')
        print(' Gene usage statistics ')
        print('++++++++++++++++++++++++++++++')

        # processed_file_nt_pval_names = glob(path_processed_data_folder + '*nucleotide_clonotypes_pval.csv')
        processed_file_aa_pval_names = glob(path_processed_data_folder + '*CDR3_amino_acid_clonotypes*.csv')

        # path_tables_and_figures_folder = path_tables_and_figures_folder + "/Basic stats/"

        if not path.isdir(path_tables_and_figures_folder):
            print("output_folder:", path_tables_and_figures_folder)
            print("Does not exist !!")
            print("Creating one !!")
            mkdir(path_tables_and_figures_folder)
        ...
        for filename_aa in processed_file_aa_pval_names:
            ...
            gene_usage(path_tables_and_figures_folder, filename_aa)

    if args.timepoints:
        print('++++++++++++++++++++++++++++++')
        print(' Time point analysis ')
        print('++++++++++++++++++++++++++++++')
        ...
        for filename_aa in processed_file_aa_pval_names:
            timepoint_analysis(path_tables_and_figures_folder, filename_aa)


    if args.analyze:
        print('++++++++++++++++++++++++++++++')
        print(' Producing figures and tables ')
        print('++++++++++++++++++++++++++++++')

        # processed_file_nt_pval_names = glob(path_processed_data_folder + '*nucleotide_clonotypes_pval.csv')
        processed_file_aa_pval_names = glob(path_processed_data_folder + '*CDR3_amino_acid_clonotypes_pval.csv')

        paths["path_tables_and_figures_folder"] = path_tables_and_figures_folder + "/Basic stats/"
        ...
        correlation_list_df = []
        for filename_aa in processed_file_aa_pval_names:
            print("amino acid sequence processing...")

            # subject_clonal_distribution(paths, filename_aa)

            df = produce_figures_and_tables(paths, filename_aa=filename_aa, pvalue_threshold=0.001, bonferroni=True, level_of_output_detail=level_of_output_detail)
            ...
            write_expanded_summary(paths, filename_aa, 'amino acid', bonferroni=True, level_of_output_detail=level_of_output_detail)
        ...
        if len(correlation_list_df) > 0:
            ...
            concat_df.to_csv(path_tables_and_figures_folder + "summary_expansion_correlations.csv", index=False)


    # if args.reproducibility:
    #     print('++++++++++++++++++++++++++++++++++++++++')
    #     print('Select sample ids from sample parameters')
    #     print('++++++++++++++++++++++++++++++++++++++++')
    #
    #
    #     parameters_for_sample_selection = {'subject_ids':[22], 'traits':['tuberculosis'], 'data_sequenced':['dna'],
    #                                'experiment_type_excluded':['aim', 'ifnapture', 'tetramer stain']}
    #
    #     # print('query: ', parameters_for_sample_selection)
    #
    #     sample_ids = get_sample_ids_from_sample_info_file(path_raw_data_folder, parameters_for_sample_selection)
    #
    #     # print('selected samples: ', sample_ids)

    if args.reproducibility:
        print('+++++++++++++++++++++++++++++++++++')
        print('Creating heatmap of reproducibility')
        print('+++++++++++++++++++++++++++++++++++')

        # processed_file_nt_pval_names = glob(path_processed_data_folder + '*nucleotide_clonotypes_pval.csv')
        processed_file_aa_pval_names = glob(path_processed_data_folder + '*CDR3_amino_acid_clonotypes_pval.csv')

        path_tables_and_figures_folder_reprod = path_tables_and_figures_folder + "/Reproducibility assessment/"

        if not path.isdir(path_tables_and_figures_folder_reprod):
            print("output_folder:", path_tables_and_figures_folder_reprod)
            print("Does not exist !!")
            print("Creating one !!")
        ...
        reprod_stats_summary_df = DataFrame(concatenate(reprod_stats_summary),
                                            columns=['subject_id', 'comparison', 'max reproducibility (%)', 'expansion p-values',
    ...
    if args.optimize:
        print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
        print('Creating negative control vs relevant peptide stimulated clonotype p-value plot')
        print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')

        path_tables_and_figures_folder_opt = path_tables_and_figures_folder + "/Optimized pvalue assessment/"

        if not path.isdir(path_tables_and_figures_folder_opt):
            print("output_folder:", path_tables_and_figures_folder_opt)
            print("Does not exist !!")
            print("Creating one !!")
        ...
        for tetramer_sp in tetramer_sp_subsets:
            irrelevant, relevant = determine_pep_specificity_thresholds_for_multiple_subjects(path_tables_and_figures_folder_opt, processed_file_aa_pval_names, tetramer_sp=tetramer_sp, bonferroni=True)

            print("Thresholds used [irrelevant, relevant]: ", [irrelevant, relevant])

            for filename_aa in processed_file_aa_pval_names:

                produce_negctrl_vs_pepstim_pval_plot(path_tables_and_figures_folder_opt, filename_aa,
        ...
        print('+++++++++++++++++++++++++++++')
        print('Creating validation bar plot ')
        print('+++++++++++++++++++++++++++++')
        ...
        path_tables_and_figures_folder_exvivo = path_tables_and_figures_folder + "/Ex vivo frequency/"

        if not path.isdir(path_tables_and_figures_folder_exvivo) :
            print("output_folder:", path_tables_and_figures_folder_exvivo)
            print("Does not exist !!")
            print("Creating one !!")
        ...
        for tetramer_sp in tetramer_sp_subsets:
            ...
            print("tetramer_sp : ", tetramer_sp)

            irrelevant, relevant = determine_pep_specificity_thresholds_for_multiple_subjects(
            ...
            for filename_aa in processed_file_aa_pval_names:
                ...
                produce_validation_bar_plot_aa(path_tables_and_figures_folder_exvivo, filename_aa, tetramer_sp=tetramer_sp, culture_thresholds=[relevant, irrelevant])
    ...
    if "8" in options['main_processing']:
        print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
        print('Creating medium only vs peptide stimulated sample p-values comparison plots')
        print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
    ...
    if "9" in options['main_processing']:
        print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
        print('Adding gene usage to previously created cell count and p-value files')
        print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
    ...
    if "10" in options['main_processing']:
        print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
        print('Thresholds without tetramer to validate')
        print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
    ...
    if "13" in options['main_processing']:
        print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
        print('Expansion thresholds identification clonotypes expanded in all repeats     ')
        print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
    ...
    if "14" in options['main_processing']:
        print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
        print('Plot mixture pure plots pvalue comparison                                  ')
        print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
    ...
    print("--- Program finishes at: %s ---" % asctime(localtime(time())))