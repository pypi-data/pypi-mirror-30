...
class ClonotypeSequencesInfoFile:
    ...
    def get_genome_counts_for_sample_id(self, sample_id):
        ...
        if genome_counts_df is not None:
            # print(sample_id)
            if sample_id == '':
                ValueError("Sample id is an empty string: " + sample_id)
            ...
            if len(gc_df) > 0:
                ...
                ValueError("No sample id in cloumns name : " + sample_id)
        return None
...
class ComparisonDataFrameFactory:
    ...
    def check_existing_file_name(self, sample_infos_path, file_names):
        ...
        print(l)
        file_names = [path.basename(file) for file in file_names]
        print(file_names)
        return setdiff1d(file_names, l)
...
#    @profile
    def merge(self):
        ...
        mylist = self.check_existing_file_name(self.input_path + 'metadata/sample_info.csv', adaptive_files)
        if len(mylist) > 0:
            raise ValueError("Samples not in sample info file: " + str(mylist))
