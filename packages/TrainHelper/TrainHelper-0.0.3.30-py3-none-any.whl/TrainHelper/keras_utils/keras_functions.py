...
def __get_model_checkpoint(base_name, monitor, save_best_only, save_weights_only=True):
    ...
    return keras.callbacks.ModelCheckpoint(base_name + '_best.h5', monitor=monitor, save_best_only=save_best_only,
                                           save_weights_only=save_weights_only, mode='auto', period=1)
...
def get_keras_callbacks(config):
    ...
    if config.ModelCheckpoint.Best:
        save_weights_only = config.ModelCheckpoint.WeightsOnly
        if save_weights_only:
            save_weights_only = True
        callbacks.append(__get_model_checkpoint(base_name, config.ModelCheckpoint.Monitor, True, save_weights_only))
