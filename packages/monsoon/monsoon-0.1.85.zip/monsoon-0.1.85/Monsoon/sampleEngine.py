...
class SampleEngine:
    ...
    def __getRecalibrationVariables(self,channel,coarse):
        if(channel == channels.MainCurrent):
            zeroCal = self.__mainCal.getZeroCal
            refCal = self.__mainCal.getRefCal
            voltageScale = self.__mainVoltageScale
            voltageIndex = self.__mainVoltageIndex
            if(coarse):
                currentIndex = self.__mainCoarseIndex
                setScale = self.monsoon.setMainFineScale
                setOffset = self.monsoon.setMainFineZeroOffset
            else:
                currentIndex = self.__mainFineIndex
                setScale = self.monsoon.setMainCoarseScale
                setOffset = self.monsoon.setMainCoarseZeroOffset
        elif(channel == channels.USBCurrent):
            raise NotImplementedError("USB recalibration not yet implemented.")
            pass
        elif(channel == channels.AuxCurrent):
            raise NotImplementedError("Aux recalibration not yet implemented.")
            pass
        else:
            raise ValueError("Unexpected channel value")
        return zeroCal,refCal,voltageScale,voltageIndex,currentIndex,setScale,setOffset

    def recalibrate(self, channel, coarse, loadResistor,minVoltage=2.0,maxVoltage=13.0,stepSize=1.0):
        """Recalibrates the Power Monitor.  Currently only supports main channel.
        This steps through a range of voltages, taking measurements against a load resistor.
        Expected current is calculated using Ohm's law (V=IR), and calibration output is calculated using linear regression.
        channel:  from sampleEngine.channels class
        coarse:  True = coarse channel, False = fine channel
        loadResistor:  Resistor connected to the channel under calibration.
        minVoltage:  Lowest value to be included in calibration.  Valid values 0.8 - 13.5V
        maxVoltage:  highest value to be included in the calibration.  Valid values = 0.8-13.5V
        stepSize: How much the voltage should be incremented at each step."""
        
        #Verify calibration values
        if(minVoltage < 0.8 or maxVoltage > 13.5):
            raise ValueError("Invalid voltage value")
        if(channel == channels.MainCurrent):
            if(coarse):
                if((minVoltage / loadResistor) * 1000 < 60):
                    errorString = "Minimum current should be more than 60 mA for coarse channel.  Calculated " + repr((minVoltage / loadResistor)*1000) + "mA"
                    raise ValueError(errorString)
                if(maxVoltage / loadResistor * 1000 > 6000):
                    errorString = "Maximum current should be no more than 6000 mA for coarse channel.  Calculated " + repr((minVoltage / loadResistor)*1000) + "mA"
                    raise ValueError(errorString)
            else:
                if(maxVoltage / loadResistor * 1000 > 60):
                    errorString = "Maximum current should be no more than 60 mA for fine channel.  Calculated " + repr((minVoltage / loadResistor)*1000) + "mA"
                    raise ValueError(errorString)
        if(channel == channels.AuxCurrent):
            if(maxVoltage >= 6):
                raise ValueError("Error:  Voltages exceeding 6V on Aux channel will damage the unit")
        
        
        X = [] #X terms for linear regression
        expectedCurrent = [] #Y terms for linear regression
        
        zeroCal,refCal,voltageScale,voltageIndex,currentIndex,setScale,setOffset = self.__getRecalibrationVariables(channel,coarse)
        for i in np.linspace(minVoltage,maxVoltage,(maxVoltage-minVoltage+1)/stepSize):
            self.monsoon.stopSampling()
            self.monsoon.setVout(i)
            self.monsoon.StartSampling(100)
            samples = []
            for p in range(20000):#TODO:  Look into shrinking this to speed things up.
                buffer = self.monsoon.BulkRead()
                for start in range(0,len(buffer),64):
                    if (self.__stopTriggerSet):
                        break
                    ...
                    Sample = self.monsoon.swizzlePacket(buf)
                    samples.append(Sample)
            bulkPackets = np.array(self.__processPacket(samples))
            if(len(bulkPackets > 1)):
                #Current = (Raw * Slope) + Intercept
                #Slope = (Scale - Zero) / (Ref - Zero)
                #Raw, Zero, and Ref are all known, scale and intercept are unknown, so we solve for them to calibrate the unit.
                Zero = zeroCal(coarse)
                Ref = refCal(coarse) - Zero
                Raw = np.mean(bulkPackets[:,currentIndex])
                if(Raw >= 65534): #Technically 65535, but this accounts for rounding errors from np.mean() 
                    raise ValueError("ADC Maxed out.  Use a smaller maxVoltage or a larger resistor")
                Raw = Raw - Zero
                voltage = np.mean(bulkPackets[:,voltageIndex] * self.__ADCRatio * voltageScale)
                expectedCurrent.append(voltage*1000 / loadResistor)
                X.append(Raw/Ref)
        slope, intercept, r, p, stderr = sp.stats.linregress(X,expectedCurrent)
        self.monsoon.stopSampling()
        self.monsoon.setVout(0)
        setScale(int(slope))
        setOffset(int(intercept))