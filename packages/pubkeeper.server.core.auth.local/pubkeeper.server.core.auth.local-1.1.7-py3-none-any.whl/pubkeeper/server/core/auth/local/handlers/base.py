...
class BaseHandler(web.RequestHandler):
    def prepare(self):
        ...
        self._db_connection = self.application.db.get_connection()
        self._db = self.application.db.get_session()
        self.token = None
    ...
    def get_current_user(self):
        ...
        user = self._db.query(User).filter_by(id=user_id.decode()).first()
        if user is not None:
            return user.id
        else:
            return None
