...
_default_encoding = _sys.getdefaultencoding()
_filesystem_encoding = _sys.getfilesystemencoding()
_stdin_encoding = _sys.stdin.encoding
_stdout_encoding = _sys.stdout.encoding
_stderr_encoding = _sys.stderr.encoding
_timezone_offset_seconds = -_time.timezone
_timezone_offset_hours = int(_timezone_offset_seconds / 3600)
_timezone_names = [
    _time.tzname[0].encode('latin-1').decode('euc-kr'),
    _time.tzname[1].encode('latin-1').decode('euc-kr')
]
...
class Datetime:
    ...
    def now(tz_offset_hours=None):
        ...
        # discard microsecond
        t = _time.time()
        d._millis = int(_math.floor(t * 1000))
    ...
    def from_epoch_millis(epoch_millis, tz_offset_hours=None):
        if type(epoch_millis) != int:
            raise TypeError('{}'.format(type(epoch_millis)))
    ...
    def from_local_millis(local_millis, tz_offset_hours=None):
        if type(local_millis) != str:
            raise TypeError('The local_millis must be str: %s' % (type(local_millis)))
        elif tz_offset_hours is None:
            tz = _TZ_LOCAL
            tz_offset_hours = _timezone_offset_hours
        else:
        ...
        # d._dt = datetime.strptime(local_millis+'000', '%Y-%m-%d %H:%M:%S.%f').replace(tzinfo=tz)
        ...
        d._dt = _datetime(
            ...
            int(local_millis[20:23]+'000'), tz
        )
        d._millis = int(_math.floor((d._dt - _datetime.fromtimestamp(0, tz)).total_seconds() * 1000))
        d._tz_offset_hours = tz_offset_hours
        return d

    @staticmethod
    def from_compact_millis(compact_millis, tz_offset_hours=None):
        if type(compact_millis) != str:
            raise TypeError('The compact_millis must be str: %s' % (type(compact_millis)))
        elif tz_offset_hours is None:
            tz = _TZ_LOCAL
            tz_offset_hours = _timezone_offset_hours
        else:
        ...
        d = Datetime()
        # qpython's locale module is not valid. and strptime doesn't work.
        # d._dt = datetime.strptime(compact_millis+'000', '%Y%m%d%H%M%S%f').replace(tzinfo=tz)
        # 20170101000000000
        # 01234567890123456
        d._dt = _datetime(
            ...
            int(compact_millis[14:17]+'000'), tz
        )
        d._millis = int(_math.floor((d._dt - _datetime.fromtimestamp(0, tz)).total_seconds() * 1000))
        d._tz_offset_hours = tz_offset_hours
        return d
    ...
    def from_iso_millis(iso_millis):
        if type(iso_millis) != str:
            raise TypeError('The iso_millis must be str: %s' % (type(iso_millis)))
        elif iso_millis[-1:] == 'Z':
            tz = _TZ_UTC
            tz_offset_hours = 0
    ...
    def to_iso_millis(self):
        ...
        return self._dt.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + tz
