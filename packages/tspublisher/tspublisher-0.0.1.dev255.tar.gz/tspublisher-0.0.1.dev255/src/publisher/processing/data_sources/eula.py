...
from publisher.settings import ENCYCLOPEDIA_EULA_DIR


def copy_eula_to_procedure(procedure_code, asset_directory):
    eula_file_name = "{0}_eula".format(procedure_code)
    source_file_path = os.path.join(ENCYCLOPEDIA_EULA_DIR, eula_file_name + ".txt")
    dest_file_path = os.path.join(asset_directory, eula_file_name + ".txt")

    for _, _, eula_files in os.walk(ENCYCLOPEDIA_EULA_DIR):
