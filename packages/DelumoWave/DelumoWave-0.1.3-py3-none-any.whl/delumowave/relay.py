...
class DelumoWaveRelay(DelumoWaveController):
    ...
    def invert(self) -> bool:
        ...
        try:
            ...
            if ex.errno == errno.EAGAIN:
                _LOGGER.debug(f'Radio controller {self._controller} is blocked.'
                              f' Try a bit later.')
                return False
            else:
                raise
    ...
    def dimmer_on(self) -> bool:
        ...
        try:
            procedure = self.RELAY['PROCEDURE']['GLOBAL_ON1']
            return self.send_cmd(node_addr=self._node_addr,
                                 procedure=procedure)
        except KeyError as ex:
            _LOGGER.error(f'{ex} is not supported for '
    ...
    def state(self, state: bool):
        ...
        try:
            ...
            _LOGGER.debug(f'Could not identify the state -> {hex_repr(state)}')
            raise
