...
class AlignmentStrategy(dynamit.integrationStrategy.IntegrationStrategy):
	...
	def doIntegration(self, sequences, searchResults):
		...
		pairsScores = {}
		for key in list(motifs.keys()):
			curKeyScores = []
			curSilScores = []
			for key2 in list(motifs.keys()):
				...
				if (key != key2) and (key2 + ", " + key not in pairsScores):
		...
		pairsOverThreshold = dict([(k, v) for k, v in list(pairsScores.items()) \
		...
		for cluster in list(set(clustering.labels_)):
			...
			clusterMotifs = [list(motifs.keys())[i] \
