...
class HeatmapPrinter(dynamit.resultsPrinter.ResultsPrinter):
	...
	def printResults(self, sequences, searchResults,
		...
		for motif in integrationResults["motifs"]:
			...
			for seq in list(seqLengths.keys()):
		...
		if self.drawBiclusters and "biclustering_motifs" in integrationResults:
		...
		plt.yticks(list(range(0, len(keys), 1)),
							 keys if (self.drawIndividualMotifs) else \
							 ["Cluster " + c for c in keys])
		plt.xticks(list(range(0, len(list(seqLengths.keys())), 1)),
							 [s[:15] for s in list(seqLengths.keys())], rotation=90)
		...
		# save heatmap values to a txt file sto that they can
		# be used for custom plotting with third-party tools.
		with open(os.path.join(os.getcwd(),
							"results_Heatmap" + plotType + ".txt"),"w") as txtOutHandle:
			txtOutHandle.write("# DynaMIT HeatmapPrinter\n")
			txtOutHandle.write("# This file contains the values used to plot the heatmap.\n")
			txtOutHandle.write("# " + ("Motifs" if (self.drawIndividualMotifs) else "Clusters") + \
												 " on rows, sequences on columns.\n")
			txtOutHandle.write("# drawBinaryHeatmap = " + str(self.drawBinaryHeatmap) + "\n")
			txtOutHandle.write("# drawIndividualMotifs = " + str(self.drawIndividualMotifs) + "\n")
			txtOutHandle.write("# drawBiclusters = " + str(self.drawBiclusters) + "\n")
			txtOutHandle.write("# Rows: " + ", ".join(keys if (self.drawIndividualMotifs) else \
												["Cluster " + c for c in keys]) + "\n")
			txtOutHandle.write("# Columns: " + ", ".join([s[:15] for s in list(seqLengths.keys())]) + "\n")
			# write the data, one motif/cluster per row.
			for row in heatmap:
				txtOutHandle.write(str(row) + "\n")

		# inform the user of results existence and location.
		print("  [HeatmapPrinter] Printing completed. Results are stored in \n" \
					"    <" + os.path.join(os.getcwd(),
					"results_Heatmap" + plotType + ".pdf") + ">  and" \
					"  <" + os.path.join(os.getcwd(),
					"results_Heatmap" + plotType + ".txt") + ">.")
		return 0
