...
def _flatten (xs, ys, original_only = False):
    assert  len (xs) == len (ys)
    _xs = []
    _ys = []
    for i, x_list in enumerate (xs):
        if original_only:
            x_list = [x_list [0]] # use original only
        for x in x_list:           
            _xs.append (x)
            _ys.append (ys [i])
...
def split_augset (total_xs, total_ys, test_size = 500):
    train_xs_0, test_xs, train_ys_0, test_ys = split (total_xs, total_ys, test_size)
    train_xs, train_ys = _flatten (train_xs_0, train_ys_0)
    valid_xs, valid_ys = _flatten (train_xs_0, train_ys_0, True)
    test_xs, test_ys = _flatten (test_xs, test_ys, True)
