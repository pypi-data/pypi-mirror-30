...
class DNN:
    ...
    def __validate (self, x, y, is_training, **kargs):
        logits = []
        costs = []
        accs = []
        for i in range (0, len (x), self.batch_size):
            ...
            logit, cost, acc =  self.run (self.logit, self.cost, self.accuracy, x = x_, y = y_, is_training = is_training, **kargs)
            logits.append (logit)
            costs.append (cost)
            accs.append (acc)  
        return np.concatenate(logits, 0), np.mean (costs), np.mean (accs)
    ...
    def train (self, x, y, **kargs):
        return self.__validate (x, y, True, **kargs)
        
    def valid (self, x, y, **kargs):
        _, cost, acc = self.__validate (x, y, False, **kargs)        
        self.overfitwatch.add_cost (cost)
