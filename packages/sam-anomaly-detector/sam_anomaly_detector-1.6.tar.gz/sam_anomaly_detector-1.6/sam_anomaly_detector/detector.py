...
class Detector:
    ...
    def _compare(self, actual: pd.DataFrame, forecast: pd.DataFrame) -> pd.DataFrame:
        ...
        if actual.empty or forecast.empty:
            syslog.syslog(syslog.LOG_ERR, 'actual data or forecast is empty')
            return pd.DataFrame()
        ...
        if actual_value > forecast_upper_bound or actual_value < forecast_lower_bound:
            ...
            if actual_value > forecast_upper_bound:
                change = actual_value - forecast_upper_bound
            else:
                change = actual_value - forecast_lower_bound

            forecast_range = forecast_upper_bound - forecast_lower_bound
            change = change / forecast_range
            anomaly['change'] = change
            severity = int(abs(change / (forecast_range / 6)))
