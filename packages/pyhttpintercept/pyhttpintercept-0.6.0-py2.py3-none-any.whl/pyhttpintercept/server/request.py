...
class HTTPRequestHandler(BaseHTTPRequestHandler, object):
    ...
    def handle(self):
        ...
        try:
            ...
            logging.exception(u'{t}: HTTPRequestHandler.handle error: {err}'.format(t=self.thread,
                                                                                    err=err))
    ...
    def finish(self):
        ...
        try:
            super(HTTPRequestHandler, self).finish()

        except socket.error as err:
            self.close_connection = 1  # Stop any further connections as we have a socket error
            logging.exception(u'{t}: HTTPRequestHandler.finish error: {err}'.format(t=self.thread,
                                                                                    err=err))
            ...
            try:
                ...
                logging.exception(u'{t}: HTTPRequestHandler.finish rfile error: {err}'.format(t=self.thread,
                                                                                              err=err))
