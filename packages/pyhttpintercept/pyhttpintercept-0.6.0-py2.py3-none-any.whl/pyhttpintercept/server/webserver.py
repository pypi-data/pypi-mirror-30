...
class HTTPWebServer(HTTPServer, object):
    ...
    def stop(self):
        ...
        t = Thread(target=self.pool.join)
        t.start()
    ...
    def process_request(self, request, client_address):
        ...
        if self._threading:
            self.pool.apply_async(func=self.process_request_threaded,
                                  args=(request, client_address))

        else:
            super(HTTPWebServer, self).process_request(request, client_address)
