...
class SubplotManager():
  ...
  def _reduce_view(self, df):
    ...
    if self.notebook.datdftype == "dioptas":
      frequency = 100#(notebook.seqdf.shape[0]-1) // 10
    elif self.notebook.datdftype == "dmc":
      frequency = df.shape[0]-1
    view = [int(i) for i in np.linspace(0, df.shape[0]-1, frequency)]
    view = set(view)
  ...
  def plotphasevolumefractions(self,  sm = "subplotmanager", title = "", ax = ""):
    ...
    """ plotting phase volume fraction for all nonmagnetic phases in model """
    ...
    self._setting_up_plot(ax, sm, title, ylabel_text = "phase weight frac. [%]")
