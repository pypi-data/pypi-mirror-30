...
def result(request, context, build_id):
    ...
    build_id = _agent_build_id(request, context, build_id)
    _result = request.registry['abuild'].result(build_id)
    request.registry['context'] = context
    status = u'a_end'
    if _result['status'] != 'a_end':
        status = u'{0} "{1}"'.format(
            _result['status'], _result['message'][:64])
    else:
        errors = [k[3] for k in _result['log'] if k[1] == 'a_error']
        if errors:
            status = u'a_error "{0}"'.format(errors[0][:64])
    log_activity(request, 'build_result', build_id, status)

    return '', _result


# =============================================================================
def buildspace_cleanup(request, context, root, filenames):
    ...
    # Clean directory
    root = join(
        request.registry.settings['buildspace.root'],
        _agent_paths(request, context, root)[0])
    if not exists(root):
        return '', ''

    for path, dirs, files in walk(root, topdown=False):
        for name in files:
            if normpath(join(relpath(path, root), name).decode('utf8')) \
                    not in filenames and exists(join(path, name)):
