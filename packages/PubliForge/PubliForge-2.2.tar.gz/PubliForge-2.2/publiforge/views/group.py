...
class GroupView(object):
    ...
    def admin(self):
        ...
        if action == 'imp!':
            ...
            log_activity(self._request, 'import_group')
            action = ''
        elif action[0:4] == 'del!':
        ...
        group_sizes = dict(
            DBSession.query(
                GROUP_USER.c.group_id, func.count(GROUP_USER.c.user_id))
            .group_by(GROUP_USER.c.group_id).all())
        ...
        return {
            ...
            'group_sizes': group_sizes, 'PAGE_SIZES': PAGE_SIZES,
    ...
    def view(self):
        ...
        return {
            'tab_set': tab_set, 'PERM_SCOPES': PERM_SCOPES,
            'USER_PERMS': USER_PERMS, 'group': group, 'members': members,
            'i_editor': has_permission(self._request, 'grp_editor')}
    ...
    def create(self):
        ...
        if form.validate():
            dbgroup = self._create(form.values)
            if dbgroup is not None:
                self._request.breadcrumbs.pop()
                log_activity(self._request, 'group_create', dbgroup.group_id)
        ...
        return {
            ...
            'PAGE_SIZES': PAGE_SIZES, 'group': None, 'members': None,
    ...
    def edit(self):
        ...
        if action == 'aur?' \
            ...
            self._add_members(group_id, items)
        elif action[0:4] == 'rur!':
            DBSession.execute(
                GROUP_USER.delete()
                .where(GROUP_USER.c.group_id == group_id)
                .where(GROUP_USER.c.user_id == int(action[4:])))
        ...
        # Environment
        dbgroup = DBSession.query(Group).filter_by(group_id=group_id).first()
        if dbgroup is None:
            raise HTTPNotFound()
        form, tab_set = self._settings_form(dbgroup)
        members = [k.user_id for k in dbgroup.users]
        if members:
            members = DBSession.query(User.user_id, User.login, User.name)\
                .filter(User.user_id.in_(members)).order_by(User.name).all()
        ...
        if action == 'sav!' and form.validate(dbgroup) \
                and self._save(dbgroup, form.values):
            log_activity(self._request, 'group_edit', dbgroup.group_id)
        ...
        return {
            ...
            'PERM_SCOPES': PERM_SCOPES, 'USER_PERMS': USER_PERMS,
            'PAGE_SIZES': PAGE_SIZES, 'group': dbgroup, 'members': members,
    ...
    def _delete_groups(self, group_ids):
        ...
        ids = set([k[0] for k in DBSession.query(GROUP_USER.c.group_id)
                   .filter(User.user_id == user_id)
                   .filter(User.user_id == GROUP_USER.c.user_id).all()])
        if group_ids & ids:
        ...
        # Delete
        deleted = []
        for dbgroup in DBSession.query(Group).filter(
                Group.group_id.in_(group_ids)):
            deleted.append(dbgroup.group_id)
            DBSession.delete(dbgroup)
        if deleted:
            DBSession.commit()
            log_activity(self._request, 'group_delete', u' '.join(deleted))
    ...
    def _export_groups(self, group_ids):
        ...
        elements = []
        exported = []
        for dbgroup in DBSession.query(Group)\
                .filter(Group.group_id.in_(group_ids)).order_by('label'):
            exported.append(dbgroup.group_id)
            elements.append(dbgroup.xml())

        name = '%s_groups.pfgrp' % self._request.registry.settings.get(
            'skin.name', 'publiforge')
        log_activity(
            self._request, 'group_export', u' '.join(exported))
    ...
    # -------------------------------------------------------------------------
    def _add_members(self, group_id, items):
        ...
        for user_id in items:
            ...
            if user_id not in user_ids:
                DBSession.execute(GROUP_USER.insert().values(
