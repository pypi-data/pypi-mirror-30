...
class HandlerManager(object):
    ...
    :param opener_manager: (:class:`~.lib.opener.OpenerManager`, optional)
        Global opener manager.
    ...
    ``{indexer_id: (value_type, extrators),...}``
    where ``extractors`` is a list of tuples like
    ``(indexed_files_regex, type, parameter, max_length)``.

    ``self._handlers`` is a dictionary of :class:`Handler` objects.
    """

    # -------------------------------------------------------------------------
    def __init__(self, settings, cache_manager, opener_manager=None):
        """Constructor method."""
        # Attributes
        self.settings = settings
        self.cache_manager = cache_manager
        self.opener_manager = opener_manager
        self._indexers = None
        self._handlers = {}
        self._thread = None
    ...
    # -------------------------------------------------------------------------
    def search(self, storage_id, fieldnames, query, limit=None):
        """Search in storage ``storage_id``.

        :param str storage_id:
        ...
        :rtype: list
        :return:
            A list of dictionaries. Each dictionary has the keys ``score``,
            ``storage_id`` and ``path`` plus the keys reprensenting the
            stored fields.
        """
        # Nothing to do
        ...
        with index.searcher() as searcher:
            ...
            for hit in results:
                result = dict([(k, hit[k]) for k in hit if k != 'time'])
                result.update({
                    'score': round(hit.score, 2), 'storage_id': storage_id})
        ...
        for path, dirs, files in walk(root):
            ...
            for name in files:
                if indexed_files.search(name) and name not in EXCLUDED_FILES:
                    ...
                    if name not in done:
                        self._add_document2index(writer, storage_id, name)
    ...
    def _open_or_create_index(self, storage_id):
        """Open and index structure (possibly create it).

        :param str storage_id:
            ID of index to open.
        :rtype: :class:`whoosh.index.Index`
        """
        # Open existing index
        index_dir = self.settings['storage.index']
        ...
        # Create schema
        schema = Schema(
            path=ID(unique=True, stored=True, sortable=True),
            time=STORED, filename=ID(stored=True), filetype=KEYWORD)
        done = set((u'path', u'time', u'filename', u'filetype'))
        for indexer in DBSession.query(Indexer):
            if indexer.indexer_id in done:
                continue
            if indexer.value_type == 'integer':
                schema.add(
                    indexer.indexer_id,
                    NUMERIC(stored=bool(indexer.result_column)))
            elif indexer.value_type == 'date':
                schema.add(
                    indexer.indexer_id,
                    DATETIME(stored=bool(indexer.result_column)))
            else:
                schema.add(
                    indexer.indexer_id,
                    TEXT(stored=bool(indexer.result_column)))
            done.add(indexer.indexer_id)
    ...
    # -------------------------------------------------------------------------
    def _add_document2index(self, writer, storage_id, path):
        """Add a document to the index.

        :type  writer: :class:`whoosh.writing.IndexWriter
        :param writer:
        ...
        # Index document
        name = basename(path)
        fullname = join(self.settings['storage.root'], storage_id, path)
        fields = {
            'path': path, 'time': time(), 'filename': name,
            'filetype': splitext(name)[1][1:]}
        content = tree = None
        self._load_indexers()
        for indexer_id in self._indexers:
            ...
            chunks = self._format_index_field(
                storage_id, fullname, indexer_id, chunks)
            if chunks:
    ...
    def _index_regex(cls, content, regex, limit, chunks):
        ...
        if limit:
            chunks[-1] = normalize_spaces(chunks[-1])[0:limit] + u'…'

    # -------------------------------------------------------------------------
    @classmethod
    def _index_iim(cls, fullname, fields, limit, chunks):
        ...
        :param list fields:
            Fields to extract.
        :param int limit:
            Maximum size of the extraction or ``None``.
        :param list chunks:
            List of chunks to complete.
        """
        metadata = image_metadata(fullname)
        chunk = u''
        if metadata.get('DocumentName') and (
                'DocumentName' in fields or 'Caption' in fields):
            chunk = u'{0}'.format(metadata['DocumentName'])
        if metadata.get('ImageDescription') and 'ImageDescription' in fields:
            chunk += u' {0}'.format(metadata['ImageDescription'])
        if not chunk:
            return

        chunks.append(normalize_spaces(chunk))
        if limit:
            chunks[-1] = chunks[-1][0:limit] + u'…'

    # -------------------------------------------------------------------------
    @classmethod
    def _index_xpath(cls, tree, xpath, limit, chunks):
        """Extract text from ``tree`` according to ``xpath``.

        :type  tree: :class:`ElementTree`
        :param tree:
            Content to process.
        :param str xpath:
            XPath to use.
        ...
        if isinstance(elements, list):
            if isinstance(elements[0], etree._Element):
                elements = u' '.join([
                    isinstance(k.text, unicode) and k.text
                    or k.text.encode('utf8') for k in elements if k.text])
            else:
                elements = u' '.join([k for k in elements if k.strip()])
        if isinstance(elements, (float, int)):
            elements = str(int(elements))
        else:
            if not elements.strip():
                return
            if limit:
                elements = normalize_spaces(elements)[0:limit] + u'…'

        chunks.append(elements)

    # -------------------------------------------------------------------------
    def _format_index_field(self, storage_id, fullname, indexer_id, chunks):
        """Format index field according to type.

        :param str storage_id:
            ID of the current storage.
        :param str fullname:
            Absolute path to indexed file.
        :param str indexer_id:
            Indexer ID.
        :param list chunks:
        ...
        if indexer_id == 'image':
            ...
            chunks = (chunks and chunks[0].split()[0]) or \
                (splitext(fullname)[1] in extensions and
                 splitext(basename(fullname))[0])
            if chunks:
                regex = re.compile(
                    r'{0}\.(png|jpg|jpeg|svg|gif)'.format(chunks))
                for path, name, files in walk(dirname(fullname)):
                    name = [k for k in files if regex.match(k)]
                    if name:
                        return relpath(
                            join(path, name[0]),
                            join(self.settings['storage.root'], storage_id))
                chunks = ''

        if not chunks:
            return ''

        if self._indexers[indexer_id][0] == 'boolean':
            chunks = u'TRUE'
        elif self._indexers[indexer_id][0] == 'integer':
            chunks = int(chunks[0]) if chunks[0].isdigit() else 0
        elif self._indexers[indexer_id][0] == 'date':
            chunks = (u'%04d') % (int(chunks[0])) \
                if chunks[0].isdigit() else chunks[0].strip()
...
class Handler(object):
    ...
    * ``'end'``: VCS operation ended with success
    ...
    def __init__(self, handler_manager, storage):
        ...
        self.opener_manager = handler_manager.opener_manager
        ...
        self.normalize_mode = storage.normalize_mode
        self._report = (
            'wait', None, 0,
    ...
    # -------------------------------------------------------------------------
    @cache_decorator()
    def dir_infos(self, request, path, sort, quick, storage):
        ...
        :param storage: (dictionary)
            Dictionary of the current storage.
        :return: (tuple)
            A tuple such as ``(dirs, files)`` where ``dirs`` and ``files`` are
            lists such as ``[name, type, size, date, revision, user,
            message, overview]``. For instance: ``['README', 'plain',
            '45.7 Kio', '2 days', '16:0e0229a916f4', 'user1', 'Bug fixed',
            '<div class="overview">See...</div>']``.
        """
        ...
        # Improve labels and add overviews
        for item in dirs:
        ...
        for item in files:
            ...
            item[3] = (age(item[3]), item[3].isoformat(' ').partition('.')[0])
            opener = not quick and self.opener_manager and \
                self.opener_manager.get_opener(
                    join(self.vcs.path, path, item[0]), storage)[0]
            item.append(opener and opener.overview(
                request, storage, join(path, item[0])) or None)
    ...
    # -------------------------------------------------------------------------
    def upload(self, user, path, upload_files, filename, message):
        """Synchronize, upload files and propagate.

        :param user: (list)
            VCS user like ``(vcs_user_id, vcs_password, user_name)``.
        :param path: (string)
            Relative path to files.
        :param upload_files: (list)
            List fo :class:`cgi.FieldStorage` of the file to upload.
        ...
        for upload_file in upload_files:
            ...
            if self.normalize_mode is not None:
                upload_filename = normalize_filename(
                    upload_filename, self.normalize_mode)
            if filename is not None and filename != upload_filename:
                self.report('error', _('File names are different.'))
                return

            # Upload files
            ext = splitext(upload_filename)[1]
            if ext != '.zip' or filename:
                upload_filename = filename or upload_filename
                with open(join(fullpath, upload_filename), 'w') as hdl:
                    hdl.write(upload_file.file.read())
            else:
                unzip(upload_file.file, fullpath, self.normalize_mode)
    ...
    def create(self, user, seed, path, name, message):
        ...
        # Create file
        if self.normalize_mode is not None:
            name = normalize_filename(name, self.normalize_mode)
    ...
    def mkdir(self, user, path, name, message):
        ...
        # Make directory
        if self.normalize_mode is not None:
            name = normalize_filename(name, self.normalize_mode, True)
    ...
    def add(self, user, path, message):
        ...
        # Check repository
        if not self._check_repository(user, message):
            self.vcs.remove_untracked(path, self)
            return
