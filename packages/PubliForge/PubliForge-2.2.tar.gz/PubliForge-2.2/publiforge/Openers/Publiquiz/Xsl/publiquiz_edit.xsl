  ...
  <xsl:template match="var" mode="variables">
    ...
    <xsl:variable name="default">
      <xsl:choose>
        ...
        <xsl:when test="@name='blanks-media'">
          <xsl:copy-of select="$node/blanks-media"/>
        </xsl:when>
        <xsl:when test="@name='blanks-choices'">
          <xsl:copy-of select="$node/blanks-choices"/>
        </xsl:when>
        <xsl:when test="@name='correct-line'">
          <xsl:copy-of select="$node/correct-line"/>
        </xsl:when>
        ...
        <xsl:when test="@name='production'">
          <xsl:copy-of select="$node/production/node()"/>
        </xsl:when>

        <xsl:when test="@name='wordsearch'">
          <xsl:copy-of select="$node/wordsearch"/>
        </xsl:when>

        <xsl:when test="@name='flashcard'">
          <xsl:copy-of select="$node/flashcard"/>
        </xsl:when>

        <xsl:when test="@name='coloring'">
          <xsl:copy-of select="$node/coloring"/>
        </xsl:when>

        <xsl:when test="@name='memory'">
          <xsl:copy-of select="$node/memory"/>
        </xsl:when>
