...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="var_default">
    ...
    <xsl:choose>
      ...
      <xsl:when test="@name='identifier_ean_epub'">
        <xsl:copy-of
            select="$node/head/identifier[@type='ean' and @for='epub']/node()"/>
      </xsl:when>
      <xsl:when test="@name='identifier_ean_print'">
        <xsl:copy-of
            select="$node/head/identifier[@type='ean' and @for='print']/node()"/>
      </xsl:when>
      ...
      <!-- Corpus -->
      <xsl:when test="@name='document' or @name='topic' or @name='quiz' or
                      @name='glossary'">
        <xsl:copy-of
            select="$node/division|$node/topic|$node/quiz|$node/glossary
