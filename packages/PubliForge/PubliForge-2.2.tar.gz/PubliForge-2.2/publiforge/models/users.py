...
class User(Base):
    ...
    perms = relationship('UserPerm', cascade='all, delete')
    ips = relationship('UserIP', cascade='all, delete')
    selection = relationship('UserFile', cascade='all, delete')
    ...
    def __init__(self, settings, login, status, password, name, email,
        ...
        self.login = normalize_name(login)[0:ID_LEN].lower()
        self.status = status
        self.set_password(settings, password)
        self.name = normalize_spaces(name, LABEL_LEN)
        self.email = email[0:LABEL_LEN].lower()
        self.lang = lang[0:5]
        self.expiration = datetime.strptime(expiration, '%Y-%m-%d') \
            if isinstance(expiration, basestring) else expiration
    ...
    def setup_environment(self, request):
        ...
        * ``perms``: list of permissions
        ...
        # Language
        langs = settings_get_list(
            request.registry.settings, 'languages', ['en'])
        request.session['lang'] = \
            (self.lang in langs and self.lang) or \
            (self.lang[0:2] in langs and self.lang[0:2]) or \
        ...
        request.session['perms'] = ('admin', ) \
            if perms.get('all') == 'admin' \
            else tuple([('%s_%s') % ((k, perms[k])) for k in perms])
    ...
    def load(cls, settings, user_elt, error_if_exists=True):
        ...
        record = {
            ...
            settings.get('pyramid.default_locale_name', 'en'),
            'expiration':
        ...
        for item in user_elt.findall('selection/file'):
            if item.text.strip() not in done:
                user.selection.append(UserFile(item.text.strip()))
                done.add(item.text.strip())
    ...
    def xml(self, i_manager=False):
        ...
        # Selection
        if self.selection:
            elt = etree.SubElement(user_elt, 'selection')
            for item in self.selection:
                etree.SubElement(elt, 'file').text = item.path
...
class UserPerm(Base):
    ...
    def __init__(self, user_id, scope, level):
        ...
        self.scope = 'all' if level == 'admin' else scope
        self.level = level
