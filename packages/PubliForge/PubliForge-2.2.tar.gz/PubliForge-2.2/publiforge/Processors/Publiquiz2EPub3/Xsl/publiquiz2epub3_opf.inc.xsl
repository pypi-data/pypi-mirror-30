...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="css_manifest">
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <item id="c_publiquiz" href="{$css_dir}publiquiz.css" media-type="text/css"/>
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:if test="$aud and $aud_custom">
          <item id="j_audio_player" href="{$js_dir}audio_player.js" media-type="application/javascript"/>
        </xsl:if>
  ...
  <!--
      =========================================================================
      Template fonts_manifest
      =========================================================================
  -->
  <xsl:template name="fonts_manifest">
    <item id="f_fontawesome_otf"  href="Fonts/FontAwesome.otf" media-type="application/vnd.ms-opentype"/>
    <item id="f_fontawesome_ttf" href="Fonts/FontAwesome.ttf" media-type="application/vnd.ms-opentype"/>
  </xsl:template>
    ...
    <guide>
      ...
      <xsl:choose>
        <xsl:when test="../quiz or name(.//*[name()='topic' or name()='quiz'])='quiz'">
          <reference type="text" href="{$fid}-quz-0001{$html_ext}" title="{$i18n_text}"/>
        </xsl:when>
        <xsl:otherwise>
          <reference type="text" href="{$fid}-tpc-0001{$html_ext}" title="{$i18n_text}"/>
        </xsl:otherwise>
      </xsl:choose>
      <xsl:if test="$index and .//index">
