...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
          ...
          <xsl:choose>
            ...
            <xsl:otherwise>
              <navPoint id="navPoint-1" playOrder="1">
                ...
                <xsl:choose>
                  <xsl:when test="../quiz|quiz"><content src="{$fid}-quz-0001{$html_ext}"/></xsl:when>
                  <xsl:when test="topic/processing-instruction('tune-html-replace-by')">
                    <content src="{normalize-space(topic/processing-instruction('tune-html-replace-by'))}"/>
                  </xsl:when>
                  <xsl:otherwise><content src="{$fid}-tpc-0001{$html_ext}"/></xsl:otherwise>
                </xsl:choose>
              </navPoint>
            </xsl:otherwise>
          </xsl:choose>
        ...
        <navPoint id="navPoint-{$number}" playOrder="{$number}">
          ...
          <xsl:choose>
            <xsl:when test="$toc_division_depth&gt;count(ancestor::division)">
              <content
                  src="{$fid}-div-{format-number(count(preceding::division|ancestor::division)+1, '0000')}{$html_ext}"/>
            </xsl:when>
            <xsl:when test="name(.//*[name()='topic' or name()='quiz'])='quiz'">
              <content
                  src="{$fid}-quz-{format-number(count(preceding::quiz)+1, '0000')}{$html_ext}#level{count(ancestor::division)+1}"/>
            </xsl:when>
            <xsl:when test=".//topic[1]/processing-instruction('tune-html-replace-by')">
              <content
                  src="{normalize-space(.//topic[1]/processing-instruction('tune-html-replace-by'))}"/>
            </xsl:when>
            <xsl:otherwise>
              <content
                  src="{$fid}-tpc-{format-number(count(preceding::topic)+1, '0000')}{$html_ext}#level{count(ancestor::division)+1}"/>
            </xsl:otherwise>
          </xsl:choose>
        ...
        <navPoint id="navPoint-{$number}" playOrder="{$number}">
          ...
          <xsl:choose>
            <xsl:when test="processing-instruction('tune-html-replace-by')">
              <content src="{substring-after(
                            normalize-space(processing-instruction('tune-html-replace-by')),
                            'Container~/OEBPS/')}"/>
            </xsl:when>
            <xsl:otherwise>
              <content src="{$fid}-tpc-{format-number(count(preceding::topic)+1, '0000')}{$html_ext}"/>
            </xsl:otherwise>
          </xsl:choose>
  ...
  <xsl:template match="quiz" mode="toc_ncx">
    <xsl:if test="head/title">
      ...
      <navPoint id="navPoint-{$number}" playOrder="{$number}">
        ...
        <content
            src="{$fid}-quz-{format-number(count(preceding::quiz)+1, '0000')}{$html_ext}"/>
      </navPoint>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template match="bibliography" mode="toc_ncx">
    ...
    <navPoint id="navPoint-{$number}" playOrder="{$number}">
      ...
      <content
          src="{$fid}-tpc-{format-number(count(preceding::topic)+1, '0000')}{$html_ext}#biblio"/>
    </navPoint>
  </xsl:template>
