...
royalty free, license to use and redistribute this software in source and
binary code form, provided that i) this copyright notice and license appear on
all copies of the software; and ii) Licensee does not utilize the software in a
...
NON-INFRINGEMENT, ARE HEREBY EXCLUDED.  ADL AND ITS LICENSORS SHALL NOT BE
...
INABILITY TO USE SOFTWARE, EVEN IF ADL HAS BEEN ADVISED OF THE POSSIBILITY OF
SUCH DAMAGES.
...
var apiHandle = null;
var output = console; // output can be set to any object that has a log(string)
                      // function such as:
                      // var output = { log: function(str){alert(str);} };
...
function doInitialize()
{
    ...
    if (result.toString() != "true") {
        ...
        if (err.code == 103) return "true";
        error("Initialize failed with error",  err);
    }
    else {
...
function doTerminate()
{
    ...
    if (api == null) {
        ...
        if (result.toString() != "true") {
            var err = ErrorHandler();
            error("Terminate failed with error",  err);
        }
    }
...
function doGetValue(name)
{
    ...
    else if (!scormInitialized && ! doInitialize()) {
        err = ErrorHandler();
        error("GetValue failed -"
              + " Could not initialize communication with the LMS", err);
    }
    else {
        result = api.GetValue(name);
        err = ErrorHandler();
        if (err.code != _NoError.code) {
            error("GetValue("+name+") failed", err);
            result = "";
        }
    }
    return result.toString();
...
function doSetValue(name, value)
{
    ...
    if (api == null) {
        ...
        {
            err = ErrorHandler();
            error("SetValue("+name+", "+value+") failed", err);
        }
    }
...
function doGetDiagnostic(errorCode)
{
    ...
    if (api == null) {
        ...
        return "Unable to locate the LMS's API Implementation. "
            + "GetDiagnostic was not successful.";
    }
...
function ErrorHandler()
{
    ...
    if (api == null) {
        ...
        error.diagnostic = "Unable to locate the LMS's API Implementation. "
            + "Cannot determine LMS error code.";
        return error;
    }
 ...
 ** Inputs:  String - message you want to send to the designated output
 **          Object - error you want to send to the designated output
 ** Return:  none
 ** Depends on: boolean debug to indicate if output is wanted
 **             object output to handle the messages. must implement a function
 **             log(string)
 **
 ** Description:
 ** This function outputs error to a specified output.
 *****************************************************************************/

function error(str, err)
{
    if (debug)
        output.log(str + ": (" + err.code + ") " + err.string
                   + " [" + err.diagnostic + "]" );
}
