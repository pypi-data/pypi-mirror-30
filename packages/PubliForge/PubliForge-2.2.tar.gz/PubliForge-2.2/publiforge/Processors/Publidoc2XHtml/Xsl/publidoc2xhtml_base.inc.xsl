...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
      ...
      <xsl:attribute name="href">
        <xsl:choose>
          ...
          <xsl:when test="not(ancestor::indexset)">
            <xsl:value-of select="concat($fid, $parent, $html_ext,
                                  '#idx', count(preceding::index))"/>
          </xsl:when>
          ...
          <xsl:when test="name(ancestor::head/..)='division'
            ...
            <xsl:value-of
                select="concat($fid, '-div-',
                        format-number(count(preceding::division|ancestor::division), '0000'),
                        $html_ext)"/>
          </xsl:when>
          <xsl:when test="name(ancestor::head/..)='division'">
            <xsl:value-of
                select="concat($fid, $parent, $html_ext,
                        '#div', count(preceding::division|ancestor::division))"/>
          </xsl:when>

          <!-- topic -->
          <xsl:when test="$onefile and (name(ancestor::head/..)='topic'
                          or name(ancestor::head/..)='quiz')">
          ...
          <xsl:when test="name(ancestor::head/..)='section'">
            <xsl:value-of
                select="concat($fid, $parent, $html_ext,
                        '#sect', count(preceding::section|ancestor::section)-1)"/>
          </xsl:when>
  ...
  <xsl:template match="division" mode="toc">
    <xsl:choose>
      <xsl:when test="$toc_division_depth&gt;count(ancestor::division)
        ...
        <li>
          <a href="{$fid}-div-{format-number(count(preceding::division|ancestor::division)+1, '0000')}{$html_ext}">
            <xsl:call-template name="division_toc_title"/>
          </a>
          <xsl:if test="$has_children='1'">
            <ul>
  ...
  <xsl:template match="division" mode="file">
    <xsl:if test="$toc_division_depth&gt;count(ancestor::division)">
      ...
      <xsl:call-template name="html_file">
        <xsl:with-param
            name="name"
            select="concat($fid, '-div-', format-number(
                    count(preceding::division|ancestor::division)+1, '0000'))"/>
  ...
  <!--
      =========================================================================
      document, topic mode meta
      =========================================================================
  -->
  <xsl:template match="document|topic" mode="meta">
    <xsl:if
      ...
      <table class="pdocMeta">
        <xsl:apply-templates select="@id|../@id" mode="meta_item"/>
        <xsl:apply-templates
            select="@type|../@type|@xml:lang|../@xml:lang
    ...
    <tr>
      <th>
        <xsl:choose>
          ...
          <xsl:when test="name()='type'"><xsl:value-of select="$i18n_type"/></xsl:when>
          ...
          <xsl:when test="name()='identifier' and @type='ean' and @for='epub'">
            <xsl:value-of select="$i18n_identifier_ean_epub"/>
          </xsl:when>
          <xsl:when test="name()='identifier' and @type='ean' and @for='print'">
            <xsl:value-of select="$i18n_identifier_ean_print"/>
          </xsl:when>
          <xsl:when test="name()='identifier' and @type='ean'">
            <xsl:value-of select="$i18n_identifier_ean"/>
            <xsl:if test="@for"><xsl:value-of select="concat(' ', @for)"/></xsl:if>
          </xsl:when>
          <xsl:when test="name()='identifier' and @type='uri'">
            <xsl:value-of select="$i18n_identifier_uri"/>
            <xsl:if test="@for"><xsl:value-of select="concat(' ', @for)"/></xsl:if>
          </xsl:when>
  ...
  <xsl:template match="section">
    <div>
      ...
      <xsl:apply-templates
          select="section|p|list|blockquote|speech
                  |table|media|processing-instruction()"/>
  ...
  <xsl:template match="bibliography" mode="toc">
    <li>
      <a href="{$fid}-tpc-{format-number(count(preceding::topic)+1, '0000')}{$html_ext}#biblio">
        <xsl:value-of select="$i18n_bibliography"/>
      </a>
    </li>
  </xsl:template>
  ...
  <xsl:template match="glossary">
    <div class="pdocGlossary">
      <h2 class="pdocTitle"><xsl:value-of select="$i18n_glossary"/></h2>
      <xsl:apply-templates select="entry" mode="glossary"/>
    </div>
  </xsl:template>
  ...
  <xsl:template match="p">
    <xsl:choose>
      <xsl:when test="processing-instruction('hold')">
        <xsl:text> </xsl:text>
      </xsl:when>
      <xsl:otherwise>
        <p>
  ...
  <xsl:template match="entry" mode="glossary">
    <div class="pdocGlossEntry">
      <h3 class="pdocGlossEntryTitle">
        <xsl:value-of select="mainterm"/>
      </h3>
      <xsl:if test="alt-terms">
        <div class="pdocAlt-terms">
          <xsl:text>(</xsl:text>
          <xsl:for-each select="alt-terms/alt-term">
            <xsl:value-of select="."/>
      ...
      <xsl:if test="seealso">
        <div class="pdocSeealso">
          <span class="pdocSeealsoLabel">
            <xsl:value-of select="concat($i18n_seealso, ' : ')"/>
          </span>
  ...
  <xsl:template match="meanings">
    <div class="pdocMeanings">
      <xsl:if test="@gramcode">
        <div class="pdocGramcode"><xsl:value-of select="@gramcode"/></div>
      </xsl:if>
  ...
  <xsl:template match="meaning">
    <div class="pdocMeaning">
      <xsl:text> </xsl:text>
      <xsl:if test="@domain">
        <div class="pdocMeaningDomain"><xsl:value-of select="@domain"/></div>
      </xsl:if>
      <xsl:if test="./definition">
        <div class="pdocMeaning-definition">
          <span class="pdocMeaningLabel pdocMeaningLabel-definition">
            <xsl:value-of select="concat($i18n_definition, ' : ')"/>
          </span>
          <xsl:apply-templates select="./definition"/>
        </div>
      </xsl:if>
      <xsl:if test="./example">
        <div class="pdocMeaning-example">
          <span class="pdocMeaningLabel pdocMeaningLabel-example">
            <xsl:value-of select="concat($i18n_example, ' : ')"/>
          </span>
      ...
      <xsl:if test="./synonym">
        <div class="pdocMeaning-synonym">
          <span class="pdocMeaningLabel pdocMeaningLabel-synonym">
            <xsl:value-of select="concat($i18n_synonym, ' : ')"/>
          </span>
          <xsl:apply-templates select="./synonym"/>
        </div>
      </xsl:if>
      <xsl:if test="./antonym">
        <div class="pdocMeaning-antonym">
          <span class="pdocMeaningLabel pdocMeaningLabel-antonym">
            <xsl:value-of select="concat($i18n_antonym, ' : ')"/>
          </span>
          <xsl:apply-templates select="./antonym"/>
        </div>
      </xsl:if>
      <xsl:if test="./translation">
        <div class="pdocMeaning-translation">
          <span class="pdocMeaningLabel pdocMeaningLabel-translation">
            <xsl:value-of select="concat($i18n_translation, ' : ')"/>
          </span>
      ...
      <xsl:if test="./dictum">
        <div class="pdocMeaning-antonym">
          <xsl:apply-templates select="./dictum"/>
        </div>
      </xsl:if>
    </div>
  </xsl:template>
  ...
  <xsl:template match="note">
    <xsl:choose>
      ...
      <xsl:otherwise>
        <a href="{$fid}-not-{format-number(count(preceding::note)+1, '0000')}{$html_ext}"
  ...
  <xsl:template match="note" mode="call">
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <sup>
          <xsl:value-of
              select="concat($str_notecall_open,
  ...
  <xsl:template name="note_file">
    <xsl:call-template name="html_file">
      <xsl:with-param name="name">
        <xsl:value-of select="concat($fid, '-not-',
                              format-number(count(preceding::note)+1, '0000'))"/>
      </xsl:with-param>
      <xsl:with-param name="title">
        <xsl:apply-templates select="." mode="title"/>
      </xsl:with-param>
  ...
  <xsl:template match="link" mode="href">
    ...
    <xsl:if test="@target">
      <xsl:attribute name="target">
        <xsl:value-of select="concat('_', @target)"/>
      </xsl:attribute>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="link_idref">
    ...
    <xsl:choose>
      <xsl:when test="not($onefile) and name($target)='topic'">
        <xsl:value-of
            select="concat($fid, '-tpc-',
                    format-number(count($target/preceding::topic)+1, '0000'),
                    $html_ext)"/>
      </xsl:when>
  ...
  <xsl:template match="example">
    <em class="pdocExample"><xsl:apply-templates/></em>
  </xsl:template>
  <xsl:template match="example" mode="link">
    <em class="pdocExample"><xsl:apply-templates mode="link"/></em>
  </xsl:template>

  <xsl:template match="nowrap">
    <span class="pdocNoWrap">
      <xsl:apply-templates/>
    </span>
  </xsl:template>
