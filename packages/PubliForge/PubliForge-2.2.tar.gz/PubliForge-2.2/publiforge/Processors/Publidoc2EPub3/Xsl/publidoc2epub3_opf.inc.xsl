...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="metadata">
    <metadata xmlns:dc="http://purl.org/dc/elements/1.1/"
      ...
      <xsl:if test="$publisher_label
                    and not(head/contributors/contributor[role='author'])
                    and not(head/contributors/contributor[role='publisher']/label)">
        <dc:creator id="creator{count(head/contributors/contributor)}">
          <xsl:value-of select="$publisher_label"/>
        </dc:creator>
        <meta property="role"
              refines="#creator{count(head/contributors/contributor)}"
      ...
      <meta property="dcterms:modified">
        <xsl:choose>
          <xsl:when test="contains(date:date-time(), '+')">
  ...
  <xsl:template name="rendition">
    <xsl:if test="$rendition_orientation!='auto'">
      <meta property="rendition:orientation">
        <xsl:value-of select="$rendition_orientation"/>
      </meta>
      <meta name="orientation" content="{$rendition_orientation}"/>
    </xsl:if>
    <xsl:if test="$rendition_spread!='auto'">
      <meta property="rendition:spread">
        <xsl:value-of select="$rendition_spread"/>
      </meta>
    </xsl:if>
    <xsl:if test="$rendition_flow!='auto'">
      <meta property="rendition:flow">
        <xsl:value-of select="$rendition_flow"/>
      </meta>
    </xsl:if>
  </xsl:template>
    ...
    <xsl:for-each select="role">
      <xsl:choose>
        ...
        <xsl:when test=".='publisher'">
          <xsl:if test="not(../../contributor/role='author')">
        ...
        <xsl:otherwise>
          ...
          <meta property="role" refines="#creator{count(preceding::contributor)}"
            ...
            <xsl:choose>
              ...
              <xsl:when test=".='translator'">trl</xsl:when>
              <xsl:otherwise>aut</xsl:otherwise>
            </xsl:choose>
          </meta>
          <meta property="file-as" refines="#creator{count(preceding::contributor)}">
            <xsl:apply-templates select="../lastname|../label" mode="text"/>
            <xsl:if test="../firstname">
              <xsl:text>, </xsl:text>
              <xsl:apply-templates select="../firstname" mode="text"/>
  ...
  <xsl:template name="manifest">
    <manifest>
      ...
      <xsl:if test="$js and $aud and $aud_custom">
        <item id="i_audio_fr_png" href="Images/audio_fr.png" media-type="image/png"/>
        <item id="i_audio_en_png" href="Images/audio_en.png" media-type="image/png"/>
        <item id="i_audio_play" href="Images/audio_play.png" media-type="image/png"/>
        <item id="i_audio_pause" href="Images/audio_pause.png" media-type="image/png"/>
      </xsl:if>
      <!-- Math -->
      <xsl:if test="$math">
      ...
      <!-- Fonts -->
      <xsl:call-template name="fonts_manifest"/>
      <!-- Extra -->
  ...
  <xsl:template match="division" mode="manifest">
    ...
    <item id="h_{translate($fid, ' ', '_')}-div{count(preceding::division|ancestor::division)+1}"
          href="{$fid}-div-{format-number(count(preceding::division|ancestor::division)+1, '0000')}{$html_ext}"
    ...
    <xsl:choose>
      <xsl:when test="processing-instruction('tune-html-replace-by')">
        <item id="h_{translate($fid, ' ', '_')}-tpc{count(preceding::topic)+1}"
              href="{substring-after(
                    normalize-space(processing-instruction('tune-html-replace-by')),
                    'Container~/OEBPS/')}"
              media-type="application/xhtml+xml"/>
      </xsl:when>
      ...
      <xsl:otherwise>
        <item id="h_{translate($fid, ' ', '_')}-tpc{count(preceding::topic)+1}"
              href="{$fid}-tpc-{format-number(count(preceding::topic)+1, '0000')}{$html_ext}"
              media-type="application/xhtml+xml">
          <xsl:if test="$js or ($img_ext='.svg' and .//media/image) or
                        normalize-space(.//image/processing-instruction('tune-html-img-format'))='svg'">
            <xsl:attribute name="properties">
              <xsl:if test="$js">scripted</xsl:if>
              <xsl:if test="($img_ext='.svg' and .//media/image) or
                            normalize-space(.//image/processing-instruction('tune-html-img-format'))='svg'">
        ...
        <xsl:if test="$aud and
          ...
          <item id="s_{translate($fid, ' ', '_')}-tpc{count(preceding::topic)+1}"
                href="{$fid}-tpc-{format-number(count(preceding::topic)+1, '0000')}.smil"
  ...
  <xsl:template match="quiz" mode="manifest">
    <item id="h_{translate($fid, ' ', '_')}-quz{count(preceding::quiz)+1}"
          href="{$fid}-quz-{format-number(count(preceding::quiz)+1, '0000')}{$html_ext}"
          media-type="application/xhtml+xml">
  ...
  <xsl:template match="note" mode="manifest">
    <item id="h_{translate($fid, ' ', '_')}-not{count(preceding::note)+1}"
          href="{$fid}-not-{format-number(count(preceding::note)+1, '0000')}{$html_ext}"
          media-type="application/xhtml+xml"/>
  </xsl:template>
  ...
  <xsl:template name="guide">
    <guide>
      ...
      <!--                   and count(.//topic)&gt;2"> -->
      <!--     <reference type="text" href="{$fid}-tpc-003{$html_ext}" title="{$i18n_text}"/> -->
      <!--   </xsl:when> -->
      <!--   <xsl:when test=".//topic[@type='title' and count(preceding::topic)=0] -->
      <!--                   and count(.//topic)&gt;1"> -->
      <!--     <reference type="text" href="{$fid}-tpc-0002{$html_ext}" title="{$i18n_text}"/> -->
      <!--   </xsl:when> -->
      <!--   <xsl:when test="//topic"> -->
      ...
      <!-- </xsl:choose> -->
      <xsl:choose>
        <xsl:when test=".//topic[1]/processing-instruction('tune-html-replace-by')">
          <reference type="text"
                     href="{normalize-space(.//topic[1]/processing-instruction('tune-html-replace-by'))}"
                     title="{$i18n_text}"/>
        </xsl:when>
        <xsl:otherwise>
          <reference type="text" href="{$fid}-tpc-0001{$html_ext}" title="{$i18n_text}"/>
  ...
  <xsl:template match="topic" mode="guide">
    <xsl:choose>
      <!-- Title -->
      <xsl:when test="processing-instruction('tune-html-replace-by')
                      and @type='title' and not(head/title)
                      and not(preceding::topic[@type='title'])">
        <reference type="{@type}"
                   href="{normalize-space(processing-instruction('tune-html-replace-by'))}"
      ...
      <xsl:when test="processing-instruction('tune-html-replace-by')
                      and @type='title' and not(preceding::topic[@type='title'])">
        <reference type="{@type}"
                   href="{normalize-space(processing-instruction('tune-html-replace-by'))}"
                   title="{head/title}"/>
      </xsl:when>
      ...
      <!-- Copyright -->
      <xsl:when test="processing-instruction('tune-html-replace-by')
                      and @type='copyright' and not(head/title)">
        <reference type="{@type}"
                   href="{normalize-space(processing-instruction('tune-html-replace-by'))}"
