            ...
            <ol>
              ...
              <xsl:choose>
                ...
                <xsl:when test="../quiz or name(.//*[name()='topic' or name()='quiz'])='quiz'">
                  <li><a href="{$fid}-quz-0001{$html_ext}">
                    <xsl:call-template name="corpus_title"/>
                  </a></li>
                </xsl:when>
                <xsl:when test=".//topic[1]/processing-instruction('tune-html-replace-by')">
                  <li><a href="{substring-after(
                               normalize-space(.//topic[1]/processing-instruction('tune-html-replace-by')),
                               'Container~/OEBPS/')}">
                    <xsl:call-template name="corpus_title"/>
                  </a></li>
                </xsl:when>
                <xsl:otherwise>
                  <li><a href="{$fid}-tpc-0001{$html_ext}">
                    <xsl:call-template name="corpus_title"/>
                  </a></li>
                </xsl:otherwise>
              </xsl:choose>
            ...
            <ol>
              ...
              <xsl:choose>
                <xsl:when test=".//topic[@type='title' and count(preceding::topic)=0]
                                ...
                                and count(.//topic)&gt;2">
                  <li><a href="{$fid}-tpc-0003{$html_ext}" epub:type="bodymatter"><xsl:value-of select="$i18n_text"/></a></li>
                </xsl:when>
                <xsl:when test=".//topic[@type='title' and count(preceding::topic)=0]
                                and count(.//topic)&gt;2">
                  <li><a href="{$fid}-tpc-0002{$html_ext}" epub:type="bodymatter"><xsl:value-of select="$i18n_text"/></a></li>
                </xsl:when>
                <xsl:when test="../quiz or name(.//*[name()='topic' or name()='quiz'])='quiz'">
                  <li><a href="{$fid}-quz-0001{$html_ext}" epub:type="bodymatter"><xsl:value-of select="$i18n_text"/></a></li>
                </xsl:when>
                <xsl:when test=".//topic[1]/processing-instruction('tune-html-replace-by')">
                  <li><a href="{substring-after(
                               normalize-space(.//topic[1]/processing-instruction('tune-html-replace-by')),
                ...
                <xsl:otherwise>
                  <li><a href="{$fid}-tpc-0001{$html_ext}" epub:type="bodymatter"><xsl:value-of select="$i18n_text"/></a></li>
                </xsl:otherwise>
              </xsl:choose>
              <xsl:if test="$index and .//index">
