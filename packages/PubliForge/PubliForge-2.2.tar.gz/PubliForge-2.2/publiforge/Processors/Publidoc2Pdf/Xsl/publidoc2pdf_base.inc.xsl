...
<xsl:apply-templates select="division|topic" mode="corpus"/>
  ...
  <xsl:template match="topic" mode="corpus">
    <xsl:apply-templates select="processing-instruction()" mode="tune"/>
    <xsl:choose>
...
<xsl:if test="header">
{\fontsize{16}{20pt}\selectfont <xsl:apply-templates select="header"/>}
</xsl:if>
~\par\vspace{3cm}
...
{\fontsize{18}{22pt}\selectfont <xsl:for-each select="head/subtitle"><xsl:apply-templates/>\par\vspace{1cm} </xsl:for-each>}
</xsl:if>
\vfill
...
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
<xsl:apply-templates select="processing-instruction()" mode="tune"/>
<xsl:if test="preceding-sibling::section and not(@type='sign')
              and not(ancestor::front)"
        >\pagebreak[3]</xsl:if>
<xsl:choose>
  <!-- box -->
...
\par\medskip
  ...
  <xsl:template name="section_children">
    <xsl:apply-templates
        select="section|p|speech|list|blockquote|table|media|processing-instruction()"/>
  </xsl:template>
...
<xsl:apply-templates select="processing-instruction()" mode="tune"/>
<xsl:if test="../section">
\chapter{<xsl:value-of select="$i18n_bibliography"/>}
</xsl:if>
...
\noindent{\small <xsl:apply-templates/>}\par
  ...
  <xsl:template match="image">
    <xsl:if test="$img">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:text>\resizebox{!}{1.5em}{\includegraphics{\detokenize{</xsl:text>
  ...
  <xsl:template match="image" mode="media">
    <xsl:if test="$img">
      <xsl:choose>
        <xsl:when test="processing-instruction('tune-latex-img-size')">
          <xsl:text>\noindent\resizebox{</xsl:text>
          <xsl:value-of select="normalize-space(processing-instruction('tune-latex-img-size'))"/>
          <xsl:text>}{!}{\includegraphics{\detokenize{</xsl:text>
          <xsl:value-of select="concat($img_dir, translate(@id, '_', '-'), '.pdf')"/>
          <xsl:text>}}}\par
          </xsl:text>
        </xsl:when>
        <xsl:when test="@type='thumbnail'">
          <xsl:text>\noindent\resizebox{.35\linewidth}{!}{\includegraphics{\detokenize{</xsl:text>
          <xsl:value-of select="concat($img_dir, translate(@id, '_', '-'), '.pdf')"/>
          <xsl:text>}}}\par
          </xsl:text>
        </xsl:when>
        <xsl:when test="@type='icon'">
          <xsl:text>\resizebox{!}{2em}{\includegraphics{\detokenize{</xsl:text>
          <xsl:value-of select="concat($img_dir, translate(@id, '_', '-'), '.pdf')"/>
          <xsl:text>}}}\par
          </xsl:text>
        </xsl:when>
        <xsl:otherwise>
          <xsl:text>\noindent\resizebox{\linewidth}{!}{\includegraphics{\detokenize{</xsl:text>
          <xsl:value-of select="concat($img_dir, translate(@id, '_', '-'), '.pdf')"/>
          <xsl:text>}}}\par
          </xsl:text>
        </xsl:otherwise>
...
\end{center}
  ...
  <xsl:template match="note">
    <xsl:choose>
      <xsl:when test="w">
        <xsl:apply-templates select="w"/>
        <xsl:if test="ancestor::title">\protect</xsl:if>
  ...
  <xsl:template match="name|foreign|acronym|term|literal|highlight|emphasis|mentioned"
                mode="link">
    <xsl:apply-templates select="."/>
  </xsl:template>
