...
def main():
    ...
    parser.add_argument(
        ...
        choices=('png',))
    parser.add_argument(
        '--density', dest='density', help='density for image conversion',
        type=int, default=300)
    parser.add_argument(
        '--factor', dest='factor', help='magnification factor',
        type=float, default=.35)
    parser.add_argument(
        '--baseline', dest='baseline', help='base line position adjustment',
        type=int, default=0)
    parser.add_argument(
        '--index', dest='index', help='index file (e.g. index.xhtml)')
    ...
    if not isdir(args.tex_dir):
        ...
        exit(2)
    setup_logging(
        args.log_level, args.log_file, '%(message)s (%(levelname)s)')
...
class TexMath(object):
    ...
    def start(self, tex_dir):
        ...
        if self._maths:
            ...
            LOG.info(self._args.index)
            for path, ignored_, files in walk(tex_dir):
                files = fnmatch.filter(files, u'{0}*.*html'.format(self.fid))
                if self._args.index is not None and \
                   exists(join(path, self._args.index)):
                    files.append(self._args.index)
    ...
    def _tex2png(self, tex_file):
        ...
        self._maths[math_id] = (
            round((int(match.group(1)) + int(match.group(2))) *
                  self._args.factor),
            -round((int(match.group(1)) + self._args.baseline) *
                   self._args.factor))
