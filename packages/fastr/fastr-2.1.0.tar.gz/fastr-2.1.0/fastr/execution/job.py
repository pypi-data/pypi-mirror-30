...
class Job(Serializable):
    ...
    def clean(self):
        fastr.log.info('Cleaning {}'.format(self.id))
        cleanup_level = JobCleanupLevel(fastr.config.job_cleanup_level)
        ...
        if cleanup_level == JobCleanupLevel.all or self.status not in [JobState.failed, JobState.execution_failed]:
            job_tmp_dir = fastr.vfs.url_to_path(self.tmpurl)
    ...
    def execute(self):
        ...
        for error in result.errors:
            if isinstance(error, exceptions.FastrError):
                ...
                fastr.log.warning('Collected a mal-formatted error: {}'.format(error))
    ...
    def get_output_datatype(self, output_id):
        ...
        requested_type = fastr.typelist[self.output_arguments[output_id]['datatype']]
    ...
    @staticmethod
    def cast_to_type(value, datatypes):
        """
        ...
        :return: casted value
        """
        fastr.log.info('Trying to cast {} to one of {}'.format(value, datatypes))
        for datatype in datatypes:
            try:
                item = datatype(str(value))
                fastr.log.info('Casted to {!r}'.format(item))
                return item
            except exceptions.FastrValueError:
                pass
        else:
