...
LOG_LEVEL = "DEBUG"
PROVIDER = "JRSS"
SELF_DEFINE_HEADER_PREFIX = "x-jrss-"
if "AWS" == PROVIDER:
...
def check_bucket_valid(bucket):
    ...
    if len(bucket) < 3 or len(bucket) > 255:
        return False
...
def get_assign(secret_access_key, method, headers=None, resource="/", result=None, debug=DEBUG):
    ...
    if not result:
        result = []
    canonicalized_jrss_headers = ""
    secret_access_key = convert_utf8(secret_access_key)
    global OSS_LOGGER_SET
    if not OSS_LOGGER_SET:
        OSS_LOGGER_SET = Logger(debug, "log.txt", LOG_LEVEL, "jrss_util").getlogger()
    ...
    if len(tmp_headers) > 0:
        ...
        for k in x_header_list:
            if k.startswith(SELF_DEFINE_HEADER_PREFIX):
                canonicalized_jrss_headers += "%s:%s\n" % (k, tmp_headers[k])

    string_to_sign = method + "\n" + content_md5.strip() + "\n" + content_type + "\n" + date + "\n" + canonicalized_jrss_headers + canonicalized_resource
    result.append(string_to_sign)
    OSS_LOGGER_SET.debug("method:%s\n content_md5:%s\n content_type:%s\n data:%s\n "
                         "canonicalized_jrss_headers:%s\n canonicalized_resource:%s\n" % (
                             method, content_md5, content_type, date, canonicalized_jrss_headers,
                             canonicalized_resource))
    OSS_LOGGER_SET.debug("string_to_sign:%s\n \nlength of string_to_sign:%d\n" % (string_to_sign, len(string_to_sign)))

    # Tuned by u.
    h = hmac.new(secret_access_key, string_to_sign.encode('utf-8'), sha)
...
def get_resource(params=None):
    ...
    override_response_list = ['response-content-type', 'response-content-language',
                              ...
                              'acl', 'uploadId', 'uploads', 'partNumber',
...
def jrss_quote(in_str):
    if not isinstance(in_str, str):
        # Tuned by u.
        # in_str = str(in_str)
        print(in_str)
        in_str = str(in_str.decode())
...
def append_param(url, params):
    ...
    for k, v in list(params.items()):
        ...
        if v is not None and v != '':
            l.append('%s=%s' % (jrss_quote(k), jrss_quote(v)))
        elif k == 'acl':
            l.append('%s' % (jrss_quote(k)))
        elif v is None or v == '':
            l.append('%s' % (jrss_quote(k)))
    if len(l):
        url = url + '?' + '&'.join(l)
...
def create_part_xml(part_msg_list=None):
    ...
    for part in part_msg_list:
        if len(part) >= 3:
            ...
            xml_string += r'<ETag>"' + str(part[2]) + r'"</ETag>'
            xml_string += r'</Part>'
        else:
...
############### operate OSS ###############
def clear_all_object_of_bucket(jrss_instance, bucket):
    """
    clean all objects in bucket, after that, it will delete this bucket.
    """
    return clear_all_objects_in_bucket(jrss_instance, bucket)


def clear_all_objects_in_bucket(jrss_instance, bucket, delete_marker="", delete_upload_id_marker="", debug=False):
    """
    it will clean all objects in bucket, after that, it will delete this bucket.

    example:
    from jrss_api import *
    host = ""
    id = ""
    key = ""
    jrss_instance = OssAPI(host, id, key)
    bucket = "leopublicreadprivatewrite"
    if clear_all_objects_in_bucket(jrss_instance, bucket):
        pass
    else:
        print "clean Fail"
    """
    ...
    delimiter = ""
    maxkeys = 1000
    delete_all_objects(jrss_instance, bucket, prefix, delimiter, delete_marker, maxkeys, debug)
    delete_all_parts(jrss_instance, bucket, delete_marker, delete_upload_id_marker, debug)
    res = jrss_instance.delete_bucket(bucket)
    if (res.status / 100 != 2 and res.status != 404):
        print("clear_all_objects_in_bucket: delete bucket:%s fail, ret:%s, request id:%s" % (
            bucket, res.status, res.getheader("x-jrss-request-id")))
        return False
    return True


def delete_all_objects(jrss_instance, bucket, prefix="", delimiter="", delete_marker="", maxkeys=1000, debug=False):
    marker = delete_marker
    delete_obj_num = 0
    while 1:
        object_list = []
        res = jrss_instance.get_bucket(bucket, prefix, marker, delimiter, maxkeys)
        if res.status != 200:
            return False
        body = res.read()
        ...
        if object_list:
            ...
            res = jrss_instance.batch_delete_object(bucket, object_list_xml)
            if res.status / 100 != 2:
                if marker:
                    print(
                        "delete_all_objects: batch delete objects in bucket:%s fail, ret:%s, request id:%s, first object:%s, marker:%s" % (
                            bucket, res.status, res.getheader("x-jrss-request-id"), object_list[0], marker))
                else:
                    print(
                        "delete_all_objects: batch delete objects in bucket:%s fail, ret:%s, request id:%s, first object:%s" % (
                            bucket, res.status, res.getheader("x-jrss-request-id"), object_list[0]))
                return False
            else:
                if debug:
...
def delete_all_parts(jrss_instance, bucket, delete_object_marker="", delete_upload_id_marker="", debug=False):
    delete_mulitipart_num = 0
    marker = delete_object_marker
    id_marker = delete_upload_id_marker
    while 1:
        res = jrss_instance.get_all_multipart_uploads(bucket, key_marker=marker, upload_id_marker=id_marker)
        if res.status != 200:
            break
        body = res.read()
        ...
        for i in fl:
            ...
            res = jrss_instance.cancel_upload(bucket, object, i[1])
            if (res.status / 100 != 2 and res.status != 404):
                print("delete_all_parts: cancel upload object:%s, upload_id:%s FAIL, ret:%s, request-id:%s" % (
                    object, i[1], res.status, res.getheader("x-jrss-request-id")))
            else:
...
def clean_all_bucket(jrss_instance):
    """
    it will clean all bucket, including the all objects in bucket.
    """
    res = jrss_instance.get_service()
    if (res.status / 100) == 2:
        h = GetServiceXml(res.read())
        for b in h.bucket_list:
            if not clear_all_objects_in_bucket(jrss_instance, b.name):
...
def pgfs_clear_all_objects_in_bucket(jrss_instance, bucket):
    """
    it will clean all objects in bucket, after that, it will delete this bucket.
    """
    b = GetAllObjects()
    b.get_all_object_in_bucket(jrss_instance, bucket)
    for i in b.object_list:
        res = jrss_instance.delete_object(bucket, i)
        if res.status / 100 != 2:
            print("clear_all_objects_in_bucket: delete object fail, ret is:", res.status, "bucket is:", bucket,
                  "object is: ", i)
            return False
        else:
            pass
    ...
    if res.status / 100 != 2 and res.status != 404:
        print("clear_all_objects_in_bucket: delete bucket fail, ret is: %s, request id is:%s" % (
            res.status, res.getheader("x-jrss-request-id")))
        return False
    return True


def pgfs_clean_all_bucket(jrss_instance):
    """
    it will clean all bucket, including the all objects in bucket.
    """
    res = jrss_instance.get_service()
    if (res.status / 100) == 2:
        h = GetServiceXml(res.read())
        for b in h.bucket_list:
            if not pgfs_clear_all_objects_in_bucket(jrss_instance, b.name):
                print("clean bucket ", b.name, " failed! in clean_all_bucket")
                return False
        return True
    else:
...
class GetAllObjects:
    ...
    def get_object_in_bucket(self, jrss, bucket="", marker="", prefix=""):
        object_list = []
        maxkeys = 1000
        try:
            # Tuned by u.
            res = jrss.get_bucket(bucket, prefix, marker, maxkeys=str(maxkeys))
            body = res.read()
    ...
    def get_object_dir_in_bucket(self, jrss, bucket="", marker="", prefix="", delimiter=""):
        object_list = []
        dir_list = []
        maxkeys = 1000
        try:
            # Tuned by u.
            res = jrss.get_bucket(bucket, prefix, marker, delimiter, maxkeys=str(maxkeys))
            body = res.read()
            hh = GetBucketXml(body)
            (fl, pl) = hh.list()
            if len(fl) != 0:
    ...
    def get_all_object_in_bucket(self, jrss, bucket="", marker="", prefix=""):
        marker2 = ""
        while True:
            (object_list, marker) = self.get_object_in_bucket(jrss, bucket, marker2, prefix)
            marker2 = marker
    ...
    def get_all_object_dir_in_bucket(self, jrss, bucket="", marker="", prefix="", delimiter=""):
        marker2 = ""
        while True:
            (object_list, dir_list, marker) = self.get_object_dir_in_bucket(jrss, bucket, marker2, prefix, delimiter)
            marker2 = marker
            if len(object_list) != 0:
                self.object_list.extend(object_list)
            if len(dir_list) != 0:
...
def get_upload_id(jrss, bucket, object, headers=None):
    """
    ...
    if not headers:
        headers = {}
    upload_id = ""
    res = jrss.init_multi_upload(bucket, object, headers)
    if res.status == 200:
        body = res.read()
...
def get_all_upload_id_list(jrss, bucket):
    """
    ...
    id_marker = ""
    while True:
        res = jrss.get_all_multipart_uploads(bucket, key_marker=marker, upload_id_marker=id_marker)
        if res.status != 200:
...
def get_upload_id_list(jrss, bucket, object):
    """
    ...
    id_marker = ""
    while True:
        res = jrss.get_all_multipart_uploads(bucket, prefix=object, key_marker=marker, upload_id_marker=id_marker)
        if res.status != 200:
            break
        body = res.read()
        hh = GetMultipartUploadsXml(body)
...
def get_part_list(jrss, bucket, object, upload_id, max_part=""):
    """
    ...
    marker = ""
    while True:
        res = jrss.get_all_parts(bucket, object, upload_id, part_number_marker=marker, max_parts=max_part)
        if res.status != 200:
            break
        body = res.read()
        h = GetPartsXml(body)
...
def get_part_xml(jrss, bucket, object, upload_id):
    """
    get uploaded part list of object.
    Returns:
            string
    """
    part_list = []
    part_list = get_part_list(jrss, bucket, object, upload_id)
    xml_string = r'<CompleteMultipartUpload>'
...
def get_part_map(jrss, bucket, object, upload_id):
    part_list = []
    part_list = get_part_list(jrss, bucket, object, upload_id)
    part_map = {}
    for part in part_list:
...
########## multi-thread ##########
def multi_get(jrss, bucket, object, localfile, thread_num, retry_times):
    length = 0
    res = jrss.head_object(bucket, object)
    if 200 == res.status:
    ...
    for i in range(len(ranges) - 1):
        ...
        exec("current = MultiGetWorker(jrss, bucket, object, file_%s, ranges[i], ranges[i + 1] - 1, %s)" % (
            i, retry_times))
    ...
    if not os.path.isfile(localfile) or length != os.path.getsize(localfile):
        print("localfile:%s size:%s is not equal with object:%s size:%s " % (
            localfile, os.path.getsize(localfile), object, length))
        return False
    else:
        return True


def multi_get2(jrss, bucket, object, localfile, thread_num, retry_times):
    res = jrss.head_object(bucket, object)
    if 200 == res.status:
        length = int(res.getheader('content-length'))
    else:
...
class DeleteObjectWorker(Thread):
    def __init__(self, jrss, bucket, part_msg_list, retry_times=5):
        ...
        while True:
            ...
            while True:
                try:
                    if retry_times <= 0:
                        break
                    res = self.jrss.delete_objects(bucket, object_list[begin:end])
                    if res.status / 100 == 2:
                        is_fail = False
                        break
                except:
...
def multi_upload_file2(jrss, bucket, object, filename, upload_id, thread_num=10, max_part_num=10000, retry_times=5,
    ...
    for i in range(retry_times):
        tmp_uploaded_part_map = get_part_map(jrss, bucket, object, upload_id)
        if tmp_uploaded_part_map:
            for k, v in list(tmp_uploaded_part_map.items()):
            ...
            if len(part) == 5:
                ...
                if part_number in uploaded_part_map:
                    ...
                    if uploaded_part_map[part_number].replace('"', "").upper() == md5.upper():
                        continue
                if is_need_upload:
                    queue.put((upload_part, jrss, bucket, object, upload_id, filename, part))
            else:
        ...
        res = jrss.complete_upload(bucket, object, upload_id, part_msg_xml, headers, params)
        if res.status == 200:
            return res
    raise Exception("-3, after retry %s, failed, multi upload file failed! upload_id:%s" % (retry_times, upload_id))


def upload_part(jrss, bucket, object, upload_id, file_path, part, retry_times=2):
    if len(part) == 5:
        part_number = str(part[0])
        partsize = part[3]
        offset = part[4]
        for i in range(retry_times):
            try:
                res = jrss.upload_part_from_file_given_pos(bucket, object, file_path, offset, partsize, upload_id,
                                                           part_number)
                if res.status != 200:
                    time.sleep(1)
                else:
...
class UploadPartWorker2(threading.Thread):
    ...
    def run(self):
        while 1:
            try:
                (upload_part, jrss, bucket, object, upload_id, filename, part) = self.queue.get(block=False)
                upload_part(jrss, bucket, object, upload_id, filename, part, self.retry_times)
...
class UploadPartWorker(Thread):
    def __init__(self, jrss, bucket, object, upload_id, file_path, part_msg_list, uploaded_part_map, retry_times=5):
        Thread.__init__(self)
        self.jrss = jrss
        self.bucket = bucket
        self.object = object
            ...
            if len(part) == 5:
                ...
                while True:
                    try:
                        ...
                        if retry_times <= 0:
                            break
                        res = self.jrss.upload_part_from_file_given_pos(bucket, object, self.file_path, offset,
                                                                        partsize, self.upload_id, part_number)
                        if res.status != 200:
                            retry_times = retry_times - 1
                            time.sleep(1)
                        else:
...
class MultiGetWorker(Thread):
    def __init__(self, jrss, bucket, object, file, start, end, retry_times=5):
        Thread.__init__(self)
        self.jrss = jrss
        self.bucket = bucket
        self.object = object
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                res = self.jrss.object_operation("GET", self.bucket, self.object, headers)
                if res.status == 206:
                    while True:
...
def split_large_file(file_path, object_prefix="", max_part_num=1000, part_size=10 * 1024 * 1024,
    ...
    if os.path.isfile(file_path):
        ...
        # Tuned by u.
        part_num = int((file_size + part_size - 1) / part_size)
        ...
        for i in range(0, part_num):
            ...
            # Tuned by u.
            temp_file_name = os.path.basename(file_path).decode() + "_" + str(part_order)
...
def is_jrss_host(host):
    if host.find("HC-24-0-2.h.chinabank.com.cn") != -1 or host.find("s3.amazonaws.com") != -1:
        return True
    return False
...
def get_param_int(str=''):
    try:
        return int(str)
    except:
        raise Exception('error int param: %s' % str)


def get_param_bool(str=''):
    if str.lower() == 'true':
        return 'true'
    elif str.lower() == 'false':
        return 'false'
    else:
        raise Exception('error bool param: %s, must be one of true/false' % str)
