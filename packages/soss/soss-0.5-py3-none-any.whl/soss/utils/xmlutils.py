...
class GetServiceXml:
    def __init__(self, xml_string):
        ...
        self.isTruncated = get_tag_text(self.xml, 'IsTruncated')
...
class CopyObjectXml:
    ...
    def show(self):
        print("LastModified: %s\nETag: %s" % (self.last_modified, self.etag))
...
class Deleted:
    def __init__(self, xml_element):
        self.element = xml_element
        self.key = get_tag_text(self.element, "Key")
        self.version_id = get_tag_text(self.element, "VersionId")
        self.delete_marker = get_tag_text(self.element, "DeleteMarker")
        self.delete_marker_version_id = get_tag_text(self.element, "DeleteMarkerVersionId")

    def show(self):
        print("Key: %s" % self.key)
        if self.version_id != '':
            print("\nVersionId: %s" % self.version_id)
        if self.delete_marker != '':
            print("\nDeleteMarker: %s\nDeleteMarkerVersionId: %s" % (self.delete_marker, self.delete_marker_version_id))
...
class DeleteError:
    def __init__(self, xml_element):
        ...
        self.key = get_tag_text(self.element, "Key")
        self.version_id = get_tag_text(self.element, "VersionId")
        self.code = get_tag_text(self.element, "Code")
        self.message = get_tag_text(self.element, "Message")

    def show(self):
        print("Key: %s\nVersionId: %s\nCode: %s\nMessage: %s" % (self.key, self.version_id, self.code, self.message))


class DeleteObjectsXml:
    def __init__(self, xml_string):
        self.xml = minidom.parseString(xml_string)
        self.deleted_list = []
        deleteds = self.xml.getElementsByTagName('Deleted')
        for d in deleteds:
            self.deleted_list.append(Deleted(d))

        self.error_list = []
        errors = self.xml.getElementsByTagName('Error')
        for e in errors:
            self.error_list.append(DeleteError(e))

    def show(self):
        if len(self.deleted_list) > 0:
            print("Deleted List:")
            for d in self.deleted_list:
                d.show()
...
class GetBucketXml:
    def __init__(self, xml_string):
        self.xml = minidom.parseString(xml_string)
        self.tenant = get_tag_text(self.xml, 'Tenant')
        ...
        self.encoding_type = get_tag_text(self.xml, 'EncodingType')
...
class Grantee:
    def __init__(self, xml_element):
        ...
        self.id = get_tag_text(self.element, "ID")
        self.display_name = get_tag_text(self.element, "DisplayName")
        self.email_address = get_tag_text(self.element, "EmailAddress")
        self.uri = get_tag_text(self.element, "URI")

    def show(self):
        if self.id != '' or self.display_name != '':
            print("ID: %s\nDisplayName: %s" % (self.id, self.display_name))
        if self.email_address != '':
            print("EmailAddress: %s" % self.email_address)
        if self.uri != '':
...
class Grant:
    def __init__(self, xml_element):
        self.element = xml_element
        self.grantee = Grantee(self.element.getElementsByTagName('Grantee')[0])
        self.permission = get_tag_text(self.element, "Permission")

    def show(self):
        print("Grantee: \n", self.grantee.show())
        print("Permission: %s" % self.permission)
...
class GetBucketAclXml:
    def __init__(self, xml_string):
        ...
        if len(self.xml.getElementsByTagName('Owner')) != 0:
            ...
            self.owner = ""
        self.grants = self.xml.getElementsByTagName('Grant')
        self.grant_list = []
        for g in self.grants:
            self.grant_list.append(Grant(g))

    def show(self):
        print("Owner Name: %s\nOwner ID: %s" % (self.owner.id, self.owner.display_name))
        print("Grant: ")
...
class GetInitUploadIdXml:
    ...
    def show(self):
        if self.tenant != '':
            print("Tenant: %s\n" % self.tenant)
        print("Bucket: %s\nKey: %s\nUploadId: %s" % (self.bucket, self.key, self.upload_id))
...
class Upload:
    def __init__(self, xml_element):
        ...
        self.storage_class = get_tag_text(self.element, 'StorageClass')
        self.owner = Owner(self.element.getElementsByTagName('Owner')[0])
        self.initiator = Owner(self.element.getElementsByTagName('Initiator')[0])
...
class GetMultipartUploadsXml:
    def __init__(self, xml_string):
        ...
        self.tenant = get_tag_text(self.xml, 'Tenant')
        self.bucket = get_tag_text(self.xml, 'Bucket')
        self.prefix = get_tag_text(self.xml, 'ListMultipartUploadsResult.Prefix')
        ...
        prefixes = self.xml.getElementsByTagName('CommonPrefixes')
        for p in prefixes:
            tag_txt = get_tag_text(p, "CommonPrefixes.Prefix")
            self.prefix_list.append(tag_txt)
