...
import time
import hypertemp
import hypertemp.http11.connection
import pytest
...
from hypertemp.common.exceptions import ProxyError
from hypertemp.common.util import HTTPVersion, to_bytestring
...
from hypertemp.http20.exceptions import ConnectionError, StreamResetError
from server import SocketLevelTest, SocketSecuritySetting

# Turn off certificate verification for the tests.
if ssl is not None:
    hypertemp.tls._context = hypertemp.tls.init_context()
    hypertemp.tls._context.check_hostname = False
    hypertemp.tls._context.verify_mode = ssl.CERT_NONE

# Cover our bases because NPN doesn't yet work on all our test platforms.
PROTOCOLS = hypertemp.http20.connection.H2_NPN_PROTOCOLS + ['', None]
...
@patch('hypertemp.http20.connection.H2_NPN_PROTOCOLS', PROTOCOLS)
class TestHyperIntegration(SocketLevelTest):
    ...
    def test_upgrade(self):
        ...
        self._start_server(socket_handler)
        conn = hypertemp.HTTPConnection(self.host, self.port, self.secure)
        conn.request('GET', '/')
        resp = conn.get_response()
    ...
    def test_version_after_tls_upgrade(self, monkeypatch):
        ...
        # forcefully upgrade.
        old_wrap_socket = hypertemp.http11.connection.wrap_socket

        def wrap(*args):
            sock, _ = old_wrap_socket(*args)
            return sock, 'h2'

        monkeypatch.setattr(hypertemp.http11.connection, 'wrap_socket', wrap)
        ...
        self._start_server(socket_handler)
        c = hypertemp.HTTPConnection(self.host, self.port, secure=True)
    ...
    def test_version_after_http_upgrade(self):
        ...
        c = hypertemp.HTTPConnection(self.host, self.port)
        assert c.version is HTTPVersion.http11
