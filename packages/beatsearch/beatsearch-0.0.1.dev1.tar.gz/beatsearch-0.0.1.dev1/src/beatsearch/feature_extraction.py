...
# class Complexity(MonophonicRhythmFeatureExtractor):
#     def __init__(self, unit: UnitType = Unit.EIGHTH, ret_fraction: bool = True):
#         super().__init__(unit)
#         self._ret_fraction = None
#         self.ret_fraction = ret_fraction  # calling setter
#
...
#
#         :param rhythm:
#         :param pre_processor_results:
#         :return:
#         """
#
#         syncopation_vector = pre_processor_results[0]
#         total_syncopation_strength = sum(x[1] for x in syncopation_vector)
#
#         binary_onset_vector = pre_processor_results[1]
#         metrical_weights = rhythm.time_signature.get_metrical_weights(self.unit)
#         syncopation_strengths = SyncopationVector.get_syncopation_strength_map(metrical_weights)
#
#         # sums up the syncopation strength for all the onsets if they
#         max_syncopation_strength = sum(sync_strength for is_onset, sync_strength in zip(
#             binary_onset_vector, syncopation_strengths) if is_onset)
#
#         # NOTE: We use binary onset vector and don't call rhythm.get_onset_count because the onset count might be
#         # different for different units (onsets might merge with large unit like Unit.QUARTER)
#         n_onsets = sum(binary_onset_vector)
