...
class FancyChart(object):
    ...
    def get_data(self, symbol):
        ...
        dtfm = dtto - timedelta(days=365)
        data_gen = self.market_data.get_price_data(symbol, "D1", to_ole(dtfm), to_ole((dtto)))
        data = next(data_gen)[::-1]
        x = pd.DataFrame(data, index=data['date']).drop('date', axis=1)
...
FancyChart(market_data).graph('GBP/USD')
