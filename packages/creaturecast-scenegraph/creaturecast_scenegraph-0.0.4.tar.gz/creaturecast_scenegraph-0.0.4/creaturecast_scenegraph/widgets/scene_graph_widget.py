...
import py_maya_core.models.database as scn
import py_maya_core.environment as env
...
class SceneGraphWidget(QFrame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        #self.horizontal_splitter.addWidget(self.node_details_widget)
        ...
        #self.horizontal_layout.addWidget(self.build_checkbox)
        self.horizontal_layout.addStretch()
        #self.horizontal_layout.addWidget(self.details_button)
        ...
        #self.console.input_console.interpreter.locals = dict(
        #    selected_nodes=[],
        #    self=self
        #)
        #self.build_checkbox.toggled.connect(self.toggle_m_objects)
        #self.details_button.toggled.connect(self.node_details_widget.setVisible)
        #self.details_button.setChecked(False)

        #self.view.items_selected.connect(self.node_details_widget.set_item)
        #self.view.items_selected.connect(self.set_details_visibility)
    ...
    def commit_to_database(self):
        node_iterator = scn.create_database_nodes(rot.scene_root)
        plug_iterator = scn.create_database_plugs(rot.scene_root)
        node_connection_iterator = scn.create_database_node_connections(rot.scene_root)
        plug_connection_iterator = scn.create_database_plug_connections(rot.scene_root)
        descendants = rot.scene_root.get_descendants()
        node_count = len(descendants) + 1
        plug_count = 0
        for d in descendants:
            plug_count += len(d.plug_dictionary.keys())

        for x in progress(node_count, "Saving Nodes", "Stop!", parent=self):
            database_node = next(node_iterator)
            ses.session.add(database_node)

        for x in progress(node_count, "Saving Node Connections", "Stop!", parent=self):
            next(node_connection_iterator)

        for x in progress(plug_count, "Saving Plugs", "Stop!", parent=self):
            database_plug = next(plug_iterator)
            ses.session.add(database_plug)

        for x in progress(plug_count, "Saving Plug Connections", "Stop!", parent=self):
            next(plug_connection_iterator)
    ...
    def import_nodes(self):
        path, ok = QFileDialog.getOpenFileName(
            ...
            directory=env.home_directory,
            filter='Json (*.json);; NodeDatabase (*.nodes)'
        )
        if ok:
            with open(path, mode='r') as f:
                node_data = json.loads(f.read())
                create_nodes_iterator = scn.create_nodes(*node_data)
                for x in progress(scn.get_node_data_length(*node_data), "Show Progress", "Stop the madness!", parent=self):
                    next(create_nodes_iterator)
    ...
    def export_nodes(self):
        ...
        if ok:
            with open(path, mode='w') as f:
                f.write(json.dumps(
                    [x.serialize() for x in scn.scene_root.children],
                    sort_keys=True,
                    indent=4,
                    separators=(',', ': ')
                ))
...
class CreateRigWorker(QThread):
    ...
    def run(self):
        ...
        self.success.emit(response_string)
