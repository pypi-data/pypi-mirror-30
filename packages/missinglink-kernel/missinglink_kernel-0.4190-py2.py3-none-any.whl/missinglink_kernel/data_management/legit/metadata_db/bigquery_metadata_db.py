...
class BigQueryMetadataDB(BaseMetadataDB, BigQueryMixin):
    ...
    def add_data_using_url(self, metadata_url, dry_mode=False):
        ...
        do_defer(self._delete_temp_table, temp_table_ref.project, temp_table_ref, _countdown=self.TEMP_TABLE_TTL)
        ...
        def query_all_meta_data_without_staging():
            query = """
                ...
                FROM (
                    SELECT {query_phase},*                    
                    FROM (
                        SELECT *, ROW_NUMBER() OVER (PARTITION BY _sha ORDER BY _ts DESC) row_number, ((FARM_FINGERPRINT({phase_seed}) + POW(2, 63)) / POW(2, 64)) as _phr
                        FROM (
                        ...
                        RIGHT JOIN (
                            ...
                            SELECT * EXCEPT(row_number)
                            FROM (
                                SELECT size, commit_sha, name as _sha, sha as _hash, url as _url, 
