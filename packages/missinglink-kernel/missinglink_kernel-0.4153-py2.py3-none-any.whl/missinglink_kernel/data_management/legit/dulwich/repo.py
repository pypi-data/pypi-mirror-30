...
class Repo(BaseRepo):
    ...
    def _upload_staged_files(self, files_to_add, callback):
        self.object_store.add_objects(files_to_add, callback)
    ...
    @classmethod
    def _gen_upload_request(cls, files_info):
        for file_info in files_info:
            yield Repo.UploadFileRequest(file_info)

    def stage(self, files_info, callback=None):
        index = self.open_index()

        upload_requests = list(self._gen_upload_request(files_info))
        self._upload_staged_files(upload_requests, callback=callback)
