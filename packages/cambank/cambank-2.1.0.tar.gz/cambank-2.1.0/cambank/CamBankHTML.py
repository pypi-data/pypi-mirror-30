...
"""
 ...
 def show_footers(self):
  return """
<h6 id="footer">
CamBank &copy; 2008-2012 Camille Huot.
</h6>
"""
...
<tr><th></th><td><input type="image" src="../images/valid.png" style="border: 0"/></td></tr>
 ...
 def show_pending_operations(self):
  ...
  # mise en cache de la liste des budgets
  buds = cb.get_budgets()
  def print_budgets_menu(default = 0):
   ...
   # if ep.eshow is True:
   #  budgets_item += "<option value=\"e%d\">Epargne - %s</option>\n" % \
   #   (ep.eid, ep.ename)
   #budgets_item += "<option value=\"auto\">Epargne - Automatique</option>\n"
   budgets_item += "</select>\n"
   return budgets_item
  ...
  for optype in sorted(set([ o.get_type() for o in ops ])):
   ...
   for op in myops:
     salairehtml = ''
     if op.omontant-op.obalance > salaire:
     ...
     html += print_budgets_menu(op.hint_bid)
     html += """
     </tr>
