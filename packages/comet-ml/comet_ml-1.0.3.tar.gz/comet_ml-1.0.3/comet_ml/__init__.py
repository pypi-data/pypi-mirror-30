...
from .connection import Reporting, RestServerConnection, server_address, visualization_upload_url, OptimizerConnection
from .console import StdLogger
from .exceptions import NotParametrizedException, OptimizationMultipleParams, PCSParsingError, ValidationError, NoMoreSuggestionsAvailable
...
EXISTING_EXPERIMENT_CONTEXT_WARNING = (
    "Existing Experiment require the use of contexts, please use"
    " train/validate/test methods")
...
class Experiment(object):
    ...
    def _mark_as_started(self):
        RestServerConnection.update_experiment_status(self.api_key, self.run_id, self.id, self.project_id, self.optimization_id, self.alive)

    def _mark_as_ended(self):
        try:
            RestServerConnection.update_experiment_status(self.api_key, self.run_id, self.id, self.project_id,
                                                      self.optimization_id, False)
        except Exception as e:
            tb = traceback.format_exc()
            print('%s \n comet.ml error: failed to report experiment status' % tb)
    ...
    def _get_source_code():
        ...
        print("Failed to find source code module", file=sys.stderr)
    ...
    def _get_optimization_id_for_api_key(self, api_key):
        if config.optimizer is not None:
            if config.optimizer.api_key == api_key:
                ...
                print(EXPERIMENT_OPTIMIZER_API_KEY_MISMTACH_WARNING)
        return None
...
class ExistingExperiment(Experiment):
    ...
    def _authenticate(self):
        ...
        # Get an id for this run
        try:
            self.run_id, full_ws_url, self.project_id, self.is_github, self.focus_link = RestServerConnection.get_old_run_id(
                self.api_key, self.previous_experiment)
    ...
    def _create_message(self):
        ...
        if self.context is None:
            print(EXISTING_EXPERIMENT_CONTEXT_WARNING)
...
class Optimizer(object):
    ...
    def _authenticate(self):
        ...
        data = {
            "optimizationId": self.id, "apiKey": self.api_key, "libVersion": version_num
        }

        self.connection.authenticate(data)
