...
class Streamer(threading.Thread):
    ...
    def wait_for_finish(self):
        ...
        if not self.messages.empty():
            ...
            print("\nComet failed to send all the data back %d" % remaining)
    ...
    def _wait_for_empty(self, timeout, verbose=False):
        ...
        while not self.messages.empty() and time.time() < end_time:
            if verbose is True:
                print("%d messages remaining to upload" % self.messages.qsize())
            time.sleep(5)
    ...
    def _report_upload_not_finished(self):
        ...
        err_msg = "Comet failed to send all the data back %d" % remaining

        # Display the message
        print("\n%s" % err_msg)

        self.connection.report(
            event_name="on_exit_didnt_finish_upload_sdk", err_msg=err_msg
        )
