...
from .connection import Reporting, RestServerConnection, server_address, optimization_address, visualization_upload_url
from .console import StdLogger
from .exceptions import NotParametrizedException, OptimizationMultipleParams, PCSParsingError, ValidationError
...
class Optimizer(object):
    ...
    def _authenticate(self):
        ...
        response = requests.post(
            optimization_address + "authenticate",
            json=data,
        ...
        try:
            while True:
                response = requests.get(
                    optimization_address + "suggestion/",
                    headers=self.headers,
                )
                response.raise_for_status()
                ...
                if suggestion:
                    ...
                    if data.get("terminated"):
                        raise Exception("No more suggestions available!")
