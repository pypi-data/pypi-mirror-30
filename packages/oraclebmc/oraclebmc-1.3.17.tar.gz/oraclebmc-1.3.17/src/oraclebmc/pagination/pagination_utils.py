...
def list_call_get_up_to_limit(list_func_ref, record_limit, page_size, *list_func_args, **list_func_kwargs):
    ...
    the version of this method which produces a generator: :py:func:`~oraclebmc.pagination.list_call_get_up_to_limit_generator`
    ...
    :return:
        A :class:`~oraclebmc.response.Response` whose data attribute contains all the records we retrieved. The other attributes of the
        :class:`~oraclebmc.response.Response` object will be sourced from the last response we received from calling the list operation on the service.
...
def list_call_get_up_to_limit_generator(list_func_ref, record_limit, page_size, yield_mode, *list_func_args, **list_func_kwargs):
    ...
    :py:func:`~oraclebmc.pagination.list_call_get_up_to_limit` function, which makes all the required service calls when it is called.
    ...
    :param str yield_mode:
        ...
        either :class:`~oraclebmc.response.Response` objects (if the value is ``response``), or whether it yields the
        individual model objects which are contained within the response's ``data`` attribute (which should
        be a list of model objects)

    :return:
        A generator that, depending on the ``yield_mode``, will yield either :class:`~oraclebmc.response.Response` objects
...
def list_call_get_all_results(list_func_ref, *list_func_args, **list_func_kwargs):
    ...
    :py:func:`~oraclebmc.pagination.list_call_get_all_results_generator`
...
def list_call_get_all_results_generator(list_func_ref, yield_mode, *list_func_args, **list_func_kwargs):
    ...
    This function produces a generator and lazily loads results. That is, service calls will only be made to fetch more data
    when needed as we iterate through results produced by the generator; this contrasts with the eagarily loaded
    :py:func:`~oraclebmc.pagination.list_call_get_all_results` function, which makes all the required service calls when it is called.
