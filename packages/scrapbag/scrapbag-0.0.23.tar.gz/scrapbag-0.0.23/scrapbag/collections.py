...
def simplify_list(element):
    ...
    if isinstance(element, (collections.Iterator, list, tuple)):
        element = exclude_empty_values(element)
        ...
        if len(element) == 1:
            ...
            for i in range(0, len(element)):
                element[i] = simplify_list(exclude_empty_values(element[i]))

    return exclude_empty_values(element)
