...
def run():
    ...
    try:
        manager.update_broker_ec2_state(args.remote_container_id, u'DRAINING')
        logger.info("it's success to drain container %s out of cluster %s.", args.remote_container_id, args.cluster_name)
    except Exception as e:
    ...
    try:
        ...
        logger.info("it's success to stop broker running in container %s.", args.remote_container_id)
    except Exception as e:
        logger.error("error happened during stop broker task, the error is %s.", e)
        sys.exit(1)
        
    # schedule task in new container
    try:
        manager.schedule_broker_task(args.local_container_id)
        logger.info("it's success to schedule a new task running in container %s.", args.local_container_id)
    except Exception as e:
        logger.error("error happened during schedule a new task, the error is %s.", e)
        manager.rollback_broker_task(args.remote_container_id)
        sys.exit(1)
        
    logger.info("Congratulations, broker migration is done, please sincerely check the Kafka cluster's status.")
