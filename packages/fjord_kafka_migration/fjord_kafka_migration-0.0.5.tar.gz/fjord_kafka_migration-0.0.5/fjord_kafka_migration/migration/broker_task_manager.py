...
class BrokerTaskManager(object):
    ...
    def stop_broker_task(self, container_id):
        ...
        try:
            ...
            while self.get_broker_task(container_id, u'STOPPED') is None:
                print('wait for 1 seconds to check task %s stopping status again.' % task_arn)
                time.sleep(1)
            
            print('the task %s which was running inside of container %s, has been successfully stopped.' % (task_arn, container_id))
    ...
    def drain_broker_ec2(self, container_id):
        ...
        try:
            ...
            print('success to get response returned after draining %s is %s.' % (container_id, response))
    ...
    def get_broker_task(self, container_id, status=u'RUNNING'):
        try:
            ...
            print("error happened during get broker task for %s is %s." % (container_id, e))
            raise e
