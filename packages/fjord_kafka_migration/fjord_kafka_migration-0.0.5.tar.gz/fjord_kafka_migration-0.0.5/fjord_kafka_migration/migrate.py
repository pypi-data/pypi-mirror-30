...
def run():
    ...
    # drain remote broker ec2, so that we can ensure no task running the replaced ec2 any more. 
    manager.drain_broker_ec2(remote_container_id)
    print('success to drain broker running in container %s.' % remote_container_id)
    ...
    print("Configuration!!!, broker migration is done, please sincerely check the Kafka cluster %s is really working well.")
