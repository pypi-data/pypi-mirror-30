...
class BrokerTaskManager(object):
    ...
    def __init__(self, region=None, profile=None, cluster_name=None, task_definition=None, remote_container_id=None, local_container_id=None):
        ...
        splits = self.task_definition.split("-taskdef")
        if len(splits) != 2:
    ...
    def __validate(self): 
        try:
            ...
            logger.info("success to list task definitions with family prefix %s, the response is %s.", self.familyPrefix, response)
