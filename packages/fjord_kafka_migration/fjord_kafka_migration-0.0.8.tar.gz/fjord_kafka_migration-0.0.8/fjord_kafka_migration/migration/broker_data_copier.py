...
class BrokerDataCopier(object):
    '''
    Broker data copier is mainly used to sync data from remote server to loal server.
    '''
    ...
    def do_pssh_copy(self):
        ...
        try:
            ...
            logger.info("create a shared pssh client with server login info host = %s, user = %s, password = ****.", self.remote_host, self.remote_host)
    ...
    def check_dir_exist(self, dir_path):
        if not os.path.exists(dir_path):
            ...
            raise Exception("the folder with %s is not exist.", dir_path)
        else:
            logger.info("the folder with %s is exist.", dir_path)
            pass
