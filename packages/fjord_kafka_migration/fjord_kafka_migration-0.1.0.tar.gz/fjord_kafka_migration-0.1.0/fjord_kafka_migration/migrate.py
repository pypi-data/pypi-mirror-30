...
def run():
    ...
    if remote_instance_id is None:
        logger.error("it's failed to retrieve ec2 instance id with container id %s, please address the issue and run again.", remote_container_id)
        sys.exit(1)
    
    remote_broker_address = manager.get_broker_ipaddress(remote_instance_id)
    if remote_broker_address is None:
        logger.error("it's failed to get server address with container id %s, the migration will stop processing and wait for your fix.", remote_container_id)
        sys.exit(1)
    ...
    try:
        manager.drain_broker_ec2(remote_container_id)
        print("it's success to drain container %s out of cluster %s.", remote_container_id, args.cluster_name)
    except Exception as e:
    ...
    try:
        ...
        print(" it's success to stop broker running in container %s.", remote_container_id)
    except Exception as e:
    ...
    try:
        ...
        print("it's success to start a new task running in container %s.", local_container_id)
    except Exception as e:
        logger.error("error happened during start a new task, the error is %s.", e)
        sys.exit(1)
        
    print("Configuration!!!, broker migration is done, please sincerely check the Kafka cluster's status.")
