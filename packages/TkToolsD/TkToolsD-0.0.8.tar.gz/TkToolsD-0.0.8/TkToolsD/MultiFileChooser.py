...
class MultiFileChooser(ttk.Treeview) :
	...
	def __updateStyles(self) :
		...
		f = Font()
		f = Font(size=str(2-height))
		self.style.configure('Treeview', rowheight=height)
		for t in self.tagNames :
			img = GetImgTk('res/%s.png'%(t), size)
			self.__imgs[t] = img
			self.tag_configure(t, image=img, font = f)		
...
# ---------------------------------scrollbar begin-----------------------------------------
	...
	def movetoPercentV(self, percent) :
		self.self.yview('moveto', str(percent/100.0))

	def movetoPercentH(self, percent) :
		self.self.xview('moveto', str(percent/100.0))
	...
	def moveUpOneStep(self) :
		self.self.yview(tk.SCROLL, -1, tk.UNITS)

	def moveDownOneStep(self) :
		self.self.yview(tk.SCROLL, 1, tk.UNITS)

	def moveLeftOneStep(self) :
		self.self.xview(tk.SCROLL, -1, tk.UNITS)
	...
	def __yview(self, *args, **dArgs) :
		...
		self.self.yview(*args, **dArgs)

	def __xview(self, *args, **dArgs) :
		# print('xview', args, dArgs)
		self.self.xview(*args, **dArgs)
# ===================================scrollbar end===============================================
