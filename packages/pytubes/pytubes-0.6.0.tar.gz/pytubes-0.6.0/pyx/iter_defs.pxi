...
# AUTO GENERATED FROM tools/defn.tmpl
cdef extern from "/Users/stephenstagg/src/pytubes/src/iters/tsv.hpp" namespace "ss::iter":
    cdef cppclass TsvIter:
        TsvIter(Chain, AnyIter, bool_t) except +
...
# AUTO GENERATED FROM tools/defn.tmpl
cdef extern from "/Users/stephenstagg/src/pytubes/src/iters/get_index.hpp" namespace "ss::iter":
    cdef cppclass IndexLookupIter[T]:
        IndexLookupIter(AnyIter, vector[size_t]) except +
...
@cython.final
cdef class Tsv(Tube):  # AUTO GENERATED FROM tools/defn2.tmpl
    
    cdef public Tube parent
    cdef public bool_t headers

    def __cinit__(self, Tube parent, bool_t headers):
        
        if parent.dtype[0] not in (ByteSlice,):
            raise ValueError(f"Cannot make a Tsv Tube with 'parent' tube of type { parent.dtype[0] }")
        self.parent = parent
    ...
    cdef IterWrapper _make_iter(self, args):
        ...
        cdef TsvIter *iter = new TsvIter(iters_to_c_chain(chains[0]), parent.iter, self.headers)
    ...
    cpdef _describe_self(self):
        return f"Tsv({repr(self.headers)})"
...
cdef class IndexLookup(Tube):  # AUTO GENERATED FROM tools/defn2.tmpl
    ...
    cpdef _describe_self(self):
        return f"IndexLookup({repr(self.indexes)})"
