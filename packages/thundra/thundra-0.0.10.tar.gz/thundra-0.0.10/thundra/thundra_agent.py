...
class Thundra:
    def __init__(self, api_key=None, disable_trace=False, request_skip=False, response_skip=False):
        ...
        self.data['request_skipped'] = self.should_skip(audit_request_skip_by_env, request_skip)

        audit_response_skip_by_env = utils.is_thundra_lambda_audit_response_skipped()
        self.response_skipped = self.should_skip(audit_response_skip_by_env, response_skip)
    ...
    def should_skip(self, disable_by_env, disable_by_param=False):
        if disable_by_env == 'true':
            return True
        elif disable_by_env == 'false':
            return False
