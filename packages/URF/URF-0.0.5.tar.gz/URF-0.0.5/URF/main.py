...
def plot_cluster_result(prox_mat, cluster_ids, n_components=2, marker=None, output=None, show_legend=True, pre_computed_sim=True, alpha=0.1, **kwargs):
    """
    Plot cluster result
    :param prox_mat:
    :param cluster_ids:
    ...
    :param output:
    :param show_legend:
    :param alpha: alpha for points on the scatter plot
    :param kwargs:
    :return:
    """
    if pre_computed_sim:
        ...
        for i in list(set(marker)):
            for j in list(set(cluster_ids)):
                ...
                plt.scatter(x_transformed[indecies, 0], x_transformed[indecies, 1],
                            c=color_set[i], marker=markers[j], alpha=alpha, label="Y: "+str(i)+"-C: "+str(j))
    else:
        plt.scatter(x_transformed[:, 0], x_transformed[:, 1], c=cluster_ids, alpha=alpha, cmap=plt.cm.Spectral)
