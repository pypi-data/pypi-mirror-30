...
class GrainedGeneticAlgorithmBase(GeneticAlgorithmBase):
    ...
    @log_method()
    @timeout(60)
    def _synchronize_with_neighbours(self, generation, rand):
        ...
        while cnt != self._population_size:
            ...
            if body:
                ...
                time.sleep(0.2)
            text_file = open("/tmp/Output" + str(rand) + ".txt", "w")
            text_file.write("curr " + str(cnt) + " needed " + str(self._population_size))
            text_file.close()
    ...
    def __call__(self, initial_data, channels):
        ...
        rand = random.choice(range(0,1000))
        for generation in range(0, self._number_of_generations):
            ...
            self._synchronize_with_neighbours(generation, rand)
