...
def _apns_send(token, alert, badge=None, sound=None, category=None, content_available=False,
    ...
    if socket:
        ...
        with closing(_apns_create_socket_to_push(certfile=certfile)) as socket:
            socket.write(frame)
...
def apns_send_message(registration_id, alert, certfile=None, **kwargs):
    """
...
def apns_send_bulk_message(registration_ids, alert, certfile=None, **kwargs):
    ...
    with closing(_apns_create_socket_to_push(certfile=certfile)) as socket:
        for identifier, registration_id in enumerate(registration_ids):
            _apns_send(registration_id, alert, identifier=identifier, socket=socket, certfile=certfile, **kwargs)
        _apns_check_errors(socket)
