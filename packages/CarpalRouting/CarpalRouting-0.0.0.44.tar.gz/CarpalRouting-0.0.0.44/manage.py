        ...
        try:
            with open(file) as setting_file:
                ...
                for index, row in df.iterrows():
                    ...
                    pickup_window = tuple(map_time_to_seconds(row['pickup_time_window']))

                    if pickup_address in locations:                
                        new_item = {'address': row['delivery_address'],
                                    'coordinates': map_address_to_coordinates(row['delivery_address'], 
                                                                              settings.get('googleAPIKey')) \
                                                   if settings.get('addressType')=='address' else \
                                                   map_str_to_coordinates(row['delivery_address']),
                                    'delivery_window': map_time_to_seconds(row['delivery_time_window']),
                                    'capacity': row['total_weight'],
                                    'order_time': 0,
                                    'ref': str(row['customer_order_number'])}
                        if pickup_window in locations[pickup_address]:             
                            locations[pickup_address][pickup_window].append(new_item)
                        else:
                            locations[pickup_address][pickup_window] = [{
                                'address': pickup_address,
                                'coordinates': map_address_to_coordinates(pickup_address,
                                                                          settings.get('googleAPIKey')) \
                                               if settings.get('addressType')=='address' else \
                                               map_str_to_coordinates(pickup_address),
                                'delivery_window': map_time_to_seconds(row['pickup_time_window']),
                                'capacity': 0,
                                'order_time': 0
                            }]
                            locations[pickup_address][pickup_window].append(new_item)
                ...
                data, not_found = LocationGrouping().group_locations(locations, 
                                                                     schedules, 
                ...
                for pickup_address, pickup_windows in data.items():
                    for pickup_window, obj in pickup_windows.items():
                        ...
                        while locs and driver_ids:
                            ...
                            #find the indices for all filtered locations in data object
                            filtered_indices = []
                                ...
                                for item in indices[0]:
                                    if ';' in locs[filtered_indices[item]].ref:
                                        for sub_item in locs[filtered_indices[item]].ref.split(';'):
                                            ...
                                            routes.append(Location(coordinates = Coordinates(locs[filtered_indices[item]].coordinates[0], 
...
def map_address_to_coordinates(address, key):
    response = requests.get('https://maps.googleapis.com/maps/api/geocode/json?address={}&key={}'.format(address.replace('\n', ' '), key))
    content = json.loads(response.__dict__.get('_content'))
    if content.get('results'):
        loc = content.get('results')[0].get('geometry').get('location')
