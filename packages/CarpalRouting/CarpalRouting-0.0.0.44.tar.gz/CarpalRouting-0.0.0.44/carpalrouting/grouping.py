...
class DriverGrouping(object):
    ...
    def group(self, drivers, locations, start_buffer_time=0, end_buffer_time=0):
        #Verifying data type        
        illegal_driver_data = [item for item in drivers if not isinstance(item, Driver)]
        illegal_location_data = [item for item in locations if not isinstance(item, Location)]
        
        if illegal_driver_data:
            raise TypeError('All element in the list should have the same type Driver.')

        if illegal_location_data:
            raise TypeError('All element in the list should have the same type Location.')
        ...
        result['unprocessed'] = [item for item in locations if item is not None]
        return result
...
class LocationGrouping(object):
    ...
    def _group_locations_by_time_slot(self, locations, time_slot, threshhold=0):
        """
    ...
    def group_locations(self, locations, schedules, merge_identical=False):
        ...
        Sample processed:
        {'pickup1': {(12000, 18000) : {(43140, 79200): [<carpalrouting.models.Location object at 0x110b62be0>], 
                     (0, 43140): [<carpalrouting.models.Location object at 0x1127d66d8>, <carpalrouting.models.Location object at 0x112a15748>]}}, 
         'pickup2': {(12000, 18000) : {(28800, 79200): [<carpalrouting.models.Location object at 0x110778320>, <carpalrouting.models.Location object at 0x112f0ecf8>, <carpalrouting.models.Location object at 0x112f0ee48>]}}, 
         'pickup3': {(12000, 18000) : {(28800, 79200): [<carpalrouting.models.Location object at 0x112a15dd8>]}}}
                    ...
                    if len(dropoff_list) > 2: 
                        for k, v in enumerate(dropoff_list):
                            ...
                            for idx in range(k+1, len(dropoff_list)):                                                              
                                if dropoff_list[idx] and dropoff_list[idx].get('coordinates') == [0, 0]:
                                    not_found.add(dropoff_list[idx].get('address'))
                                    dropoff_list[idx] = None
                                elif dropoff_list[idx] and v.get('coordinates') == dropoff_list[idx].get('coordinates') and \
                                    v.get('delivery_window') == dropoff_list[idx].get('delivery_window'):
                                        if ',' not in v['ref']: 
                                            v['ref'] = ','.join([v['ref'], str(v['capacity'])]) + ';' + ','.join([dropoff_list[idx].get('ref'), str(dropoff_list[idx]['capacity'])])
                                        else:
                                            v['ref'] = v['ref'] + ';' + ','.join([dropoff_list[idx].get('ref'), str(dropoff_list[idx]['capacity'])])

                                        v['capacity'] += dropoff_list[idx].get('capacity')
                                        dropoff_list[idx] = None
                            #dropoff_list = [loc for loc in dropoff_list if loc]                                                                        
                    elif dropoff_list[1].get('coordinates') == [0, 0]:
                        not_found.add(dropoff_list[1].get('address'))
                        dropoff_list[1] = None

                    pickup_windows[pickup_window] = [loc for loc in dropoff_list if loc]    

        for pickup, pickup_windows in locations.items():
            result[pickup] = {}
            for pickup_window, dropoff_list in pickup_windows.items():                                 
                result[pickup][pickup_window] = {'locations': [Location(coordinates=Coordinates(item.get('coordinates')[0], item.get('coordinates')[1]),
                                                                        address=item.get('address'),
                                                                        delivery_window=item.get('delivery_window'),
                                                                        capacity=item.get('capacity', 1),
                                                                        ref=item.get('ref', '')) for item in dropoff_list], 'driver_ids': []}
                ...
                for time_slot, driver_list in copied_schedules.items():
                    if driver_list:
                        if time_slot[0] <= pickup_window[0] and time_slot[1] >= pickup_window[1]:
                            driver = driver_list.pop(0)
                            result[pickup][pickup_window]['driver_ids'].append(driver.id)
                ...
                for time_slot, driver_list in copied_schedules.items():
                    if driver_list:
                        if time_slot[0] <= pickup_window[0] and time_slot[1] >= pickup_window[1]:
                            result[pickup][pickup_window]['driver_ids'].extend([driver.id for driver in driver_list])
                            driver_list = []                                         
...
class WaveGrouping(object):
    ...
    def group(self, waves, locations, buffer_time=0):
        illegal_wave_data = [item for item in waves if not isinstance(item, Wave)]
        illegal_location_data = [item for item in locations if not isinstance(item, Location)]

        if illegal_wave_data:
            raise TypeError('All element in the list should have the same type Wave.')
        ...
        result = {}
        for wave in waves:
            result[wave.cutoff_time] = [locations.pop(idx) for idx, val in enumerate(locations) if \
                val.order_time + buffer_time < wave.cutoff_time]
