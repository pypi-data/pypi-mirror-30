                    ...
                    if len(dropoff_list) > 2: 
                        for k, v in enumerate(dropoff_list):
                            ...
                            for idx in range(k+1, len(dropoff_list)):                                                              
                                ...
                                elif dropoff_list[idx] and v.get('coordinates') == \
                                    dropoff_list[idx].get('coordinates') and \
                                    v.get('delivery_window') == dropoff_list[idx].get('delivery_window'):
                                        if ',' not in v['ref']: 
                                            v['ref'] = ','.join([v['ref'], str(v['capacity'])]) + \
                                                       ';' + ','.join([dropoff_list[idx].get('ref'), 
        ...
        for pickup, pickup_windows in locations.items():
            ...
            for pickup_window, dropoff_list in pickup_windows.items():                                 
                ...
                for time_slot, driver_list in copied_schedules.items():
                    if driver_list and time_slot[0] <= pickup_window[0] and \
                        time_slot[1] >= pickup_window[1]:
                ...
                for time_slot, driver_list in copied_schedules.items():
                    if driver_list and time_slot[0] <= pickup_window[0] and \
                        time_slot[1] >= pickup_window[1]:
                        result[pickup][pickup_window]['driver_ids'].extend(
                            [driver.id for driver in driver_list])
                        copied_schedules[time_slot] = []                                       
