...
# :Copyright: © 2008, 2013, 2014, 2015, 2016, 2017, 2018 Lele Gaifax
#
...
def test_query_metadata():
    ...
    assert [f['default'] for f in res['metadata']['fields'] if f['name'] == 'smart'] == [True]
    assert [f['default'] for f in res['metadata']['fields'] if f['name'] == 'somevalue'] == [42]
...
def test_with_join():
    ...
    res = proxy(sas, sorters='[{"property":"number","direction":"DESC"},{"property":"non-existing","direction":"ASC"}]')
    assert res[0].firstname == 'Lele'
...
def test_union():
    query1 = select([persons.c.firstname],
                    persons.c.firstname == 'Lele')
    query2 = select([persons.c.firstname],
