...
def clean(args, db):
    ...
    for d in _allDbs(args):
        ...
        if not args.db_prefix and i and d not in toKeep and i in toClean:
            ...
            if rc != 0:
                return rc
        elif args.db_prefix and d not in toKeep and i in toClean:
            ufload.progress("Cleaning other database for instance %s: %s_%s" % (i, args.db_prefix, d))
            killCons(args, d)
            rc = psql(args, 'DROP DATABASE IF EXISTS \"%s_%s\"' % (args.db_prefix, d))
            if rc != 0:
                return rc
    return 0            
