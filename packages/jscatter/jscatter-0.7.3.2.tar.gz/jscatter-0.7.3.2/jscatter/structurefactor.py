...
from . import formel
from .lattice import rhombicLattice, bravaisLattice, scLattice,bccLattice,\
    fccLattice,hexLattice,hcpLattice
...
def _sqcoefOriginalHP(ir,eta,gek,ak,a=0.,b=0.,c=0.,f=0.,u=0.,v=0.,gamk=0.,seta=0.,sgek=0.,sak=0.,scal=0.,g1=0.):
    ...
    For some parameter conditons the rescaling is needed but not done.
    ...
    if ak>=(1+8.*eta) or gamk>=0.15:
        ...
        if ak>=(1+8.*eta): # in this case return anyway
            ...
            if  seta>=eta:  # seta>eta indicates succesfull rescaling with g1 as zero
                return ir,eta,gek,ak,a,b,c,f,u,v,gamk,seta,sgek,sak,scal,g1
...
def stickyHardSphere(q,R,width,depth,molarity=None,phi=None):
    ...
    tau=math.exp(-depth)/12./eps  # stickyness
    eta=phi*(1-eps)**3
...
def _qhkl(a, type, h, k, l=0):
    # position of hexagonal peaks
    if type == 'hex':
        return (4. * np.pi / a / 3 ** (1 / 2.)) * (h ** 2 + k ** 2 + h * k) ** (1 / 2.)
    elif type in ['hcp']:
        return (2. * np.pi / a) * (4. / 3 * (h ** 2 + k ** 2 + h * k) + 3. / 8 * l ** 2) ** (1 / 2.)
    elif type in ['sc','bcc','fcc']:
        return (2. * np.pi / a) * (h ** 2 + k ** 2 + l ** 2) ** (1 / 2.)
    elif type == 'sq':
        return (2. * np.pi / a) * (h ** 2 + k ** 2) ** (1 / 2.)
    elif type == 'lam':
        return (2. * np.pi / a) * (h ** 2) ** (1 / 2.)


def _fhkl(type, h, k, l):
    # extinction rules
    if type in ['hex', 'lam', 'sq', 'sc', ]:
        return 1.
    elif type == 'bcc':
        return 2. if ((h + k + l) % 2 == 0) else 0.
    elif type == 'fcc':
        if ((h % 2 == 0) & (k % 2 == 0) & (l % 2 == 0)):
            return 4.
        elif ((h % 2 != 0) & (k % 2 != 0) & (l % 2 != 0)):
            return 4.
        else:
            return 0.
    elif type == 'hcp':
        if ((l % 2 == 0) & ((h + 2 * k) / 3).is_integer()):
            return 2.
        elif ((l % 2 != 0) & ((h + 2 * k - 1) / 3).is_integer() & ((h + 2 * k - 2) / 3).is_integer()):
            return 3 ** 0.5
        elif ((l % 2 == 0) & ((h + 2 * k - 1) / 3).is_integer() & ((h + 2 * k - 2) / 3).is_integer()):
            return 1.
        else:
...
def _mhkl(type, h, k, l):
    # peak multiplicity
    if type == 'lam': # 1D
        return 1.
    elif type == 'hex': # 2D
        return 6 if (k == 0) or (h == k) else 12
    elif type == 'sq': # 2D
        return 4 if (k == 0) or (h == k) else 8
    elif type in ['bcc', 'fcc', 'sc']:
        if (k == 0) & (l == 0):
            return 6
        elif (h == k) & (l == 0):
            return 12
...
def latticeStructureFactor(q,a=1,domainsize=1000,type='sc',asym=0,lg=1,rmsd=0.02,beta=None,hklmax=7,c=1.):
    r"""
    Structure factor of a crystal lattice with particle asymmetry, DebyeWaller factor, diffusive scattering and domain size.

    This is the structure factor :math:`S(q)` in equ 30 of [1]_. :math:`\Omega_d` omitted
    To get the full scattering the formfactor needs to be included (See Notes and module formfactor).
    ...
    domainsize : float
        Domainsize of the crystal, units as a.
    ...
    asym : float, default=0
        ...
        For a=0 the Voigt is symmetric with fwhm. See formel.voigt .
    ...
    - The scattering intensity of a crystal domain is
      ...
      For inhomogenious particles we can incorporate :math:`\Delta\rho(r)` in the formfactor :math:`P(q)`
    ...
    - The structure factor is
      ...
      with
       - :math:`\beta(q)=<F(q)>^2/<F(q)^2>` as asymmetry factor [3]_ dependent on the
         scattering amplitude :math:`F(q)` and particle polydiispersity
    ...
    - The  lattice factor is
      ...
      with
       ...
       - :math:`f_{hkl}`     symmetry factor that takes into account symmetry-related extinction rules
    ...
    - For the limitting behaviour q->0 see the discussion in [1]_ in 3.9. :
       "... The zero-order peak is not explicitely considered because of the q^(1-dim) singularity and
    ...
    ln2=np.log(2)
    if type  == 'hex':
        af = 1                      # closest distance between next neighbors af*a
        omd = 2*np.pi               # D dimensional solid angle
        nn = 1                      # number of particles per unit cell
        vd= 3**0.5*a*a/2.           # volume unit cell
        ...
        raise KeyError('Type not implemented! ')
    # generate list of indices dependent on dimension; fhkl contains selection rules
    if dim==1:
        hkl = [(h, 0, 0) for h in np.r_[0:hklmax]  ][1:]
    elif dim==2:
        hkl = [(h, k, 0) for h in np.r_[0:hklmax] for k in np.r_[0:h + 1] ][1:]
    elif dim==3:
        hkl=[(h,k,l) for h in np.r_[0:hklmax] for k in np.r_[0:h+1] for l in np.r_[0:k+1]][1:]

    # Bragg peak shape
    Lhkl=lambda q,center,lg,domainsize,asym: formel.voigt(q,center=center,lg=lg,fwhm=2*np.pi/domainsize,asym=asym).Y

    # Debye Waller factor
    DW=np.exp(-q ** 2 * rmsd ** 2 )

    # lattice factor
    Z0q = np.c_[[_mhkl(type,h,k,l) *
                 _fhkl(type,h,k,l)**2 *
                 Lhkl(q,_qhkl(a,type,h,k,l),lg,domainsize,asym)
                                         for h,k,l in hkl if _fhkl(type,h,k,l)>0]].sum(axis=0)
    Z0q *= (2 * np.pi) ** (dim - 1) * c / nn / vd / q ** (dim - 1) #  /omd  # omd not mentioned in [5]_
    ...
    result.setColumnIndex(iey=None)
    result.q_hkl= [_qhkl(a,type,h,k,l)  for h,k,l in hkl if _fhkl(type,h,k,l) > 0]
    result.hkl=   [[h,k,l]              for h, k, l in hkl if _fhkl(type, h, k, l) > 0]
    result.fhkl = [_fhkl(type, h, k, l) for h, k, l in hkl if _fhkl(type, h, k, l) > 0]
    result.mhkl = [_mhkl(type, h, k, l) for h, k, l in hkl if _fhkl(type, h, k, l) > 0]
    result.latticeconstant=a
    result.peakFWHM=2*np.pi/domainsize
    result.peaksigma=(result.peakFWHM/ (2*np.sqrt(2*ln2)))
...
def hydrodynamicFunct(wavevector,Rh,molarity,intrinsicVisc=None,DsoverD0=None,
    ...
    numberOfPoints : integer, default 50
        Determines number of integration points in equ 5 of ref [1]_ and therfore accuracy of integration.
    ...
    if structureFactor==None:
        # we use PercusYevic Structure factor --> hard spheres
