...
class GraceGraph:
    ...
    def gen_axis(self, axis_prefix, ax_min=None, ax_max=None,
        ...
        label : string, list or Label object
            ...
            If list it determines [labelstring,charsize,place,color]
    ...
    def save(self, filename,size=(1012,760),dpi=300, format='agr'):
        ...
        # to be basically the same each time, and each device is subtly different
            ...
            com=['PAGE RESIZE %i,%i' % (size[0],size[1]),
             'device "%s" dpi %3i' % (string.upper(ext[1:]),dpi),
             'hardcopy device "%s"' % (string.upper(ext[1:])),
             'print to "%s"' % filename,
             'print']
...
class Symbol:
    ...
    size : float
        is self explanatory, 0.5 is 50 in the GUI
    pattern : int 0..24
        appears to be the pattern of the outline of the symbol, usually it will be 1,
        c
    linewidth : float
