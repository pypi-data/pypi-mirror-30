...
import copy
import json
from creaturecast_designer.extensions.qtpy.QtCore import *
from creaturecast_designer.extensions.qtpy.QtGui import *
from creaturecast_designer.extensions.qtpy.QtWidgets import *
import creaturecast_designer.extensions.PySignal as psg
...
from creaturecast_designer.services.thread_worker import GeneratorWorker
...
class ExtensionWidget(QFrame):
    ...
    def download(self):
        ...
        self.worker = GitFetchWorker(self.table_object.git_url)
...
class ToolWidget(QFrame):
    ...
    def emit_open_widget(self):
        self.open_tool.emit(self.table_object)

    def emit_new_widget(self):
        self.new_tool.emit(self.table_object)
    ...
    def uninstall(self):
        ...
        self.open_button.setVisible(False)
        self.git_line_edit.setText(self.table_object.git_url.split('/')[-1])
        package_name = self.table_object.class_location.split('.')[0]
    ...
    def finish_install(self):
        ...
        tol.add_tools(dict(
            git_url=self.table_object.git_url,
            name=self.table_object.name,
            icon=self.table_object.icon,
            image=self.table_object.image,
            class_location=self.table_object.class_location
        ))
    ...
    def finish_uninstall(self):
        ...
        existing_tools = tol.session.query(tob.Tool).filter(tob.Tool.git_url == self.table_object.git_url).all()
        ...
        self.uninstalling_label.setVisible(False)
        #self.icon_label.setEnabled(False)
        self.delete_tool.emit(self.table_object)
...
class GitFetchWorker(QThread):
    ...
    def run(self):
        ...
        class ProgressHandler(RemoteProgress):
            update_progress = psg.ClassSignal()
