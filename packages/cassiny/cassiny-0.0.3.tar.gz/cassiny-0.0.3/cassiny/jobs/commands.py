...
@click.option('--preemptible', help="Use a spot machine", is_flag=True)
@click.option('--command', '-c', help="Command to run", default=None)
def create(description, blueprint, machine, gpu, preemptible, command):
    """Create a new job."""
...
def list():
    ...
    data = []
    data.append(["", "Name", "Command", "Blueprint", "Status", "Machine/GPU/Preemptible"])
    for index, job in enumerate(jobs):

        gpu = "Y" if job['specs']['gpu'] else "N"
        preemptible = "Y" if job['specs']['preemptible'] else "N"
        data.append([
            index,
            job['name'],
            job['specs']['command'],
            ...
            job['status'],
            f"{job['specs']['machine_type']}/{gpu}/{preemptible}",
        ])
