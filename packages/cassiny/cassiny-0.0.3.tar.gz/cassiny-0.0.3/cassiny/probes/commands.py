...
def create(description, blueprint, machine, gpu, preemptible):
    """Create a new probe."""
...
def list():
    ...
    data.append(["", "Name", "Url", "Token", "Blueprint",
                 "Status", "Machine/GPU/Preemptible"])
    for index, probe in enumerate(probes):

        gpu = "Y" if probe['specs']['gpu'] else "N"
        preemptible = "Y" if probe['specs']['preemptible'] else "N"
        data.append([
            index,
            probe['name'],
            probe['url'],
            ...
            probe['status'],
            f"{probe['specs']['machine_type']}/{gpu}/{preemptible}",
        ])
