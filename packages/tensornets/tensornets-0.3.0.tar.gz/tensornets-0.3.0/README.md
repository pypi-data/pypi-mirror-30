...
Each object detection model **can be coupled with any network in TensorNets** (see [performances](#object-detection)) and takes two arguments: a placeholder and a function acting as a stem layer. also returns `tf.Tensor` as its output. Here is an example of `YOLOv2` with `nets.Darknet19` for PASCAL VOC:
...
- The object detection models can be coupled with any network but mAPs could be measured only for the models with pre-trained weights. Note that:
  ...
  * `FasterRCNN\_ZF\_VOC`: `FasterRCNN(inputs, ZF)`
  * `FasterRCNN\_VGG16\_VOC`: `FasterRCNN(inputs, VGG16, stem_out='conv5/3')`.
- The mAPs were obtained with TensorNets on **PASCAL VOC2007 test set** and may slightly differ from the original ones.
Each object detection model **can be coupled with any network in TensorNets**
