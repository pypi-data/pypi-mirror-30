...
from .psf_library import PSFLibrary,PSFLibrary_v1
...
class Instrument(InstrumentConfig):
    ...
    **kwargs: list of keyword/value pairs
    ...
    def _load_psf_library(self,psf_key):
        ...
        # Select old-style versus new-style PSF library
        versionfile = open("{0:}/VERSION_PSF".format(default_refdata_directory))
        versionfile.readline()
        version = versionfile.readline()[:-1]  # strip newline
        if "V2 Library" in version:
            library = PSFLibrary(path=psf_path, aperture=psf_key)
        else:
            library = PSFLibrary_v1(path=psf_path, aperture=psf_key)
    ...
    def get_exposure_pars(self, name="pattern_name"):
        ...
        nprerej = self.ramp_config["nprerej"]
        npostrej = self.ramp_config["npostrej"]
