    ...
    **kwargs: list of keyword/value pairs
    ...
    def _error_sum(self, a_ij, product_subscripts, my_detector_signal, my_detector_noise):
        ...
        for product_subscript in product_subscripts:
            ...
            nsaturated = {'partial':nsaturated_partial,'full':nsaturated_full}
    ...
    def _add_exposure_products(self, exposure_products_list):
        ...
        exposure_sum = {
            ...
            'saturation_products': {'full':np.zeros(product_shape),'partial':np.zeros(product_shape)}
...
class ImagingApPhot(Strategy):
    ...
    **kwargs: list of keyword/value pairs
    ...
    def __init__(self, instrument, webapp=False, config={}, **kwargs):
        ...
        # https://github.com/STScI-SSB/pandeia/issues/1686
...
class Coronagraphy(ImagingApPhot):
    ...
    def _add_exposures(self, exposure_list, exposure_sum, reconstructed_signal, reconstructed_noise, reconstructed_saturation):
        ...
        exposure_sum, reconstructed_signal, reconstructed_noise, reconstructed_saturation = Strategy._add_exposures(self, exposure_list, exposure_sum, reconstructed_signal, reconstructed_noise, reconstructed_saturation)
    ...
    def _make_dither_weights(self, exposure_list):
        ...
        if self.do_contrast:
            ...
            if self.psf_subtraction == "optimal":
                ...
                exposure_list = exposure_list[:2]
            # this option has no autoscaling, and was the default behavior in v1.1.1 and previous. Called "Optimal (No PSF Autoscaling)" in the client.
            elif self.psf_subtraction == "no_autoscale":
                self.dither_weights = [1, -1]
                exposure_list = exposure_list[:2]
            # this option returns only the first subreport (the scene w/central source and snr target), called "Unsubtracted Science Scene" in the client.
            elif self.psf_subtraction == "target_only":
                self.dither_weights = [1]
                exposure_list = [exposure_list[0]]
            # this option returns only the second subreport (just the PSF subtraction source), called "PSF Subtraction Source Only" in the client.
    ...
    def rotate(self,sources):
        """
        Rotate a Coordinates strategy calculation

        Parameters
        ----------
        sources: Source
            A Source class object with sources in the scene that contain both coordinates and source geometry

        Returns
        -------
        sources: Source
               The Source class object, with its sources rotated (and extended sources spun)
        """
        for i, s in enumerate(sources):
            x = s.position['x_offset']
            y = s.position['y_offset']
            sources[i].position['x_offset'],sources[i].position['y_offset'] = Strategy._rotate(self,[x,y])
        ...
        # rotate Coronagraphy-specific things
        self.psf_subtraction_xy = Strategy._rotate(self,self.psf_subtraction_xy)
        self.pointing_error = Strategy._rotate(self,self.pointing_error)
        self.unocculted_dither = Strategy._rotate(self,self.unocculted_xy)
        self.target_xy = Strategy._rotate(self,self.target_xy)
...
#        self._check_circular_aperture_limits(src_region, sky_subs, my_detector_signal.grid, aperture, annulus)
        ...
        # Straight-up summation without background
        weight_matrix = np.matrix(src_region)
...
class TACentroid(Strategy):
    ...
    **kwargs: list of keyword/value pairs
    ...
    def _create_weight_matrix(self, my_detector_signal_list, my_detector_noise_list):
        ...
        weight_matrix_x = np.matrix(dist_x*src_region/np.sum(my_detector_signal.rate*src_region))
        weight_matrix_y = np.matrix(dist_y*src_region/np.sum(my_detector_signal.rate*src_region))
