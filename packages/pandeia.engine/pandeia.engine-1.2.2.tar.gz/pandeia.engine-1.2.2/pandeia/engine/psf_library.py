...
class PSFLibrary:
    ...
    def _calculate_distances(self, psf_offsets, source):
        # Internal function used in associate_offset_to_source in order to consolidate the code
...
class PSFLibrary_v1(PSFLibrary):
    ...
    def _calculate_distances(self, psf_offsets, source):
        # Internal function used in assocate_offset_to_source in order to consolidate the code
        psf_xy_offsets = [(psf_offset[0] * np.cos(np.pi * psf_offset[1] / 180),
                           psf_offset[0] * np.sin(np.pi * psf_offset[1] / 180)) \
                          for psf_offset in psf_offsets]

        distances = [np.sqrt((source.position['x_offset'] - psf_xy_offset[0]) ** 2 + \
                             (source.position['y_offset'] - psf_xy_offset[1]) ** 2) \
                     for psf_xy_offset in psf_xy_offsets]
        return distances
    ...
    def associate_offset_to_source(self, sources, instrument, aperture_name):
        ...
        unique_angles = np.unique([psf_offset[1] for psf_offset in psf_offsets if psf_offset[0] != 0])

        psf_associations = []
        for source in sources:
            if unique_angles.size == 0:
                distances = self._calculate_distances(psf_offsets, source)

            elif unique_angles.size == 1:
                # if there is only one angle, we will assume azimuthal symmetry
                source_radius = np.sqrt(source.position['x_offset'] ** 2 + source.position['y_offset'] ** 2)
                distances = [np.abs(psf_offset[0] - source_radius) for psf_offset in psf_offsets]
            else:
                # multiple angles indicate that we'd expect the full 2D field
                # to be appropriately sampled by the PSF library.
                distances = self._calculate_distances(psf_offsets, source)
