...
from .io_utils import ref_data_interp, read_json, get_json_parameters
...
class NIRSpec(JWSTInstrument):
    ...
    def get_detector_pars(self):
        """
        Copied this here but may not need to override.
        """

        det_dict = {key: get_json_parameters(self.det_pars, key, readmode=self.detector_readmode)
                    for key in self.det_pars.keys() if not key == 'meta'}
...
class NIRCam(JWSTInstrument):
    ...
    def get_detector_pars(self):
        ...
        det_dict = {key: get_json_parameters(self.det_pars, key, aperture=aperture_lu)
                    for key in self.det_pars.keys() if not key == 'meta'}
...
class NIRISS(JWSTInstrument):
    ...
    def get_detector_pars(self):
        ...
        det_dict = {key: get_json_parameters(self.det_pars, key) for key in self.det_pars.keys() if not key == 'meta'}
...
class MIRI(JWSTInstrument):
    ...
    def _loadpsfs(self):
        ...
        """
        if (self.instrument['mode'] in ('target_acq')) and (self.instrument['aperture'] in ('fqpm1065','fqpm1140','fqpm1550','lyot2300')):
