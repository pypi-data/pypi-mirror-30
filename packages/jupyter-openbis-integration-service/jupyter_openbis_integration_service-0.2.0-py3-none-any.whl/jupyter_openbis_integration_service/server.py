...
class CreateNotebook(tornado.web.RequestHandler):
    ...
    def post(self, whatever):
        ...
        if test == None or test == 'False':
            test = False
        if test:
            self.write(json.dumps({ "fileName": os.path.join('/home/', username, folder, filename) }))
            return
        ...
        # add a number to the end of the file, if file exists (OS X like)
        # file.ext
        # file.ext 2
        # file.ext 3
        i = 1
...
@click.option('--create_users', is_flag=True, help='automatically create users, if they are not present in the system')
def start_server(port, openbis, create_users):
