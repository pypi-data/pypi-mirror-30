...
if os.name == 'nt':
    from .jemu_uart_tcp import JemuUartTcp as JemuUart
else:
...
class Vlab(object):
    ...
    def _load_from_local_instructions(self, gen_new_so):
        try:
            if gen_new_so:  # create new
                ...
                transpiler_cmd = ["node", "index.js", "--debug", "--platform", "nrf52", "--bin", self._firmware, "--zip",
                                  "--os", user_os]
    ...
    def _thread_stdout_function(self):
        ...
        while True and self._jemu_process:
            data = self._jemu_process.stdout.readline()  # todo: 1 should be removed
            if data == '':
                break
