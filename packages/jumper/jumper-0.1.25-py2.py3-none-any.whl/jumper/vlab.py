...
from .jemu_uart_tcp import JemuUartTcp
...
class Vlab(object):
    ...
    def __init__(
        ...
        # self._uart = JemuUartTcp(self)
        if os.name == 'nt':
            self._uart = JemuUartTcp(self)
        else:
            self._uart = JemuUart(os.path.join(self._working_directory, 'uart'), self)
        ...
        try:
            ...
            if not self._local_jemu and (user_os != CORE_LINUX_OS):
                self._web_api.add_event({'event': 'os error', "labels": {'os': user_os}})
                raise VlabException("Jumper Virtual Lab is currently only supported on Ubuntu 16.04 - try using Docker or Ubuntu.\nHead to https://docs.jumper.io/docs/install.html to learn more.", 6)
            ...
            for key in args.keys():
                if args[key]:
                    self._web_api.add_event({'event': 'jumper run ' + str(key)})
    ...
    def _send_gpio_event(self):
        self._web_api.add_event({'event': 'jumper run gpio'})

    def _send_stop_after_event(self):
        self._web_api.add_event({'event': 'jumper run stop_after'})
    ...
    def _load_from_cloud(self, gen_new_so, gen_new_jemu, file_path, new_signature):
        ...
        if gen_new_jemu or gen_new_so:
            if new_signature in self._examples_hash_map:
                self._web_api.add_event({'event': 'upload example firmware', "labels": {'example': True, 'example_name': self._examples_hash_map[new_signature], 'filename': filename}})
            else:
                self._web_api.add_event({'event': 'upload new firmware', "labels": {'example': False, 'filename': filename}})
    ...
    def start(self, ns=None):
        ...
        if self._jemu_debug:
            ...
            # self._jemu_port, self._uart.uart_port = self._get_ports_number()
    ...
    #                 port = line_data[index + 1].split('\n')[0]
    #             if line_data[index] == 'uart_port:':
    #                 uart_port = line_data[index + 1].split('\n')[0]
    #         loop_number += 1
            
    #         if port != 0 and uart_port != 0:
