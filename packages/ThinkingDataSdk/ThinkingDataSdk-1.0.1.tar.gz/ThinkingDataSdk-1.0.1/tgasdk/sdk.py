...
class TGAnalytics(object):
    ...
    def __assertProperties(self,action_type,properties):
        ...
        if properties is not None:
            ...
            if "#time" not in properties.keys():
                raise TGAIllegalDataException("property [#time] must be set")
            else:
                try:
                    time_temp = properties.get('#time')
                    if isinstance(time_temp, datetime.datetime) and isinstance(time_temp, datetime.date):
                        properties['#time'] = time_temp.strftime('%Y-%m-%d %H:%M:%S')
                    elif is_str(time_temp) and re.compile(r'^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}$').match(time_temp):
                        pass
                    else:
            ...
            for key, value in properties.items():
                ...
                if not is_str(value) and not is_int(value) and not isinstance(value, float) and not isinstance(value,datetime) and not isinstance(value,date):
    ...
    def __getKeyNamePattern(self,type):
        if 'track' == type:
            return re.compile(r"^(#time)|(#ip)|(#country)|(#province)|(#city)|(#os_version)|(#manufacturer)|(#lib_version)|(#os)|(#device_id)|(#screen_height)|(#device_model)|(#app_version)|(#screen_width)|(#lib)|(#network_type)|(#carrier)|(#browser)|(#browser_version)|([a-z][a-z0-9_]{0,50})$", re.I)
        else:
            return re.compile(r"^(#time)|(#ip)|([a-z][a-z0-9_]{0,50})$",re.I)
