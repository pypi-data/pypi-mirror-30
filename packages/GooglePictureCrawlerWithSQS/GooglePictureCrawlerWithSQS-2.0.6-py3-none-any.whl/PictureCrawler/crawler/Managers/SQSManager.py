...
class SQSManager(object):
    ...
    def __init__(self, awscredentialprofile):
        session = ''
        if awscredentialprofile == '':
            session = boto3.Session(aws_access_key_id=AWSConstants.AWS_CREDENTIAL_KEY, aws_secret_access_key=AWSConstants.AWS_CREDENTIAL_SECRET)
        else:
