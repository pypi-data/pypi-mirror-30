  ...
  #              _method.is_remote_server_method = True
   #             setattr(server,amethod[0],_method)
    ...
    def sync_send_request(self,body):
        msg = [b'',b'C',CREQUEST] + self.messageid.to_bytes(1,'big') + body
        server = body[0]
        self.send_message(msg)
    ...
    async def async_send_request(self,body):
        ...
        if server in self.futureobjectdict.keys():
            while self.messageid in self.futureobjectdict[server].keys():
                self.messageid += 1
        ...
        self.send_message(msg)
        ident = self.messageid.to_bytes(1,'big') + server
        answ = await self.recieve_message(ident)
        self.futureobjectdict.pop(ident)
        return answ
       # return msg
            ...
            try:
                ...
                if items:
                    ...
                    if command == CREQUEST + CSUCCESS:
                        ...
                        print('got here')
                        self.futureobjectdict[server+messageid].set_result(msg)
                        print('and here')
                    elif command == CREQUEST + CFAIL:
                        messageid = msg.pop(0)
                        server = msg.pop(0)
                        self.futureobjectdict[server+messageid].set_exception(Exception(msg.pop(0)))
