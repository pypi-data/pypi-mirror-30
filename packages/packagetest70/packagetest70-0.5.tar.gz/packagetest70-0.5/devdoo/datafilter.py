...
class DataFilter:
    ...
    def add(self, field, value):
        if type(value) == str:
            prepare_filter = self.__data_types.to_str(field + "(" + value + ")")
        else:
    ...
    def __conflict(self):
        ...
        if len(fields_errors) > 0:
            self.status.warn(3363569, "ERROR:: CAMPOS EM CONFLITOS" + self.__data_types.to_str(fields_errors))
        ...
        for param in list_params:
            ...
            if match_comparison_query_perators:
                ...
                if re.match(r"^([a-z0-9_]+\.?)+$", group_filter, re.IGNORECASE):
                    ...
                    self.status.warn(87541545, "ERROR - Filtro inválido" + self.__data_types.to_str(group_filter))
                ...
                if not re.match(r"(?:[a-z0-9]+\([\s\S]+\)[^+])\Z", group_value, re.IGNORECASE):
                    if re.match(r"^([a-z0-9_]+\.?)+$", group_filter, re.IGNORECASE):
                        ...
                        self.status.warn(523152, "ERROR FILTRO- de separador '|' nao encontrado" + self.__data_types.to_str(group_filter))
                ...
                if not re.match(r"(?:[a-z0-9]+\([\s\S]+\)[^|])\Z", group_value, re.IGNORECASE):
                    if re.match(r"^([a-z0-9_]+\.?)+$", group_filter, re.IGNORECASE):
                        ...
                        self.status.warn(5454545454, "Filtro inválido" + self.__data_types.to_str(group_filter))
    ...
    def __to_list(self, params, sep=";"):
        ...
        if type(params) == str or type(params) == unicode:
            ...
            for item in params:
                if self.is_empty(item) is not False:
