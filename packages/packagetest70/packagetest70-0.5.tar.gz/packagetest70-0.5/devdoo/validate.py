...
class Validate(object):
    def __init__(self, custom_service, status):
        self.status = status
    ...
    def array(self, scheme, value):
        ...
        original_value = value = DataTypes.to_str(value)
        ...
        value = DataTypes.to_array(value)

        # Verifica se o resultado do tipo convertido é um array válido
        if not DataTypes.is_array(value):
            field = scheme["field"]
    ...
    def boolean(self, scheme, value):
        ...
        value = DataTypes.to_boolean(value)

        # Verifica se o resultado do tipo convertido é um boleano válido
        if not DataTypes.is_boolean(value):
            field = scheme["field"]
            # Registra mensagem de erro no serviço de log e retona ao cliente
    ...
    def enum(self, scheme, value):
        ...
        if (value == '__empty__') or (value == '__none__') or (value not in options):
            field = scheme["field"]
            # Registra mensagem de erro no serviço de log e retona ao cliente
            self.status.alerts("TYPE_INVALID_ENUM", value, [field, Validate.clean_value(original_value), DataTypes.to_str(options)])
            value = '__error__'
    ...
    def object(self, scheme, value):
        ...
        # Converte valor para número
        value = DataTypes.to_object(value)

        # Verifica se o resultado do tipo convertido é objeto válido
        if not DataTypes.is_object(value):
            field = scheme["field"]
            # Registra mensagem de erro no serviço de log e retona ao cliente
    ...
    def object_id(self, scheme, value):
        ...
        # Converte valor para número
        value = DataTypes.to_object_id(value)

        # Verifica se o resultado do tipo convertido é um object_id válido
        if not DataTypes.is_object_id(value):
            field = scheme["field"]
            # Registra mensagem de erro no serviço de log e retona ao cliente
    ...
    def timestamp(self, scheme, value):
        ...
        # Converte valor para número
        value = DataTypes.to_timestamp(value)

        # Verifica se o resultado do tipo convertido é um timestamp válido
        if not DataTypes.is_timestamp(value):
            field = scheme["field"]
            # Registra mensagem de erro no serviço de log e retona ao cliente
    ...
    def __field(self, field_name, scheme, value, action=None):
        ...
        if action is not None:
            field_name, field_value = self.__custom(action, scheme, field_value, self.status)
    ...
    # __custom
    # --------------------------------
    def __custom(self, action, scheme, field_value, error):
        field_name = scheme["field"]
        print "CUSTOM", action, error, self.__custom_service
        '''
        if action in scheme.keys():
            # Verifica se a acao existe na instancia do servico
            if hasattr(self.__custom_service, scheme["field"] + "_" + action):
                method = getattr(self.__custom_service, scheme["field"] + "_" + action)
    ...
    def __max_length(self, scheme, value):
        ...
        if (max_length is not None) and (len(value) > max_length):
            ...
            has_error = True
            # Registra mensagem de erro no serviço de log e retona ao cliente
            self.status.alerts("MAX_LENGTH", value, [field, DataTypes.to_str(max_length), DataTypes.to_str(len(value))])
        return has_error
    ...
    def __min_length(self, scheme, value):
        ...
        if (min_length is not None) and (len(value) < min_length):
            ...
            has_error = True
            # Registra mensagem de erro no serviço de log e retona ao cliente
            self.status.alerts("MIN_LENGTH", value, [field, DataTypes.to_str(min_length), DataTypes.to_str(len(value))])
        return has_error
    ...
    def __max_value(self, scheme, value):
        ...
        if value > DataTypes.to_number(max_value):
            field = scheme["field"]
            has_error = True
            # Registra mensagem de erro no serviço de log e retona ao cliente
            self.status.alerts("MAX_VALUE", value, [field, DataTypes.to_str(max_value), DataTypes.to_str(value)])
        return has_error
    ...
    def __min_value(self, scheme, value):
        ...
        if value < DataTypes.to_number(min_value):
            field = scheme["field"]  # onde está a condição   valor < min_value
            has_error = True
            # Registra mensagem de erro no serviço de log e retona ao cliente
            self.status.alerts("MIN_VALUE", value, [field, DataTypes.to_str(min_value), DataTypes.to_str(value)])
        return has_error
