...
def display_message():
    """
    returns a basics.DisplayMessageBuilder, which is used for building a message to display to the user,
    with several options that can be added via method chaining.
    """
    request = basics.DisplayMessageBuilder()
    _add_output_object(request)
    return request
...
def create_option(trigger_rule, display, actions, name=None, description=None):
    ...
    """
    existing-variables = {}
    option = basics.Option(basics.create_preliminary_identifier('option'), name, description, trigger_rule, display, actions, existing-variables)
