...
def _get_valid_identifier_types():
    ...
    """
    res = [a[0] for a in basics.get_shared_truth()['valid_identifier_types']]
    return res
...
def _get_valid_event_request_names():
    ...
    """
    res = [a[0] for a in basics.get_shared_truth()['valid_event_types']]
    return res
...
def create_option(trigger_rule, display, actions, name=None, description=None):
    ...
    """
    existing_arguments = {}
    option = basics.Option(basics.create_preliminary_identifier('option'), name, description, trigger_rule, display, actions, existing_arguments)
...
def make_output_available():
    ...
    """
    # for every object of type TagRequest that has also_create_signal==True, create an additional output_object
    tmp = [a for a in _output_objects if isinstance(a, basics.TagRequest)]
    for a in tmp:
