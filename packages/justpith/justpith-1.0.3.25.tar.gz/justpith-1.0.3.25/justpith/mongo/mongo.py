...
class Mongo(object):
    ...
    def get_list_news_for_recommendations_service(self, list_id_news):
        selected_collection = self.connection['News']
        result = selected_collection.find( { "_id" : { "$in": list_id_news } }, {"_id":1, "category_title":1, "news_source":1,"category_id":1, "news_source_id":1, "timestamp":1} )
        return result

    def get_raccomandations(self, user_id):
        selected_collection = self.connection['Raccomandations_LifeStyle']
