...
class Mongo(object):
    ...
    ##### JUSTPITH SERVER #####
    def save_travel_research(self, data):
        selected_collection = self.connection['TravelSearch']
    ...
    def get_travel_research(self, search_id, page_number, page_size):
        ...
        result = selected_collection.find({"search_id": str(search_id)}).skip(page_size*(page_number)).limit(page_size)
