...
class BaseApi(object):
    ...
    def deal_post(self, postData):
        ...
        um = models.Message()
        datautils.node2model(root, um, ["createTime"])
        d = {}
        if um.msgType != "text":
            d = dict([(k, v) for k, v in datautils.node2dict(root).iteritems() if
                      k not in ['ToUserName', 'FromUserName', 'CreateTime', 'MsgType', 'MsgId']])
            um.content = json.dumps(d)
        um.save()
        return um
    ...
    def response_user(self, um):
        s = u"""<xml>
            ...
            <Content><![CDATA[%s]]></Content>
                         ...
                         um.toUserName,
                         time.mktime(um.createTime.timetuple()),
                         "text",
                         self.auto_reply(um)  #
                         )
        log.info(s)
    ...
    def auto_reply(self, message):
        ...
        res = replys.get(message.msgType)
        if callable(res):
            res = res(message)
        return res or ""

    def OAuthUrl(self, redirect_uri):
        return get_wx_oauth_url(self.appid, redirect_uri,scope="snsapi_userinfo")
