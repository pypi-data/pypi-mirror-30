...
class Task(models.Model):
    ...
    receiver_ids = modelutils.JSONField(u"所有接收者ID", blank=True, null=True)
...
class Message(models.Model):
    ...
    task = models.ForeignKey(Task, verbose_name=Task._meta.verbose_name, related_name="messages",
                             on_delete=models.PROTECT)
    is_read = models.BooleanField(u"是否已读", default=False, blank=True)
    create_time = models.DateTimeField(u"创建时间", auto_now_add=True)
    ...
    def save(self, **kwargs):
        ...
        self.user = self.receiver.as_user()
