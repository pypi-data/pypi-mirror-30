...
from django_szuprefix.utils.datautils import phonemask as phone_mask
...
def phonemask(value):
    return phone_mask(six.text_type(value))
...
class StatObjectNode(Node):
    ...
    def render(self, context):
        c={}
        if self.as_var:
            from util.statutils import StatObject
            c[self.as_var] = StatObject(context[self.obj])
        with context.push(**c):
...
def stat_by_fields(qset,fields):
    from util.statutils import StructorStat
    ss=StructorStat(qset,fields.split(","))
    return ss.stat()
