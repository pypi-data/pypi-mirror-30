...
class X0Functions:
    ...
    def calculate_activation_energy(temperature, reaction_rate, boltzmann_constant=0.0083144621):
        if len(temperature)>4:
            linear_fit, v = np.polyfit(1 / (temperature+273), np.log(reaction_rate), 1, cov=True)
            activation_energy = -linear_fit[0]*boltzmann_constant
            error = np.sqrt(v[0,0])*boltzmann_constant
        else:
            linear_fit = np.polyfit(1 / (temperature+273), np.log(reaction_rate), 1)
            activation_energy = -linear_fit[0]*boltzmann_constant
            error = None
        return activation_energy, error, linear_fit
