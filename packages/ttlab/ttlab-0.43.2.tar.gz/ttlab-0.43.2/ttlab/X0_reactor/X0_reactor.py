...
class X0Reactor:
    ...
    def calculate_activation_energy(self,time_intervals,gas,method='steps'):
        ...
        if len(temperature)>4:
            linear_fit, v = np.polyfit(1 / (temperature+273), np.log(ion_current), 1,cov=True)
            activation_energy = -linear_fit[0]*boltzmann_constant
            error = np.sqrt(v[0,0])*boltzmann_constant
        else:
            linear_fit = np.polyfit(1 / (temperature+273), np.log(ion_current), 1)
            activation_energy = -linear_fit[0]*boltzmann_constant
            error = 0
        return activation_energy, temperature, ion_current, linear_fit, error

    def plot_activation_energy(self,time_intervals,gas,ax=None,method='steps'):
        activation_energy, temperature, ion_current, linear_fit, error  = self.calculate_activation_energy(time_intervals,gas,method)
        boltzmann_constant = 0.0083144621 # kj/(K*mol)
