...
def async_redis_get(redis_cli, key, future, timeout, call_back=None, *args):
    """
    author: 崔俊
    github: https://github.com/joobih/tornado_redis_yield
    version: 1.0
    通过不断地询问redis 中key值是否有了数据，没有就等待0.5秒继续询问;有就调用回调函数并把结果作为参数传递进去
    ...
    try:
        ...
        if not data:
            if timeout <= 0:
                IOLoop.current().call_later(0, lambda: call_back("", *args))
            else:
                IOLoop.current().call_later(0.5, async_redis_get, redis_cli, key, future, timeout-1, call_back, *args)
        else:
            IOLoop.current().call_later(0, lambda: call_back(data, *args))
    except Exception as e:
        raise e


def redis_listen(redis_cli, key, call_back, timeout=50, *args):
    future = Future()
    if call_back is not None:
        call_back = stack_context.wrap(call_back)
        async_redis_get(redis_cli, key, future, timeout, call_back, *args)
    return future
