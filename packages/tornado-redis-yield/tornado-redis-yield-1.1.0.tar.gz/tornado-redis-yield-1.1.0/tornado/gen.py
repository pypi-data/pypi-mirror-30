...
def async_redis_get(redis_cli, key, call_back, future):
    """
    author: 崔俊
    ...
    if not data:
        IOLoop.current().call_later(0.5, async_redis_get, redis_cli, key, call_back, future)
    else:
        IOLoop.current().call_later(0,
                                lambda: call_back(data))


def redis_sleep(redis_cli, key, call_back):
    f = _create_future()
    async_redis_get(redis_cli, key, call_back, f)
    return f
