...
def async_redis_get(redis_cli, key, future, call_back=None, *args):
    """
    author: 崔俊
    ...
    if not data:
        IOLoop.current().call_later(0.5, async_redis_get, redis_cli, key, future, call_back, *args)
    else:
        IOLoop.current().call_later(0,
                                lambda: call_back(data, *args))


def redis_sleep(redis_cli, key, call_back, *args):
    f = Future()
    if call_back is not None:
        call_back = stack_context.wrap(call_back)
        async_redis_get(redis_cli, key, f, call_back, *args)
    return f
