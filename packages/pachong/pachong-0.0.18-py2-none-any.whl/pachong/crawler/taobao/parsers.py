...
from crawler.common.utils import keep_float
from crawler.common.utils import keep_int
...
def parse_shop(fetcher):
    ...
    shop_config = re.compile('window\.shop_config = ({[^;]+});') \
        .search(page_property.text if isinstance(fetcher, Requests) else page_property.get_attribute('text'))
    if not shop_config:
...
def parse_comments(fetcher):
    ...
    if not content:
        raise LookupError
    comments = json.loads(content.strip()[27: -1])
    total = comments.get('total', 0)
    comments = comments.get('comments')
    if not comments:
        raise LookupError('RateLimited.')
