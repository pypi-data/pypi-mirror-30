...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def go_button_clicked(self):
        ...
        for index, page in enumerate(front_side_pages):
            ...
            page_model.mergeScaledTranslatedPage(paired_pages.getPage(page[0]), tx=0, ty=0, scale=0.7)
            page_model.mergeScaledTranslatedPage(paired_pages.getPage(page[1]), tx=size, ty=0, scale=0.7)
        ...
        with open(os.path.join(directory, "(front_side_pages) " + os.path.basename(pdf_file)), "wb") as stream:
            print("Writing files...")
            front_output.write(stream)

        with open(os.path.join(directory, "(back_side_pages) " + os.path.basename(pdf_file)), "wb") as stream:
            print("Writing files...")
