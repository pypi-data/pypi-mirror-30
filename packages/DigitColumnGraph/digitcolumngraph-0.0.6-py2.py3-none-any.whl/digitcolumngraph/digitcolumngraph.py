...
DrawOptions = Enum('DrawOptions', 'ShrinkWrap')
...
class DigitColumnGraph:
    ...
    def _draw_titles(self, options=None):
        result = []
        for key, value in self.items.items():
            ...
            if options == None: #fill title with whitespace and spacing
                while len(result) % (self.col_width() + self.spacing) != 0:
                    if len(result) == self.width():
                        break
                    result.append(' ')

            elif DrawOptions.ShrinkWrap in options:
