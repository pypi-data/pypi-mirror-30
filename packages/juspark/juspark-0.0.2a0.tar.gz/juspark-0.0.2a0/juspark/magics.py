...
class PySparkMagics(Magics):
    ...
    def juspark(self, line, cell=""):
        ...
        if args.profile:
            profile_dirs = [os.path.join(os.environ['HOME'],".ipyspark", "profiles"), '/opt/ipyspark/profiles']
            OUT.debug("Looking for  profile: %s in %s" % (args.profile, profile_dirs))
            profile_files = filter(os.path.isfile, map(lambda pd: os.path.join(pd,args.profile), profile_dirs))
            if not profile_files:
                OUT.fail("Profile file does not exists for: %s in %s" % (args.profile, profile_dirs))
            else:
                profile_file = profile_files[0]
            #this is somewhat tricky but it should work
            with open(profile_file,'r') as profile_f:
                args = parser.parse_args(filter(lambda s:len(s)>0, shlex.split(profile_f.read()) + shlex.split(line)))
                OUT.debug("Params merged with profile: %s" % vars(args))
        options = json.loads(cell) if cell else dict() 
