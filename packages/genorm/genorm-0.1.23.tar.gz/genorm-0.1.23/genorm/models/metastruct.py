    ...
    def get_findable_attributes(self):
        ...
        for k in keys:
            if k.startswith('__') and k.endswith('__'):
                continue
            c = type.__getattribute__(self.cls, k)
            if not isinstance(c, (ColumnAttributeAbstract, ForeignAttr)):
                continue
            if isinstance(c, ColumnAttributeAbstract) and c.findable:
                attrs.append(c)
            elif isinstance(c, ForeignAttr) and c.findable is True:
                attrs.append(c)
        return attrs
