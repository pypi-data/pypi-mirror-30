...
class OrmRequest(object):
    def __init__(self, request_type, *args, **kwargs):
        ...
        self.foreign_attrs = dict()
        self._autojoins = dict()
        self._related_objects = OrderedDict()
        self._related_obj_attrs = list()
        self._for_update = kwargs.pop('for_update', False)
        self._lock_in_share_mode = kwargs.pop('lock_in_share_mode', False)
    ...
    def _resolve_attr(self, item, allow_alias=False, table_name=None):
        if isinstance(item, ForeignAttr) and item in self.foreign_attrs:
            item = self.foreign_attrs[item]
        if item is None:
            return "NULL"
    ...
    def resolve_expr(self, expr, allow_alias=False, table_name=None):
        def _true(expr_):
            if isinstance(expr_, (ColumnAttributeAbstract, ForeignAttr)):
                return expr_.true_
            return expr_
        ...
        if not isinstance(expr, (list, tuple)):
            expr = ['AND', expr, ]
        if isinstance(expr[0], str) and expr[0] not in ('EXISTS', '!EXISTS', 'AND', 'OR', 'NOT'):
        ...
        if expr[0] == 'EXISTS':
            ...
            if len(expr) != 2:
                raise OrmRequestError("Incorrect filter with NOT modifier!")
            r_ = self.resolve_expr(expr[1], table_name=table_name)
            if not r_:
                return ""
            return "NOT(" + r_ + ")"
        ...
        for f_ in fc_:
            ...
            if isinstance(f_, OrmComparator):
                item_ = f_.item
                if isinstance(item_, ForeignAttr):
                    if item_ not in self.foreign_attrs:
                        raise OrmRequestError(
                            "foreign attribute (ForeignAttr) is used in the request condition, but has not been"
                            " resolved (possible it's related column does not loaded because 'only_own' or"
                            " 'always_defer' is set)."
                        )
                    item_ = self.foreign_attrs[item_]
                ...
                if comparator_ == 'ISNULL':
                    ...
                    continue
                right_ = f_.value
                if isinstance(right_, ForeignAttr):
                    if right_ not in self.foreign_attrs:
                        raise OrmRequestError(
                            "foreign attribute (ForeignAttr) is used in the request condition, but has not been"
                            " resolved (possible it's related column does not loaded because 'only_own' or"
                            " 'always_defer' is set)."
                        )
                    right_ = self.foreign_attrs[right_]
                if isinstance(right_, ColumnAttributeAbstract):
                    right_ = right_.make_filter(self)
                ...
                if rf_:
                    rf_ = ("(" + rf_ + ")") if isinstance(f_, (list, tuple)) and len(f_) > 1 else rf_
                    r_.append(rf_)
        if not r_:
            return ""
    ...
    def _handle_related_attrs(self):
        ...
        if self.only_own:
            return
        foreign_attrs = resulting_model.__meta__.get_foreign_attrs()
        foreign_attrs_related2one = set()
        for c in foreign_attrs:
            foreign_attrs_related2one.add(c.related_k)
        ...
        for c in attrs:
            ...
            for fc in attrs:
                ...
                if c.model_k in foreign_attrs_related2one:
                    fa_ = None
                    for fa in foreign_attrs:
                        if fc.model_k == fa.referenced_k:
                            fa_ = fa
                            break
                    if fa_ is not None:
                        self.foreign_attrs[fa_] = fc_

    def _finish(self, for_union=False):
        if self._is_finished:
            raise OrmRequestError("Trying to modify request which is already finished and SQL query already prepared!")
    ...
    def _sql_select(self, for_union=False, page=None, per_page=None, limit=None):
        ...
        if q_limit:
            p.append("LIMIT")
            p.append(q_limit)
        if self._for_update and not for_union:
            p.append("FOR UPDATE")
        elif self._lock_in_share_mode and not for_union:
...
class OrmSearchRequest(OrmRequest):
    def __init__(self, model, what, *args, **kwargs):
        ...
        if not isinstance(what, str):
            raise OrmParameterError("'find' method requires a search phrase of type (str)!")
