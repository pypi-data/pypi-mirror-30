...
class AttributeValueDate(AttributeValueAbstract, datetime.date):
    ...
    def str_value(self):
        val = self.json_value
        return '"%s"' % val if val is not None else "null"
...
class AttributeValueTime(AttributeValueAbstract, datetime.time):
    ...
    def json_value(self):
        ...
        if not isinstance(val, (datetime.datetime, datetime.time)):
            return "null"
        return "%02d:%02d:%02d" % (val.hour, val.minute, val.second)
...
class AttributeValueDateTime(AttributeValueAbstract, datetime.datetime):
    ...
    def json_value(self):
        ...
        if not isinstance(val, datetime.datetime):
            return "null"
        return "%04d-%02d-%02d %02d:%02d:%02d" % (val.year, val.month, val.day, val.hour, val.minute, val.second)
...
class ValueArrayAbstract(AttributeValueVirtual):
    ...
    def _assign_with_instance(self, obj):
        ...
        obj.__parent__ = self.instance, obj.ibfk_parent
