...
class OrmSessionAbstract(object):
    ...
    def begin(self):  # Abstract
        pass

    def rollback(self):  # Abstract
        pass

    def commit(self):  # Abstract
        pass

    def _lock_tables(self, lock_type, *table_names):  # Abstract
        pass

    def lock_tables(self, lock_type, *table_names):
        """ Locks table in the database for read or for write. """
        if not isinstance(lock_type, str):
            raise OrmParameterError("lock_tables() requires 'lock_type' to be type (str)!")
        lock_type = lock_type.upper()
        if lock_type not in ('READ', 'WRITE'):
            raise OrmParameterError("lock_tables() 'lock_type' must be one of 'READ' or 'WRITE'!")
        self._lock_tables(lock_type, *table_names)

    def unlock_tables(self):  # Abstract
        pass
    ...
    def _execute(self, request, parameters):
...
class MysqlSession(OrmSessionAbstract):
    ...
    def _execute(self, request, parameters):
        ...
        try:
            ...
            if e.args[0] == 1062 and "duplicate entry" in e.args[1].lower():
                raise OrmUniquenessError()
            raise
    ...
    def _lock_tables(self, lock_type, *table_names):
        self._execute("LOCK TABLE %s" % (",".join(["%s %s" % (t, lock_type) for t in table_names])), None)

    def unlock_tables(self):
        self._execute("UNLOCK TABLES", None)
    ...
    def set_time_zone(self, time_zone):
        ...
        if self.time_zone is None or isinstance(self.time_zone, AttributeValueNull):
            ...
            tz = ("+%i:00" % int(tzoffset)) if tzoffset >= 0 else ("%i:00" % int(tzoffset))
...
class PostgresqlSession(OrmSessionAbstract):
    ...
    def begin(self):
        """ Not implemented in the 'psycopg2'. """
        self._check_opened()
        # self.cn.begin()
    ...
    def _lock_tables(self, lock_type, *table_names):
        ...
        self._execute("LOCK %s" % (",".join(["%s IN %s MODE" % (t, lock_type) for t in table_names])), None)

    def unlock_tables(self):
        """ PostgreSQL does not supports UNLOCKING tables until transaction been commited or rolled back. """
        pass
