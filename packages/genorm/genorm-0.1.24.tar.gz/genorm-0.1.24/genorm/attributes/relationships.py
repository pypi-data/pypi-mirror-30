...
class ForeignAttr(OrmComparatorMixin, DataAttributeAbstract):
    def __init__(self, related_attr, referenced_k=None, **kwargs):
        ...
        self._findable = kwargs.pop('findable', None)
        self._strict_find = kwargs.pop('strict_find', None)
    ...
    @property
    def related(self):
        related_k = self.related_k
        related_ = getattr(self.model, related_k)
        related_.validate()
        return related_

    @property
    def foreign_model(self):
        return self.related.foreign_model
    ...
    @property
    def findable(self):
        if self._findable is not None:
            return bool(self._findable)
        return getattr(getattr(self.foreign_model, self.referenced_k), 'findable', False)
    ...
    @property
    def strict_find(self):
        if self._strict_find is not None:
            return bool(self._strict_find)
        return getattr(getattr(self.foreign_model, self.referenced_k), 'strict_find', False)

    @strict_find.setter
    def strict_find(self, value):
        self._strict_find = value
