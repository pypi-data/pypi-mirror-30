...
class DataAttributeAbstract(AttributeAbstract):
    ...
    def __init__(self, **kwargs):
        ...
        for k in kwargs:
            if k in ('default', 'journaling', 'required', 'nullable', 'l10n', 'no_new', 'on_change', 'on_text_repr',
                     'on_log_repr', 'setter', 'getter', 'caption', 'abbreviation', 'options'):
                continue
    ...
    def text_repr(self, value, instance=None):
        ...
        return "" if value is None or isinstance(value, AttributeValueNull) else self._l10n_text(value)
...
class KeyAttributeAbstract(AttributeAbstract):
    def __init__(self, *args):
        if not args:
            raise OrmParameterError("No attributes are specified when declaring KEY '%s'!" % self.model_k)
    ...
    def validate(self):
        ...
        for arg in self._args:
            if not isinstance(arg, (str, ColumnAttributeAbstract)):
                raise OrmParameterError(
                    "When declaring KEYs: attributes must be set using textual names (str) or attributes themselfs."
                )
