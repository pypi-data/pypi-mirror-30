...
def yichuxing_backup1(df_point, df_ycx, lng, lat, meter=500):
    PF = []
    for lng1, lat1 in tqdm(df_point.loc[:, [lng, lat]].values):
        peo_flow = 0 # 人流指数
...
def main(pointFile, ycxFile, lnglat, meter, core_num):
    ...
    funcList = []
    for df in df_split:
        f = p.apply_async(run_task, args=(df,df_ycx, lng, lat, meter,))
        funcList.append(f)
