...
from cpyf.mGlobalConst import CMD0_ECHO_CMD, P2PKIND_P2PLAYOUT, CHAR_SEP_P2PLAYOUT
...
class TCmdStringSckP2PLayout(TCmdStringSck):
    def __init__(self, sHubId, sHostAndPort, sPairId, sSuffix, sAcctPwd,
        ...
        TCmdStringSck.__init__(self, sHubId, P2PKIND_P2PLAYOUT, sHostAndPort,
                               sAcctId, sAcctPwd, 'Y',
                               sClientInfo, bVerbosePrintCmdStr)
        self.iOneSecondClock = 0
        self.sSuffixDoneList = ''
    ...
    def DoTraverseCount(self, sSuffixFm, sSuffixTo, sPeerCmd, CmdStr):
        ...
        if len(lsLeft) <= 0:
            ...
            tupleParam = (CMD3_P2PLAYOUT_TRAVERSE_COUNT, str(iCountValueNow), ','.join(lsSuffixDone),)
            self.SendRequestP2PLayoutCmd(sSuffixTarget, tupleParam, 'sLogicParam')
    ...
    def LaunchTraverseCount(self):
        ...
        if len(lsLeft) <= 0:
            ...
            sSuffixTarget = lsLeft[0]
            iCountValueNow = 1
            tupleParam = (CMD3_P2PLAYOUT_TRAVERSE_COUNT, str(iCountValueNow), self.sSuffix,)
            self.SendRequestP2PLayoutCmd(sSuffixTarget, tupleParam, 'sLogicParam')
