...
"""
# import ._addpath
from . import _addpath  # WeiYF.20180115 在Python3下这种写法是对的，在Python2下也可以
import sys

from weberFuncs import GetCurrentTime, PrintTimeMsg, md5, CAppendLogBase
from cpyf.CSockReadWrite import CSockReadWrite
from cpyf.cstpFuncs import IsHeartBeat, IsCmdNotify, GetCmdType, \
    GetCmdRequestFmReply, GetCmdReplyFmRequest  # CMD0_CHECK_AUTH

from cpyf.mGlobalConst import CMD0_CHECK_AUTH, CMD0_KICK_OFFLINE
from cpyf.cstpErrorFuncs import CSTPError, GenErrorTuple

import gevent
from gevent.queue import Queue
from gevent.server import StreamServer
from gevent import monkey
monkey.patch_all()  # WeiYF.20160322 打上补丁
...
class TCmdStringHub(CAppendLogBase):
    def __init__(self, sHostName4Param, sServerIPPort, oHubCallback,
        ...
        self.greenletRequest = None
        self.greenletReply = None
            ...
            if sClientIPPort == '':  # 表示没有数据
                ...
                if sClientIPPort == '*':  # 广播给所有链接
                    ...
                    if v:
                        ...
                        sMsg = 'sClientIPPort=%s,iCmdId=%s,CmdOStr=%s' % (
                            sClientIPPort, dwCmdId, str(CmdOStr))
                ...
                if sRet == 'OK':
                    ...
                    if IsHeartBeat(dwCmdId):
                        ...
                        if not self.DealSpecialCmdRequest(oObj, dwCmdId, CmdStr):
                            if self.queueRequest.qsize() > self.iMaxQueueReqNum:
                                # WeiYF.20160324 进行流控，消息积压则暂缓
    ...
    def DealSpecialCmdRequest(self,oObj,dwCmdId,CmdIStr):
        ...
        if CmdIStr[0] == CMD0_CHECK_AUTH:  # .startswith('ABLOGIN.ChkPasswd'):
            if oObj.cLoginStatus != 'L':
                ...
                (sClientIPPort,dwCmdId,CmdOStr) = self.oHubCallback.HandleCheckAuth(
...
def mainCmdStringHub():
    ...
    from hub.demoSettingsP2PLayout import gDictP2PLayoutByPairId
    sHostName4Param = 'LocalTest'
    sServerIPPort = '0.0.0.0:8888'
    # sPythonDir = 'D:/WeiYFGitSrc/PythonProject/WxScanServ/ptSNetSck/'
    sPythonDir = 'D:/WeberWork/ArchitectRoot/WeiYF/GitRoot/BitBucket/WeiYFSrcRoot/wxscanservptSNetSck'
