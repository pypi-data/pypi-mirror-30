...
from weberFuncs import ClassForAttachAttr, WyfAppendToFile, GetSystemPlatform
from cpyf.cstpFuncs import CMDID_HREAT_BEAT, IsCmdNotify, GetCmdReplyFmRequest
from cpyf.mGlobalConst import P2PKIND_ACCTSHARE
...
class CHubCallbackBasicBase:
    ...
    def __init__(self, sHubId):
        ...
        if GetSystemPlatform() == 'Windows':
            ...
            # WeiYF.20171205 为避免Win7在D:根目录创建文件的权限限制
            # 这里直接采用当前目录的相对路径
            self.sFNameLogClient = r'..\log\%s' % sFNLogClient
    ...
    def SetCallBackQuitFlag(self, sHint):
        ...
        PrintTimeMsg("SetCallBackQuitFlag(%s)..." % sHint)
    ...
    def GetHubAttrValue(self, sAttrName, sDefValue=''):
        ...
        if self.oBind is None: return sDefValue
        if not hasattr(self.oBind, 'oHub'):
            return sDefValue
    ...
    def HandleCheckAuth(self, sClientIPPort, dwCmdId, CmdIStr):
        ...
        if sP2PKind != P2PKIND_ACCTSHARE and sHubId != self.sHubId:
            ...
            oLink = self.dictObjLinkByCIP.get(sClientIPPort,None)
            if oLink:
                CmdOStr = self.DoHandleCheckAuth(oLink, dwCmdId, sHubId, sP2PKind,
    ...
    def DoHandleCheckAuth(self, oLink, dwCmdId, sHubId, sP2PKind,
                          sAcctId, sAcctPwd, ynForceLogin, sClientInfo):
