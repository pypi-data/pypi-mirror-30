...
#####
# WeiYF.20180409 处理python3兼容问题
(Python36-32) λ python
Python 3.6.4 (v3.6.4:d48eceb, Dec 19 2017, 06:04:45) [MSC v.1900 32 bit (Intel)] on win32
Type "help", "copyright", "credits" or "license" for more information.
>>> b = b'800000010000007C'
>>> b
b'800000010000007C'
>>> list(b)
[56, 48, 48, 48, 48, 48, 48, 49, 48, 48, 48, 48, 48, 48, 55, 67]
>>> b''.join(list(b))
...
class CSockReadWrite:
            ...
            try:
                try:
                    ...
                    if IsPython3:
                        ...
                        # datBuf = datBuf.decode('utf-8')  # to string
                        PrintTimeMsg('shouldReadLenData(%s)=%s=' % (iDataLen, datBuf))
                    else:
                        datBuf = sock.recv(iDataLen-len(listOut))
    ...
    def ReadCmdStrFromLink(self, iSecondsTimeOut):
        ...
        if IsPython3:
            # (dwCmdId, dwDataLen) = SerialCstpHeadFmString(''.join(listHead.decode('utf-8')))
            (dwCmdId, dwDataLen) = SerialCstpHeadFmString(bytes(listHead))
        else:
        ...
        if IsPython3:
            # CmdStr = SerialCmdStrFmString(''.join(listData.decode('utf-8')))
            CmdStr = SerialCmdStrFmString(bytes(listData))
        else:
