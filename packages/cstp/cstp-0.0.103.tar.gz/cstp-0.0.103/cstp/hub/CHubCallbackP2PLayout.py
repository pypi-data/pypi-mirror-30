...
from cpyf.mGlobalConst import P2PKIND_P2PLAYOUT, CHAR_SEP_P2PLAYOUT
from cpyf.cstpFuncs import GetCmdReplyFmRequest, CMDID_NOTIFY_MSG
from cpyf.cstpErrorFuncs import CSTPError, GenErrorTuple, GenOkMsgTuple
from hub.CHubCallbackQueueBase import CHubCallbackQueueBase

from cpyf.mGlobalConst import CMD0_KICK_OFFLINE
from cpyf.mP2PLayoutConst import CMD0_P2PLAYOUT_SEND_CMD_TOPEER, CMD0_P2PLAYOUT_SEND_SYSTEM_MSG
...
class CHubCallbackP2PLayout(CHubCallbackQueueBase):
    ...
    def SendP2PMsgToPeerByPairId(self, sPairId, sSuffixTo, CmdIStr):
        ...
        if not sClientIPPortTo:
            return GenErrorTuple(CSTPError.P2P_SEND_MSG_CIP_NOT_FOUND, sWhereMsg,
    ...
    def DoHandleCheckAuthLayout(self, oLink, dwCmdId, sHubId,
        ...
        if cSep == '':
            ...
            if CmdOStr[0] == 'OK':  # bPass:
                if self.bVerbosePrintCmdStr:
                    PrintTimeMsg('DoHandleCheckAuthP1.*******.dictCIPByPeerId=%s=' %
    ...
    def DoHandleCheckAuth(self, oLink, dwCmdId, sHubId,
        ...
        if sP2PKind == P2PKIND_P2PLAYOUT:  # .startswith('P1'):
            CmdOStr = self.DoHandleCheckAuthLayout(
                oLink, dwCmdId, sHubId, sP2PKind, sAcctId, sAcctPwd, ynForceLogin, sClientInfo)
        else:
            CmdOStr = CHubCallbackQueueBase.DoHandleCheckAuth(
                self, oLink, dwCmdId, sHubId, sP2PKind, sAcctId, sAcctPwd, ynForceLogin, sClientInfo)
        return CmdOStr
    ...
    def HandleClientEnd(self, sClientIPPort):
        ...
        if sPairId != '' and sSuffix != '':
            ...
            PrintTimeMsg('CHubCallbackP2PLayout.HandleClientEnd.sPairId=(%s),sSuffix=(%s)Nodo!' % (
                sPairId, sSuffix))
        sPeerId = self.GetLinkAttrValue(sClientIPPort,'sPeerId', '')
        if sPeerId:
            if not bKickOff:
                del self.dictCIPByPeerId[sPeerId]
            PrintTimeMsg('CHubCallbackP2PLayout.HandleClientEnd.sPeerId=(%s),del(%s)!' % (
                sPeerId, sClientIPPort))
        else:
    ...
    def HandleRequestCmd(self, sClientIPPort, dwCmdId, CmdIStr):
        ...
        if not bDone:
            if CmdIStr[0] == CMD0_P2PLAYOUT_SEND_CMD_TOPEER:
                ...
                if CmdOStr[0] == 'OK':
                    lsRet = list(CmdOStr)
                    lsRet[3] = '%s(%s)To(%s)!' % (CMD0_P2PLAYOUT_SEND_CMD_TOPEER,
                                                  sClientIPPort, CmdIStr[2])
                    CmdOStr = tuple(lsRet)
