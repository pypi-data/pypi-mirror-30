...
from weberFuncs import IsPython3
from weberFuncs import SerialCmdStrToByteArray, SerialCmdStrFmByteArray, PrintCmdStrArray
...
def HexToInt8(sHex):
    """ 将8字节16进制串转为双字无符号整数 """
    try:
        if IsPython3() and type(sHex)!=str:
            return int(str(sHex, encoding='utf8'), 16)
        return int(sHex, 16)
    except Exception as e:
        PrintTimeMsg('HexToInt8(%s).Error=%s=' % (sHex, str(e)))
        raise
...
def tryCmdId():
    PrintTimeMsg( Int8ToHex(0x00008000) )
    PrintTimeMsg( Int8ToHex(0x80000000) )
    PrintTimeMsg( Int8ToHex(0x8FFFFFFF) )
    PrintTimeMsg( Int8ToHex(0xFFFFFFFF) )
    PrintTimeMsg( Int8ToHex(HexToInt8('8FFFFFFF')) )
    PrintTimeMsg( '-'*8 )
    PrintTimeMsg( IsHeartBeat(0) )
    PrintTimeMsg( IsHeartBeat(0x80000000) )
    PrintTimeMsg( IsCmdNotify(0) )
    PrintTimeMsg( '-'*8 )
    PrintTimeMsg( IsCmdRequest(0) )
    PrintTimeMsg( IsCmdRequest(1) )
    PrintTimeMsg( IsCmdRequest(0x80000000) )
    PrintTimeMsg( IsCmdRequest(0x08000000) )
    PrintTimeMsg( '-'*8 )
    PrintTimeMsg( IsCmdReply(0) )
    PrintTimeMsg( IsCmdReply(0x80000000) )
    PrintTimeMsg( IsCmdReply(0x80000001) )
    PrintTimeMsg( IsCmdReply(0x08000000) )
    PrintTimeMsg( '-'*8 )
    PrintTimeMsg( Int8ToHex(GetCmdReplyFmRequest(1234)) )
    PrintTimeMsg( Int8ToHex(GetCmdReplyFmRequest(0x80000001)) )
    PrintTimeMsg( '-'*8 )
    PrintTimeMsg( Int8ToHex(GetCmdRequestFmReply(1234)) )
    PrintTimeMsg( Int8ToHex(GetCmdRequestFmReply(0x80000123)) )
    PrintTimeMsg( '-'*8 )
    PrintTimeMsg( Int8ToHex(GenNewReqCmdId()) )
    PrintTimeMsg( Int8ToHex(GenNewReqCmdId()) )
    PrintTimeMsg( Int8ToHex(GenNewReqCmdId()) )
...
def SerialCmdStrToString(CmdStr):
    # 序列化CmdStr到字符串
    return SerialCmdStrToByteArray(CmdStr)
...
def SerialCmdStrFmString(sData):
    # 从字符串反序列化到CmdStr
    return SerialCmdStrFmByteArray(sData)
    # lsCmdStr = []
...
def PrintTestSerialCmdStr(lsCmdStr):
    ...
    PrintCmdStrArray(CmdStr, "SerialCmdStrFmString")
...
def trySerialCstCmdStr():
    if IsPython3():
        sGBK = u"国标汉字".encode('GBK')
    else:
    ...
    PrintTimeMsg('%s,%s' % (Int8ToHex(dwCmdId),CmdStr))
...
def tryBytes():
    s = '1234测试test'
    PrintTimeMsg('%s,%s' % (s,len(s)))

    if IsPython3():
        b = s.encode('utf-8')
    else:
        b = bytes(s)
    PrintTimeMsg('%s,%s' % (b,len(b)))
