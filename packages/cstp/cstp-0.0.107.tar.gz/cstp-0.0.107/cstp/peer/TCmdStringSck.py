...
from weberFuncs import PrintTimeMsg, PrintAndSleep, printHexString, \
    printCmdString, GetCurrentTime  # , GetTimeInteger, md5

from cpyf.CssException import CssException, gef
from cpyf.CGlobalExitFlag import CGlobalExitFlag
from cpyf.CQueueThread import CQueueObject, CQueueThread, CStartLoopThread

from cpyf.mGlobalConst import CMD0_CHECK_AUTH, CMD0_ECHO_CMD
from cpyf.cstpFuncs import CMDID_HREAT_BEAT, CMDID_NOTIFY_MSG, GenNewReqCmdId,\
    IsHeartBeat, IsCmdNotify, GetCmdType, GetCmdRequestFmReply, GetCmdReplyFmRequest

from cpyf.mP2PLayoutConst import CMD0_P2PLAYOUT_SEND_SYSTEM_MSG, CMD0_P2PLAYOUT_SEND_CMD_TOPEER
from cpyf.CSockReadWrite import CSockReadWrite
...
class TCmdStringSck:
    ...
    def StartMainThreadLoop(self):
        ...
        try:  # 以下是主线程；python 没有连接上的事件？那就直接调用
            iTryCnt = 0
            while not self.waitCheckIsConnected(1) and iTryCnt < 3:  # 最多尝试3s
                PrintAndSleep(1, 'StartMainThreadLoop.WaitForChkPasswd')
                iTryCnt += 1
    ...
    def WaitForMainThreadQuit(self, iTimeoutSeconds):
        ...
        def cbWaitForAsynQuit(sHint, iWaitCount):
            ...
            return []
        return self.gef.WaitAndCheck('WaitForMainThreadQuit',
                                     cbWaitForAsynQuit, iTimeoutSeconds)
    ...
    def cbReadFmServerToQueue(self, iLoopCnt):  # called by StartOneThread.run()
        ...
        try:
            ...
            if sRet == 'OK':
                ...
                # sErrMsg = oTuple
                self.TerminateMainThread("cbReadFmSocketToQueue.sErrMsg=(%s,%s)" % (sRet, oTuple))  # 会退出
        except Exception as e:
    ...
    def SendReqCmd_ChkPasswd(self):
        # 发送登录请求命令
        CmdStr = (CMD0_CHECK_AUTH, self.sP2PKind, self.sHubId,
                  self.sAcctId, self.sAcctPwd, self.ynForceLogin, self.sClientInfo)
    ...
    def waitCheckIsConnected(self, iTimeoutSeconds):
        ...
        return (self.sck is not None) and (len(lsRet) > 0) and (lsRet[0][0] == 'O')
