...
def scrapeImages( search_queries='nicolas cage', limit=5, folder_name='<query-string>', output_directory='downloads', delay=0 ):

  search_queries = [str(item) for item in search_queries.split(',')]
  ...
  for i in range(len(search_queries)):
    ...
    ''' SCRAPE URLS '''
    print("\n Query no.: " + str(i + 1) + " -->" + '"' + str(search_query) + '"')
    print("Scraping...")
    ...
    dir_name = search_query if folder_name == '<query-string>' else folder_name

    # make a new directory for this search_query
    create_sub_directory(dir_name, output_directory)

    errorCount += download_images( imgURLs, output_directory + "/" + dir_name, delay=delay )
