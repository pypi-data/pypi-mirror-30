...
def build(branch, installPath, node=False):
    ...
    with lcd(installPath):
        ...
        if node in TRUTH_VALUES:
            local('if [[ -d "config" ]]; then rsync -rva config dist/config; fi;')
            local('if [[ -d "public" ]]; then rsync -rva public dist/public; fi;')
            local('if [[ -d "node_modules" ]]; then rsync -rva node_modules dist/node_modules; fi;')
