...
def build(branch, installPath, node=False):
    ...
    with lcd(installPath):
        ...
        if node in TRUTH_VALUES:
            local('if [[ -d "config" ]]; then rsync -ra --stats config dist/config; fi;')
            local('if [[ -d "public" ]]; then rsync -ra --stats public dist/public; fi;')
            local('if [[ -d "node_modules" ]]; then rsync -ra --stats node_modules dist/node_modules; fi;')
