...
def xconfig(path: str, default_value=None):
    ...
    if __config__ is None:
        ...
        cfg = client.get(data_id, "DEFAULT_GROUP")
        xassert(cfg)
        cfg = yaml.load(cfg)
        xassert(cfg)
        __config__ = cfg
    ...
    if env is None:
        path = ''.join('/', lst)
        return dpath.util.get(base, '/base/%s' % path, default_value)
    else:
        path = ''.join('/', lst)
        value1 = dpath.util.get(base, '/base/%s' % path, default_value)
        value2 = dpath.util.get(base, '/%s/%s' % (env, path), None)
