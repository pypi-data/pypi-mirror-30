...
DEFAULT_DNS_NAMES = [DNSName(address)
                     for address in get_my_addresses()
                     if not address.endswith(u'in-addr.arpa') and u'.' in address]
DEFAULT_EXTENSIONS = [
...
class _SSLConstant(object):
    ...
    pem = attr.ib(default=u'pem', init=False)
    der = attr.ib(default=u'crt', init=False)
