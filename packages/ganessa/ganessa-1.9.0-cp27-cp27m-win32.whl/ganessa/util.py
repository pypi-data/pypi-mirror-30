...
#****f* functions/ascii, unistr, str2uni, utf2uni, con2uni
# SYNTAX
...
#   * bret = is_text(item)
# ARGUMENT
#   * unicode or string input_str: string value to be converted to ascii or unicode
#   * item: any object
# RESULT
#   * unicode or string atxt: string of same type as input_str where non-ascii charcacters
#     have been replaced with the best match (i.e. 'e' is replaced with 'e')
#   * unicode utxt: unicode string decoded from input_str (multiple codecs tried in turn);
#     unistr tries cp1252 then utf-8; str2uni tries utf-8 then cp1252;
#     con2uni tries cp850 then cp1252 then utf-8.
...
if PY3:
    def unistr(input_str):
        if isinstance(input_str, str):
            ...
            for codec in ('cp1252', 'iso-8859-1', 'utf-8', 'cp850'):
                try:
    ...
    def str2uni(input_str):
        if isinstance(input_str, str):
            ...
            for codec in ('utf-8', 'cp1252', 'iso-8859-1', 'cp850'):
                try:
                    return str(input_str, codec)
                except UnicodeError:
                    continue
    ...
    def con2uni(input_str):
        if isinstance(input_str, str):
            ...
            try:
                return str(input_str, 'cp850')
            except UnicodeError:
                return unistr(input_str)
        else:
            return str(input_str)
    ...
    def unistr(input_str):
        if isinstance(input_str, unicode):
            return input_str
        elif isinstance(input_str, basestring):
            for codec in ('cp1252', 'iso-8859-1', 'utf-8', 'cp850'):
                try:
                    return unicode(input_str, codec)
                except UnicodeError:
                    continue
        else:
    ...
    def con2uni(input_str):
        if isinstance(input_str, unicode):
            ...
            try:
                return unicode(input_str, 'cp850')
            except UnicodeError:
                return unistr(input_str)
        else:
            return unicode(input_str)
...
# tostr converts bytes to unicode: default cp1252 with py2, utf8 with py3
tostr = str2uni if PY3 else unistr
...
#****f* functions/codewinfile, winstr, utf
# SYNTAX
...
def winstr(input_str):
    ...
    return unistr(input_str).encode('cp1252')  # Windows-1252
codewinfile = winstr

def utf(input_str):
    # return bytes(input_str, 'utf-8') if PY3 else unistr(input_str).encode('utf-8')
    return unistr(input_str).encode('utf-8')
...
def gbool(val):
    if isinstance(val, basestring):
        val = val.lower()
        if val in ("true", "oui", "yes", "o", "ok", "y", "1"):
            return True
        elif val in ("false", "non", "no", "n", "0"):
            return False
    return bool(val)
...
def update_package(package_name, minvers= '', pypi_name=None, deps= False, https_proxy= None):
    ...
    cmd = [OP.join(pyroot, 'Scripts', 'pip'), 'install', '--default-timeout', '2', pypi_name]
    if not deps:
        cmd[4:4] = ['--no-deps']
    if vers:
        if not minvers:
