...
from ScopusWp.register import PublicationRegister
from ScopusWp.wordpress.main import Wordpress
...
class TopController:
    ...
    def __init__(self):
        ...
        self.register = PublicationRegister()

    def add_source(self, name, data_source_class):
        self.source_classes[name] = data_source_class
        try:
            self.sources[name] = data_source_class(self)
        except Exception as e:
            print(e)
            print(type(e))

    def add_sink(self, name, data_sink_class):
        self.sink_classes[name] = data_sink_class
        try:
            self.sinks[name] = data_sink_class(self)
        except Exception as e:
            print(e)
            print(type(e))

    def run(self):
        # Running all the data sources
        general_publications = []
        for name, data_source in self.sources.items():

            for general_publication in data_source.fetch():
                try:
                    general_publications.append(general_publication)
                    if not self.register.contains_publication(general_publication):
                        self.register.insert(general_publication)

                except Exception as e:
                    self.register.session.rollback()
                    print(e)
                    print(type(e))
                    raise e

        for name, data_sink in self.sinks.items():
    ...
    def post_scopus_publication(self, scopus_publication):
        ...
        self.scopus_controller.database_controller.save()
        self.reference_controller.save()
    ...
    def wipe_website(self):
        ...
        self.scopus_controller.database_controller.wipe()
    ...
    def select_all_scopus_cache(self):
        return self.scopus_controller.select_all_publications_database()


class Dummy:

    def __init__(self, general_publication):
        self.content = general_publication
...
def print_l(line):
    if ' scopus ' in line and ('[Top]' in line or '[Progress]' in line):
        print(line[:-1])
...
if __name__ == '__main__':
    ...
    l = LogFileObserver(controller.logging_controller.log_file_path)
    l.add_callback(print_l)
    l.start()
    controller.register.wipe()
    controller.add_source('scopus', ScopusTopController)
    controller.add_sink('wordpress', Wordpress)
    controller.run()