...
class ScopusTopController(DataSource):
    ...
    def fetch(self):
        ...
        print('TOTAL AMOUNT OF DIRECT PUBLICATIONS')
        ...
        for author_profile in author_profiles:
            ...
            for scopus_id in author_profile.publications:
                ...
                try:
                    ...
                    #black_list.append(scopus_id)
                    print(black_list)
                    continue
                # Deciding whether to actually return it
                if self.observation_controller.supports_publication(scopus_publication):

                    history.append(len(scopus_publication.citations))
                    median = sum(history) / len(history)
                    print('\nCITATION PUBLICATIONS FOR "{}"'.format(scopus_publication.id))
                    print(len(scopus_publication.citations))
                    print('NEW MEDIAN: {} ESTIMATED REMAINING: {}'.format(median, median * len(total)))
