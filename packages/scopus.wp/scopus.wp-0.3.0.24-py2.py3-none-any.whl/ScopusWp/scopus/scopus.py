    ...
    def run(self):
        while True:
            if self.id is not None:
                try:
                    self.active = True
                    self.publication = self.scopus_controller.get_publication(self.id)
                    self.callback(self.publication)
...
class ScopusWorkerPool:
    ...
    def __init__(self, worker_count=4):
        ...
        for i in range(worker_count):
            worker = ScopusWorker(lambda x: self.publications.append(x))
