...
def locked(func):
    ...
    def func_wrapper(*args, **kwargs):
        ...
        try:
            ...
            if release:
                cls.lock.release()
            cls.session = ScopusMySQLDatabase.get_session()
    return func_wrapper
...
class ScopusDatabaseController:
    ...
    def insert_publication(self, publication):
        ...
        if self.session.query(Publication).filter(publication.scopusID == publication.id) is not None:
    ...
    def select_publication(self, scopus_id):
        #self.lock.acquire()
        publication = self.session.query(Publication).filter(Publication.scopusID == int(scopus_id)).first()
        #self.lock.release()
