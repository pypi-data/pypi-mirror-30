...
class Reports:
    ...
    def get_analysis_report(self, team_id, project_id, analysis_id):
        ...
        url = self.endpoint + self.url + 'getAnalysis?team_id=%s&project_id=%s&analysis_id=%s' % (team_id, project_id, analysis_id)
        return requests.get(url, headers={"authorization": self.token}).json()

    def get_raw_analysis_report(self, team_id, project_id, analysis_id, limit=5):
        ...
        url = self.endpoint + self.url + 'getProject?team_id=%s&project_id=%s&analysis_id=%s&limit=%s' % (team_id, project_id, analysis_id, limit)
        return requests.get(url, headers={"authorization": self.token}).json()
