...
def identify_protein_ID_OO(fulltext, simple=False, infile=None, locdir=None, verbose=False):
    """
    Uses Regular Expressions to find Protein IDs in the, and then cross-checks against the PDB list
    of entities.
...
def decompose_matches(matches, encode=True):
    """
    ...
    for match in matches:
        if hasattr(match, "position") and hasattr(match, "threeletter"):
            ...
            warnings.warn("Match is missing either position or threeletter")
    return newmatches
