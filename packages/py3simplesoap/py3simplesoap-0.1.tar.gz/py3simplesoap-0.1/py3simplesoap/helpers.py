...
def fetch(
    ...
    if cache and os.path.exists(filename) and not force_download:
        ...
        if url_scheme == 'file':
            ...
            log.info('XML %s' % str(xml))
            log.info('Cache %s' % str(cache))
        if cache:
            log.info('Writing file %s' % filename)
            if not os.path.isdir(cache):
                log.info('Making cache dir...')
                os.makedirs(cache)
            try:
                f = open(filename, 'w+')
                log.info('file %s' % str(f))
                f.write(str(xml))
                f.close()
            except Exception as e:
                log.warn("BOOM! : " + str(e))
    log.info('XML:  ' + str(xml))
    return xml
...
def make_key(element_name, element_type, namespace):
    ...
    if eltype not in ('element', 'complexType', 'simpleType'):
        raise RuntimeError(
            "Unknown element type %s = %s" % (element_name, eltype)
        )
    ...
    for tag in node:
        ...
        for e in children:
                ...
                for a in e.children():
                    for k, v in a[:]:
                        if k.endswith(":arrayType"):
                            ...
                            if (fn_array is None and type_name != "anyType"
                                    and fn_namespace):
                                # get the complext element:
                                ref_type = "complexType"
                                key = make_key(
                                    type_name, ref_type, fn_namespace
                                )
                                fn_complex = elements.setdefault(
                                    key, Struct(key)
                                )
                                ...
                                fn_array.namespaces[None] = fn_namespace
            ...
            if (e['name'] is not None and not alias) or e['ref']:
                ...
                struct.refers_to = fn
            if (e is not None and e.get_local_name() == 'extension' and
                    e.children()):
...
def postprocess_element(elements, processed):
    ...
    # elements variable contains all eelements and complexTypes defined in
    ...
    if part_name:
        ...
        if len(parts) > 1:
            ...
            for part_name_key in parameter_order:
                ...
                if not part:
                    log.error(
                        'Part %s not found for %s' % (
                            part_name_key, message_name
                        )
                    )
                elif not new_msg:
                    new_msg = part.copy()
    ...
    for element in schema.children() or []:
        if element.get_local_name() in ('import', 'include',):
            ...
            if schema_location is None:
                log.debug(
                    'Schema location not provided for %s!' % schema_namespace
                )
                continue
            ...
            log.debug(
                'Importing schema %s from %s' % (
                    schema_namespace, schema_location
                )
            )
            # Open uri and read xml:
            xml = fetch(
                schema_location, http, cache, force_download, wsdl_basedir
            )

            # recalculate base path for relative schema locations
            path = os.path.normpath(
                os.path.join(wsdl_basedir, schema_location)
            )
        ...
        if element_type in ('element', 'complexType', "simpleType"):
            ...
            log.debug("Parsing Element %s: %s (%s)" % (
                element_type, element_name, element_ns
            ))
    ...
    try:
        ...
        try:
            ...
            if s[-1] == "Z" or (s[-3] == ":" and s[-6] in (' ', '-', '+')):
                ...
                warnings.warn(
                    'removing unsupported "Z" suffix or UTC offset. Install '
                    '`iso8601`, `isodate` or `python-dateutil` package to '
                    'support it', RuntimeWarning
                )
            ...
            if "." in s:
                warnings.warn(
                    'removing unsuppported microseconds', RuntimeWarning
                )
                s = s[:s.index(".")]
            return _strptime(s, fmt)
...
class Struct(dict):
    ...
    def __eq__(self, other):
        return isinstance(other, Struct) and self.key == other.key and \
    ...
    def __repr__(self):
        try:
            ...
            s = "{%s}" % ", ".join(
                ['%s: %s' % (repr(k), unicode(e)) for k, v in self.items()]
            )
            warnings.warn(unicode(e), RuntimeWarning)
