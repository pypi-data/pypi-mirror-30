...
class TextReplacements:
    ...
    default = {
      ...
      '"': {' ': u'« ', '': u'« ', 0: u' »'},
      # Conventions for bold, italic anf highlighting
      '[': '<b>', ']': '</b>',
      '<': '<i>', '>': '</i>',
...
class Text2Html:
    '''Allows to convert chunk of texts into XHTML. Some conventions apply, ie,
       rows of text starting with a dash are converted to bulleted lists.'''
    ...
    def convert(self, s):
        ...
        for row in rows:
            if row.startswith('- '):
                ...
                isLast = (i+1 == total) or not rows[i+1].startswith('- ')
                if isLast:
