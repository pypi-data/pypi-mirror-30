...
The result is Mode and its generic ``Service`` class.

Services
    Programs in Mode are built out of one or more services, where the
    services organizes themselves at runtime into a dependency graph.

    In practice this means services may start other services, that
    when told to stop, does so in the correct order::
