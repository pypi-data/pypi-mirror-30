...
import struct
if TYPE_CHECKING:
    from ...tl.types import TypeInputBotInlineMessageID, TypeInputUser, TypeInputMedia, TypeInputDocument, TypeInputGeoPoint, TypeInputStickerSet, TypeMessageEntity, TypeReplyMarkup, TypeInputChatPhoto, TypeMessagesFilter, TypeInputBotInlineResult, TypeSendMessageAction, TypeInputStickeredMedia, TypeInputEncryptedChat, TypeBool, TypeInputPeer, TypeInputSingleMedia, TypeInlineBotSwitchPM, TypeInputEncryptedFile, TypeInputMessage, TypeShippingOption
...
class CreateChatRequest(TLObject):
    ...
    async def resolve(self, client, utils):
        self.users = [utils.get_input_user(await client.get_input_entity(_x)) for _x in self.users]
...
class GetPeerDialogsRequest(TLObject):
    ...
    def __init__(self, peers):
        """
        :param List[TypeInputPeer] peers:
    ...
    async def resolve(self, client, utils):
        self.peers = [utils.get_input_peer(await client.get_input_entity(_x)) for _x in self.peers]
...
class GetRecentLocationsRequest(TLObject):
    ...
    def from_reader(cls, reader):
        _peer = reader.tgread_object()
        _limit = reader.read_int()
        return cls(peer=_peer, limit=_limit)
...
class ReorderPinnedDialogsRequest(TLObject):
    ...
    def __init__(self, order, force=None):
        """
        :param List[TypeInputPeer] order:
    ...
    async def resolve(self, client, utils):
        self.order = [utils.get_input_peer(await client.get_input_entity(_x)) for _x in self.order]
...
class SearchRequest(TLObject):
    ...
    async def resolve(self, client, utils):
        self.peer = utils.get_input_peer(await client.get_input_entity(self.peer))
        self.from_id = utils.get_input_user(await client.get_input_entity(self.from_id)) if self.from_id else None
    ...
    def from_reader(cls, reader):
        ...
        _max_id = reader.read_int()
        _min_id = reader.read_int()
        return cls(peer=_peer, q=_q, filter=_filter, min_date=_min_date, max_date=_max_date, offset_id=_offset_id, add_offset=_add_offset, limit=_limit, max_id=_max_id, min_id=_min_id, from_id=_from_id)
