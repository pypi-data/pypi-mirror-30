...
import struct
if TYPE_CHECKING:
    from ...tl.types import TypeBool, TypeChannelBannedRights, TypeInputMessage, TypeChannelAdminLogEventsFilter, TypeChannelParticipantsFilter, TypeInputChannel, TypeInputStickerSet, TypeChannelAdminRights, TypeInputUser, TypeInputChatPhoto
...
class GetAdminLogRequest(TLObject):
    ...
    async def resolve(self, client, utils):
        self.channel = utils.get_input_channel(await client.get_input_entity(self.channel))
        self.admins = [utils.get_input_user(await client.get_input_entity(_x)) for _x in self.admins] if self.admins else None
...
class GetChannelsRequest(TLObject):
    ...
    async def resolve(self, client, utils):
        self.id = [utils.get_input_channel(await client.get_input_entity(_x)) for _x in self.id]
