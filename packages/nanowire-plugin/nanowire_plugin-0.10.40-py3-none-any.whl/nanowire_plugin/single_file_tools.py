...
def bind(function, name, version="1.0.0", pulserate=25):
    ...
    try:
        if environ["AMQP_SECURE"] == "1":
            ...
            #parameters = pika.URLParameters('amqps://
            #test
            #:WejejG3J53J3
            #@portal1133-8.nanowire-beta-rabbitmq.spotlightdata.composedb.com
            #:24992/')
            parameters = pika.URLParameters('amqps://%s:%s@%s:%s/'%(environ['AMQP_USER'], environ['AMQP_PASS'], environ['AMQP_HOST'], environ['AMQP_PORT']))
            
            parameters._set_url_heartbeat_interval(pulserate)
            '''
