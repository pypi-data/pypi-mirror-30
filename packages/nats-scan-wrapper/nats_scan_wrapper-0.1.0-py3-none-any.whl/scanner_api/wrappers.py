...
from .errors import *
from ._modes import function_mode, process_mode
...
class scanner_wrapper:
    ...
    async def _run(self, loop):
        ...
        # @asyncio.coroutine
        # async def func_message_handler(msg):
        #     with (await self.sem):
        #         data, meta, cur_pipeline = self.decode_msg(msg)

        #         new_pipeline = ".".join(cur_pipeline.split(".")[1:])

        #         logging.info("Received from '{cur_pipeline}': {meta}".format(
        #             cur_pipeline=cur_pipeline, meta=meta))
        ...
        #                 result_array = self.config["worker_function"](cur_pipeline, data, meta)
        #             except Exception as e:
        #                 status = "func_error"
        #                 raise WorkerFunctionError()
        #             if result_array:
        #                 packages = self.make_packages(data, meta, result_array)
        #                 if new_pipeline:
        #                     try:
        #                         status = "ok"
        #                         await self.nats_publisher(new_pipeline, packages)

        #                         logging.info("Result: {result} \n Was sent to '{pipeline}'".format(
        #                             result=packages, pipeline=new_pipeline))
        ...
        #                 else:
        #                     status = "end_pipeline"
        #                     logging.info("Result wasn't sent! End of pipeline!")
        #             else:
        #                 status = "empty_result"
        ...
        async def message_handler(msg):
            ...
            """
            # async def process_wrapper(self, args, output_handler, meta, data):
            with (await self.sem):
                data, meta, cur_pipeline = self.decode_msg(msg)
                new_pipeline = ".".join(cur_pipeline.split(".")[1:])
                status = ""
                ...
                try:
                    if self.config["worker_function"]:
                        await function_mode(self, data, meta, new_pipeline)
                    ...
                    logging.error("Exception in main logic! Report will be sended!")
                    logging.error(str(e))
                    status = "error"
