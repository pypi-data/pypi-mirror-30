...
def inregion(data,centre,length):
	indices= np.where((data[:,0] >= centre[0]-length) & (data[:,0] <= centre[0]+length) & (data[:,1] >= centre[1]-length) & (data[:,1] <= centre[1]+length) & (data[:,2] >= centre[2]-length) & (data[:,2] <= centre[2]+length))
	dataIn = data[indices.tolist,:]
	print(dataIn)
