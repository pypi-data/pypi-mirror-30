...
def velocityCube(obsv, directory, tag, fileType='SNAPSHOT',printA=False,region=[-1,[0,0,0]]):
	...
	if not region[0] == -1:
		print("Cutting")
		[positions, indices] = inregion(position,centre,region[1],region[0]) 
		position = positions
...
def inregion(data,centre,offset,length):
	indices= np.asarray(np.where((data[:,0] >= centre[0]-length+offset[0]) & (data[:,0] <= centre[0]+length+offset[0]) & (data[:,1] >= centre[1]-length+offset[1]) & (data[:,1] <= centre[1]+length+offset[1]) & (data[:,2] >= centre[2]-length++offset[1]) & (data[:,2] <= centre[2]+length+offset[1])))
