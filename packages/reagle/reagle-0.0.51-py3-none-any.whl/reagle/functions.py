...
def velocityCube(obsv, directory, tag, fileType='SNAPSHOT',printA=False,region=-1):
	...
	if not region == -1:
		print("Cutting")
		[positions, indices] = inregion(position,centre,region) 
		position = positions
...
def inregion(data,centre,length):
	indices= np.asarray(np.where((data[:,0] >= centre[0]-length) & (data[:,0] <= centre[0]+length) & (data[:,1] >= centre[1]-length) & (data[:,1] <= centre[1]+length) & (data[:,2] >= centre[2]-length) & (data[:,2] <= centre[2]+length)))
