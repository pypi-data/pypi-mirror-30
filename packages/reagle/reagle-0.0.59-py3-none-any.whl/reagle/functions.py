...
def velocityCube(obsv, directory, tag, fileType='SNAPSHOT', offset=[0,0,0], printA=False):
...
def inregion(data,centre,length,offset=[0,0,0]):
	indices= np.asarray(np.where((data[:,0] >= centre[0]-length+offset[0]) & (data[:,0] <= centre[0]+length+offset[0]) & (data[:,1] >= centre[1]-length+offset[1]) & (data[:,1] <= centre[1]+length+offset[1]) & (data[:,2] >= centre[2]-length++offset[2]) & (data[:,2] <= centre[2]+length+offset[2])))
