...
def run_cmd_eql(ssh_session, cmd, verbose=True, return_output=False, timeout=None):
    """
    ...
    """
    # it might be a good idea to disable the following settings before running a command
    # SETTINGS = ("paging", "confirmation", "events", "formatoutput")
    # Eqlogic prompt when finishes printing output
    ...
    # We did log in, so process messages until prompt
    wait_eql_prompt(chan)
    ...
    for c in cmd if not isinstance(cmd, str) else [cmd]:
        ...
        # Eqlogic requires \r
        chan.send('\n\r')
        stdout_buf += wait_eql_prompt(chan)
    chan.close()
    error = 0
    # The shell will echo the input command as first line on output, just remove it
    stdout_buf = stdout_buf.split("\n")[1:-1]
    if verbose:
        print(stdout_buf)
...
def wait_eql_prompt(chan):
    """
    Read session until find prompt
    """
    eql_prompt = "> "
    tmp_buf = ""
    stdout_buf = ""
    while not tmp_buf.endswith(eql_prompt):
        if chan.recv_ready():
            # rl, wl, xl = select.select([chan], [], [], 0.0)
            # if len(rl) > 0:
            tmp_buf = chan.recv(1000000)
            if not tmp_buf:
                raise EOFError
                # continue
            tmp_buf = tmp_buf.replace("\r", "")
            # print "(%s)" % tmp_buf
            stdout_buf += tmp_buf
            continue
    return stdout_buf
...
class Eqlogic:
    ...
    def _run(self, cmd, return_output=False, verbose=True):
        ...
        try:
            ret = run_cmd_eql(session, cmd, return_output=return_output, verbose=verbose,
    ...
    def get_version(self):
        ...
        for key in list(member_dict.keys()):
            if "version" in list(member_dict[key].keys()):
    ...
    def query_all_luns(self):
        ...
        self.luns = []
        for lun_name in list(output_dict.keys()):
    ...
    def _lun_wwid(lun_info_dict):
        ...
        eql_wwid_iqn_regex = re.compile("^iqn\..*equallogic:(\S+?)-(\S+?)-(\S+?)-(\S+?)-\S+$")
    ...
    def lun_info(self, lun_name):
        ...
        if "_volume_information_" not in list(output_dict.keys()):
            return None
        vol_info_dict = output_dict["_volume_information_"]
        lun_info_dict = {}
        for key in list(vol_info_dict.keys()):
        ...
        if "_access_records_" in list(output_dict.keys()):
            map_infos = output_dict["_access_records_"]
            map_info = None
            for key in list(map_infos.keys()):
                map_info = dict()
        ...
        if "iscsi_name" in list(lun_info_dict.keys()):
    ...
    def lun_map(self, name):
        ...
        for map_info in self.map_info:
            if "h_iqn" not in list(map_info.keys()):
    ...
    def lun_query_access(self, name):
        ...
        if not info_dict:
            ...
            return None
        if "map_infos" in list(info_dict.keys()):
