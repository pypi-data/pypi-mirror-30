...
class ontap:
    ...
    def get_version(self):
        ...
        if ret != 0:
            ...
            return None
        lines = output.split("\n")
        release_regex = re.compile("NetApp Release ([0-9]+)([^ ]+)")
        for line in lines:
    ...
    def query_all_lun_info(self, recheck=False):
        ...
        _lun_basic_regex = re.compile("\s+(/vol/\S+)\s+(\S+[kmgt])\s+\((\d+)\)\s+\((.*)\)?")
        # _lun_basic_regex = re.compile("\s+(/vol/\S+)\s+(\d+[kmgt])\s+\((\d+)\)\s+\(.*\)")
        _lun_serial_regex = re.compile("\s+Serial\#:\s+(\S+)")
        _lun_map_regex = re.compile("\s+Maps:\s+(\S+)=(\S+)")
    ...
    def query_all_igroup_info(self, recheck=False):
        ...
        _igroup_basic_regex = re.compile("\s+(\S+)\s+\((\S+)\):$")
        _igroup_os_type_regex = re.compile("\s+Os Type:\s+(\S+)")
        _igroup_member_regex = re.compile("\s+Member:\s+(\S+)")
    ...
    def sa_ctrler_t_wwpns(self, ctrler_ip):
        ...
        if not output:
            return None
        lines = output.split("\n")
        online_regex = re.compile("FC Portname:\s+(\S+)")
    ...
    def sa_ctrler_reboot():  # self , ctrler_ip
        ...
        # cmd = "reboot -t 0"
        # ret, output = self._run(cmd, ctrler=ctrler_ip, return_output=True, verbose=False)
        # It seems command return fail when it reboots
        # if ret != 0:
        ...
        #
        # _print("FAIL: Thought reboot the controller, but it is still online")
        # return False
        return True
    ...
    def sa_ctrler_check(self, ctrler_ip):
        ...
        if not output:
            return "offline"
        lines = output.split("\n")
        online_regex = re.compile("FCP service is running")
        for line in lines:
        ...
        if not output:
            return "offline"
        lines = output.split("\n")
        online_regex = re.compile("iSCSI service is running")
        for line in lines:
            if online_regex.search(line):
                iscsi_status = "online"
        ...
        online_regex = re.compile("\s+\S+\sup\s+\S+")
        for line in lines:
