...
def create_sanmgmt_for_storage_array(storage_name, san_conf=None):
    ...
    for san_cfg in san_conf_list:
        if "options" not in list(san_cfg.keys()):
            continue
...
def _load_san_conf(san_conf=None):
    ...
    san_conf_list = libsan.host.conf.load_config(san_conf)
    ...
    if alias_dict:
        for entry in san_conf_list:
            if entry["name"] == "alias":
                continue
            for sub_key in list(entry["options"].keys()):
                if entry["options"][sub_key] in list(alias_dict.keys()):
    ...
    for san_hw in san_conf_list:
        for key in list(san_hw["options"].keys()):
            ...
            if mac_match:
                ...
                if mac:
                    ...
                    if "port_of" not in list(san_hw["options"].keys()):
                        san_hw["options"]["port_of"] = {}
                    if mac in list(san_hw["options"]["port_of"].keys()):
            ...
            if wwpn_match:
                ...
                if wwpn:
                    if "port_of" not in list(san_hw["options"].keys()):
                        san_hw["options"]["port_of"] = {}
                    if wwpn in list(san_hw["options"]["port_of"].keys()):
            ...
            if libsan.host.iscsi.is_iqn(value):
                if "port_of" not in list(san_hw["options"].keys()):
                    san_hw["options"]["port_of"] = {}
            ...
            if simplex_match:
                if "simplex_con" not in list(san_hw["options"].keys()):
            ...
            if duplex_match:
                if "duplex_con" not in list(san_hw["options"].keys()):
    ...
    for san_hw in san_conf_list:
        if "type" not in list(san_hw["options"].keys()):
            continue
        if san_hw["options"]["type"] != "iscsi_initiator":
            if "model" not in list(san_hw["options"].keys()):
            ...
            for option in list(san_hw["options"].keys()):
                m_iface = re.match("^iface_(\d+)$", option)
                if m_iface:
                    if "ifaces" not in list(iscsi_host_conf.keys()):
                        san_iscsi_conf[san_hw["name"]]["ifaces"] = {}
                    if m_iface.group(1) not in list(iscsi_host_conf["ifaces"].keys()):
                ...
                m_iface = re.match("^iface_mask_(\d+)$", option)
                if m_iface:
                    if "ifaces" not in list(iscsi_host_conf.keys()):
                        iscsi_host_conf["ifaces"] = {}
                    if m_iface.group(1) not in list(iscsi_host_conf["ifaces"].keys()):
                        iscsi_host_conf["ifaces"][m_iface.group(1)] = {}
                    iscsi_host_conf["ifaces"][m_iface.group(1)]["mask"] = san_hw["options"][option]

                m_iface = re.match("^iface_gateway_(\d+)$", option)
                if m_iface:
                    if "ifaces" not in list(iscsi_host_conf.keys()):
                        iscsi_host_conf["ifaces"] = {}
                    if m_iface.group(1) not in list(iscsi_host_conf["ifaces"].keys()):
                        iscsi_host_conf["ifaces"][m_iface.group(1)] = {}
                    iscsi_host_conf["ifaces"][m_iface.group(1)]["gateway"] = san_hw["options"][option]
...
def check_sw_conf(config_file=None, switch_id=None):
    ...
    for key in switch_ids:
        ...
        if config[key]["model"] == "Cisco-NXOS":
            switch = libsan.switch.cisco.nxos.nxos(ip, user, passwd)
        elif config[key]["model"] == "Brocade-FOS":
            switch = libsan.switch.brocade.fos.fos(ip, user, passwd)
        else:
            # unsupported switch
            continue
        ...
        for p_id in list(wwpn_dict.keys()):
...
class SanMgmt:
    ...
    def load_conf(self, san_conf=None):
        ...
        if not self.san_conf_list:
            self.san_conf_list = []
            san_general_dict = dict()
            san_general_dict["name"] = "general"
            san_general_dict["options"] = {}
            san_general_dict["options"]["conf_loaded"] = False
    ...
    def is_config_loaded(self):
        ...
        for san_hw in self.san_conf_list:
            if san_hw["name"] == "general":
                san_general_dict = san_hw["options"]
                if "conf_loaded" in list(san_general_dict.keys()):
    ...
    def _load_sa_conf(self):
        ...
        for san_hw in san_conf_list:
            san_dev = san_hw["name"]
            if "options" not in list(san_hw.keys()):
                continue
            dev_conf_dict = san_hw["options"]
            if "type" not in list(dev_conf_dict.keys()):
                continue
            if dev_conf_dict["type"] != "array":
                continue
            # Check if array has port configured
            if "port_of" not in list(dev_conf_dict.keys()):
                continue

            dev_conf_dict["name"] = san_dev
            _all_configured_tgt_ports.extend(dev_conf_dict["port_of"])

            if _targets:
                # We are connected to some target
                for target in _targets:
                    if (target in list(dev_conf_dict["port_of"].keys()) or
                            ...
                            ((re.match(t, target) in t for t in list(dev_conf_dict["port_of"].keys())) and
                             libsan.host.iscsi.is_iqn(target) and dev_conf_dict["model"] == "DELL-EQLOGIC")):
                        # Add SAN device to sa_config dict
                        if ("devices" in list(sa_conf_dict.keys()) and
                                san_dev in list(sa_conf_dict["devices"].keys())):
                            # We already stored the configuration of this device
                            continue
                        if san_dev not in list(sa_conf_dict.keys()):
                            if "ctrl_ip" not in list(dev_conf_dict.keys()):
                                _print("FAIL: _load_sa_conf() - ctrl_ip not set on %s" % san_dev)
                                continue
                            ip = dev_conf_dict["ctrl_ip"]

                            if "ctrl_user" not in list(dev_conf_dict.keys()):
                                _print("FAIL: _load_sa_conf() - ctrl_user not set on %s" % san_dev)
                                continue
                            user = dev_conf_dict["ctrl_user"]

                            if "ctrl_pass" not in list(dev_conf_dict.keys()):
                                _print("FAIL: _load_sa_conf() - ctrl_pass not set on %s" % san_dev)
                                continue
                            passwd = dev_conf_dict["ctrl_pass"]

                            tmo = None
                            if "ctrl_tmo" in list(dev_conf_dict.keys()):
                            ...
                            found_sa_conf = False
                            for san_hw1 in san_conf_list:
                                if san_hw1["name"] == "sa_conf":
                                    found_sa_conf = True
    ...
    def _load_sw_conf(self, flag_physwitch=None):
        ...
        for san_hw in san_conf_list:
            ...
            for addr in list(dev_conf_dict["port_of"].keys()):
                # In case there are duplicated address
                # do not process it more than once
                if addr in list(sws_conf_dict.keys()):
                    continue
                ...
                if switch_obj:
                    ...
                    for sw_cap in all_sw_caps:
                        if sw_cap in list(switch_cap.keys()) and switch_cap[sw_cap]:
                            if sw_cap not in list(san_capability_dict["options"].keys()):
    ...
    def sa_names(self):
        ...
        for san_hw in self.san_conf_list:
            if san_hw["name"] == "sa_conf":
                return list(san_hw["devices"].keys())
        return None
    ...
    def get_sa_obj_self(self):
        ...
        if "class_obj" not in list(sa_val.keys()):
