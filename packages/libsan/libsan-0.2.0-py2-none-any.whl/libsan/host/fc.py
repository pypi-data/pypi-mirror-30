...
regex_target_id = re.compile("target(\d+):(\d+):(\d+)")
regex_target = re.compile("(\d+):(\d+):(\d+)")
...
def scsi_disk_of_htwwpn_wwid(h_wwpn, t_wwpn, wwid):
    ...
    for scsi_disk_id in scsi_disk_ids:
        if re.match("%s:\d+$" % target_id, scsi_disk_id):
...
def set_value_rport_parameter(r_port, r_port_param, value):
    ...
    ret, output = run("echo %s > %s" % (value, sys_r_port_param),
