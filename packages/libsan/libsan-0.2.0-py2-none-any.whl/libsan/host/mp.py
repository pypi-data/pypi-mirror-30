...
def get_mpath_of_disk(disk):
    ...
    if libsan.host.linux.dist_name() == "RHEL" and libsan.host.linux.dist_ver() < 7:
        ...
        if retcode != 0:
            ...
            print(output)
            return None
        deps_cmd = "dmsetup deps | grep \"%s\" | awk -v device=%s '{print $1}' | tr -d \"\:\"" % (output, device)
...
def multipath_query_all(mpath_name=None):
    ...
    regex_mpath = "^(?:create\:\ ){0,1}"  # might have when ceating mpath
    regex_mpath += "(?:reload\:\ ){0,1}"  # might have when ceating mpath
    regex_mpath += "([^ ]+?)\ +"  # 1. mpath_name
    regex_mpath += "(?:\((\S+)\)\ +){0,1}"  # 2. wwid if different from #1
    regex_mpath += "(?:(dm\-[0-9]+)\ +){0,1}"  # 3. dm_name if known
    regex_mpath += "(?:([^,]+),){0,1}"  # 4. vendor, might not have
    regex_mpath += "(?:([^,]+)){0,1}$"  # 5. product,  might not have

    regex_feature = "size=([0-9\.]+[A-Z])\ +"  # 1. size
    regex_feature += "features=\'([^\']+)\'\ +"  # 2. features
    regex_feature += "hwhandler=\'([^\']+)\'\ +"  # 3. hardware handler
    regex_feature += "(?:wp=([rw]+)){0,1}"  # 4. write permission if known

    regex_pg = "[^a-z]+policy=\'([^\`]+)\'\ +"  # 1. policy of this PG path_selector
    regex_pg += "(?:prio=([0-9\-]+)\ +){0,1}"  # 2. priority of this PG if known
    regex_pg += "(?:status=([a-z]+)){0,1}"  # 3. status of this PG if known (active or enable, etc)

    regex_disk = "[^0-9]+"
    regex_disk += "([0-9]+:[0-9]+:[0-9]+:[0-9]+)\ +"  # 1. scsi_id
    regex_disk += "([a-z]+)\ +"  # 2. dev_name
    regex_disk += "([0-9]+:[0-9]+)\ +"  # 3. major:minor
    regex_disk += "(?:([a-z]+)\ +){0,1}"  # 4. dm_status if known, kernel level
    #   failed or active
    regex_disk += "(?:([a-z]+)\ +){0,1}"  # 5. path_status if known, maintained
    #   by multipathd path_checker
    #   ready  or ghost or
    #   faulty or shaky
    regex_disk += "(?:([a-z]+)){0,1}"  # 6. online_status: running or offline
...
def multipath_setup_blacklist():
    ...
    if not mp_add_blacklist_wwid("*"):
        return False

    for wwid in current_wwids:
        if not mp_add_blacklist_exception_wwid(wwid):
            return False

    _print("INFO: Reloading updated multipath configuration")
    # map { $ok = 0 unless mp_add_blacklist_exception_wwid($_) } @current_wwids;
    multipath_reload_conf()
    return True


def mp_add_blacklist_wwid(wwid):
    """
    Usage
        mp_add_blacklist_wwid(wwid)
    Purpose
        Add wwid to 'blacklist' section.
        Will call mp_query_saved_conf() and mp_write_conf().
        Will not reload config, need run multipath_reload_conf() by yourself.
    Parameter
        wwid
    Returns
        True
    ...
    mp_conf = mp_query_saved_conf()
    if "blacklist" not in list(mp_conf.keys()):
        mp_conf["blacklist"] = {}

    if "wwids" not in list(mp_conf["blacklist"].keys()):
        mp_conf["blacklist"]["wwids"] = []

    if wwid not in mp_conf["blacklist"]["wwids"]:
        mp_conf["blacklist"]["wwids"].append(wwid)

    global multipath_conf_path
    success = _save_config(mp_conf, multipath_conf_path)
    if not success:
        _print("FAIL: Failed to write multipath configuration.")
        return False
    return True
...
def mp_add_blacklist_exception_wwid(wwid):
    """
    Usage
        mp_add_blacklist_exception_wwid(wwid)
    Purpose
        Add wwid to 'blacklist_exception' section.
        Will call mp_query_saved_conf() and mp_write_conf() sequently.
        Will not reload config, need run multipath_reload_conf() by yourself.
    Parameter
        wwid
    Returns
    ...
    mp_conf = mp_query_saved_conf()
    if "blacklist_exceptions" not in list(mp_conf.keys()):
        mp_conf["blacklist_exceptions"] = {}

    if "wwids" not in list(mp_conf["blacklist_exceptions"].keys()):
        mp_conf["blacklist_exceptions"]["wwids"] = []

    if wwid not in mp_conf["blacklist_exceptions"]["wwids"]:
        mp_conf["blacklist_exceptions"]["wwids"].append(wwid)
...
def mp_rm_blacklist_exception_wwid(wwid):
    """
    Usage
        mp_rm_blacklist_exception_wwid(wwid)
    Purpose
        Remove wwid to 'blacklist_exception' section.
        Will call mp_query_saved_conf() and mp_write_conf() sequently.
        Will not reload config, need run multipath_reload_conf() by yourself.
    Parameter
        wwid
    ...
    mp_conf["blacklist_exceptions"]["wwids"].remove(wwid)

    global multipath_conf_path
    success = _save_config(mp_conf, multipath_conf_path)
    if not success:
        _print("FAIL: mp_rm_blacklist_exception_wwid() - Failed to write multipath configuration.")
        return False
    return True
...
def mpath_device_2_mpath_name(mpath_device):
    ...
    """
    multipath_dev_regex = "/dev/mapper\/(.*)"
