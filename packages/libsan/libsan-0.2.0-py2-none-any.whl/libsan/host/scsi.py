...
def query_all_scsi_disks(scsi_disk=None):
    ...
    for disk_id in disk_ids:
        ...
        if scsi_disk and scsi_disk != scsi_name:
...
def get_hosts(somethings=None):
    ...
    for something in somethings if not isinstance(somethings, basestring) else [somethings]:
...
def query_scsi_host_info(host_id):
    ...
    for sysfs_host in sysfs_hosts:
        if not os.path.isdir(sysfs_host):
            continue
        host_files = [f for f in listdir(sysfs_host) if os.path.isfile(os.path.join(sysfs_host, f))]
...
def scsi_driver_of_host_id(host_id):
    ...
    ret, output = run("cat %s" % scsi_drv_sysfs, return_output=True, verbose=False)
    scsi_driver = output
    if scsi_driver == "(null)" or scsi_driver == "":
        ...
        if os.path.isfile(driver_sysfs_file):
            ret, output = run("cat %s" % driver_sysfs_file, return_output=True, verbose=False)
            return output

        model_sysfs_file = "/sys/class/scsi_host/host%s/model_name" % host_id
        if os.path.isfile(model_sysfs_file):
            ret, output = run("cat %s" % model_sysfs_file, return_output=True, verbose=False)
        ...
        if os.path.isfile(symbolic_sysfs_file):
            ret, output = run("cat %s" % symbolic_sysfs_file, return_output=True, verbose=False)
            if re.search("bnx2fc", output):
                return "bnx2fc"

        pci_id = pci_id_of_host_id(host_id)
        if pci_id:
            lspci_regex = "Kernel modules:\s+(\S+)"
            ret, output = run("lspci -s \"%s\" -v | grep \"Kernel modules:\"" % pci_id,
                                                  return_output=True,
                                                  verbose=False)
            if output:
        ...
        if scsi_driver == "fcoe":
            ...
            ret, output = run("cat %s" % drv_version_path, return_output=True, verbose=False)
...
def pci_id_of_host_id(host_id):
    ...
    net_regex = re.compile("devices/virtual/net/(.*)\.")
...
def wwid_of_disk(scsi_disk):
    ...
    # It seems scsi_id is the preferable tool to query scsi WWID, but could not use scsi_id to find out the
    # wwid of LUNs that were removed on target first
    # NOTE: checking info from ATA devices ID_SERIAL from udevadm might does not match output from scsi_id
    cmd = "udevadm info --name=%s --query=all" % scsi_disk
    # cmd = "scsi_id --whitelisted --replace-whitespace /dev/%s" % scsi_disk
    retcode, output = run(cmd, return_output=True, verbose=False)
    if retcode != 0:
        # _print("FAIL: wwid_of_disk() - Could not query %s" % scsi_disk)
        # print output
        return None
    # return output
...
def get_free_disks(exclude_boot_device=True, exclude_lvm_device=True, exclude_mpath_device=True, filter_only=None):
    ...
    all_mp_info = None
    if (not libsan.host.mp.is_multipathd_running()) and exclude_mpath_device:
    ...
    for scsi_disk in list(all_scsi_disks.keys()):
        ...
        if pvs and exclude_lvm_device:
            for pv in list(pvs.keys()):
                if scsi_device_2_scsi_name(pv) == scsi_disk:
...
def disk_sys_check(scsi_disk):
    ...
    cmd = "cat %s 2>/dev/null" % sys_path
    retcode, state = run(cmd, return_output=True, verbose=False)
    if not state:
...
def timeout_of_disk(scsi_disk):
    ...
    cmd = "cat %s 2>/dev/null" % sys_path
    retcode, timeout = run(cmd, return_output=True, verbose=False)
    if not timeout:
