...
class Procedures:
    def __init__(self, frame, base):
        ...
        self.fields = ("ProcedureID","ProcedureName","Cost")
    ...
    def create_tree(self):
        ...
        self.tree = Treeview(treeframe,
            parent_self=self, style='T.Treeview', headers=["ProcedureID", "ProcedureName", "Cost"])
        self.tree.pack(side='bottom', fill=BOTH, expand=1)
        self.load_register()
        self.tree.set_register(self.REGISTERED)

    def find_by_id(self, pt_id=None):
        sql = """SELECT * FROM procedures WHERE ProcedureID="%s" """%pt_id
        if self.connection:
    ...
    def load_register(self):
        sql = "SELECT ProcedureID, ProcedureName, Cost FROM procedures"
        if self.connection:
            cursor = self.connection.cursor()
            cursor.execute(sql)
            results = cursor.fetchall()
    ...
    def update(self):
        procid = self.entries["ProcedureID"].get()
        if procid:
            entryItems = self.entries.items()
            query = "UPDATE procedures SET"
            for key, value in entryItems:
                if not key == "ProcedureID":
                    text = value.get()
                    query += """ %s = "%s",""" % (key, text)
            query = query[:-1] + " WHERE ProcedureID = %s " % procid
    ...
    def delete(self):
        ...
        if procid:
            ans = askquestion("Confirm",
                "Delete %s from clients?"%self.entries["ProcedureName"].get())
            if not ans =="yes":return
            sql= "DELETE FROM procedures WHERE ProcedureID='%s'"%procid
            if self.connection:
                try:
                    cursor=self.connection.cursor()
                    cursor.execute(sql)
    ...
    def find_next(self):
        try:
            _next = int(self.entries["ProcedureID"].get()) +1
    ...
    def find_prev(self):
        try:
            prev = int(self.entries["ProcedureID"].get()) -1
