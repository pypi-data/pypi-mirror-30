...
class TelegramBot(telepot.aio.Bot):
    ...
    async def answer_handler(self, msg: dict, is_text_answer: bool, test_index: int, question_index: int):
        ...
        try:
            ...
            if is_text_answer:
                ...
                message = "Вопрос {0}. Ваш ответ: {1}. Ваш балл: {2}.".format(question_index+1, query_data, res)
                await self.sendMessage(from_id, message)
                msg_obj = {"data": str(test_index), "from": {"id": from_id}}
            ...
            if is_text_answer:
                if content_type == "text":
                    ...
                    message = "Вопрос {0}. Ваш ответ: {1}. Ваш балл: {2}.".format(question_index+1, answer, res)
                    await self.sendMessage(chat_id, message)
                    # TO DO: удаляем сообщение с вопросом
                    # переходим к следующему вопросу
                    msg_obj = {"data": str(test_index), "from": {"id": chat_id}}
                    await self.run_test(msg_obj)
                else:
    ...
    async def show_user_question(self, _id: int, question_obj: dict,
        ...
        if question_obj["answer_variants"]:
            ...
            for ans in question_obj["answer_variants"]:
                inline_keyboard[0].append(InlineKeyboardButton(text=ans, callback_data=str(ans)))
        ...
        message = "Вопрос {0}:".format(question_index+1)
        ...
        try:
            ...
            if current_question >= len(questions):  # Закончен ли тест?
                ...
                await self.sendMessage(
                    from_id, ("Поздравляем, вы завершили тест <b>{0}</b>!\nВаш результат: {1}%".format(
                        test['name'], res['result'])), parse_mode="HTML")
            else:
    ...
    async def show_package(self, _id: int, pkg_obj: dict, edit_mode: bool=False):
        ...
        for test in tests:
            ...
            inline_keyboard.append([InlineKeyboardButton(
                text=test_name, callback_data="test={0}".format(test_name))])
        ...
        text = ("Вы редактируете пакет <b>" + str(pkg_obj['name']) + "</b>. Текущие значения параметров:\n"
                "Краткое описание: <i>" + str(pkg_obj['short_description']) + "</i>\n"
                "Описание: <i>" + str(pkg_obj['description']) + "</i>\n"
                "Цена: <b>" + str(pkg_obj['price']) + "</b>\n")
        try:
    ...
    async def show_test(self, _id: int, test_obj: dict, edit_mode: bool=False):
        ...
        for q_id in range(len(questions)):
            inline_keyboard.append([InlineKeyboardButton(
                text=q_id+1, callback_data="question={0}".format(q_id))])
        ...
        # TODO: Дополнить text
        text = ("Вы редактируете тест <b>" + str(test_obj['name']) + "</b>. Текущие значения параметров:\n"
                "Цена: <b>" + str(test_obj['price']) + "</b>\n")
        try:
    ...
    async def show_question(self, _id: int, question_obj: dict, edit_mode: bool=False):
        ...
        text = ("Вы редактируете вопрос. Текущие значения параметров:\n"
                "Баллы: <b>" + str(question_obj['points']) + "</b>\n"
                "Ответ: <b>" + str(question_obj['answer']) + "</b>\n"
                "Варианты ответов: <b>" + str(question_obj['answer_variants']) + " "
                "(при редактировании вводить через \";\" без ковычек)</b>\n"
                "Текст вопроса: <b>" + str(question_obj['text']) + "</b>\n"
                "<a href=\"" + str(question_obj['img']) + "\">Картинка</a>")
        try:
...
class MyChatHandler(telepot.aio.helper.ChatHandler, TelegramBot):
    ...
    async def all_tests(self, _id: int):
        ...
        for name in packages:
            ...
            text += "Добавить все тесты пакета: /add_package_{0}\n".format(p_index)
            text += "Добавить тесты отдельно:\n"
            t_index = 1
            for test in package["tests"]:
                text += "&gt;&gt;  <b>{0}</b> -> /add_test_{1}_pkg_{2}\n".format(test['name'], t_index, p_index)
                t_index += 1
            text += "\n"
            p_index += 1
    ...
    async def edit_packages(self, _id: int):
        ...
        inline_keyboard = [
            [InlineKeyboardButton(text=package,
                                  callback_data="package={0}".format(package)
    ...
    async def edit_bot_setting(self, msg: dict, setting: str):
        ...
        if content_type == "text":
            ...
            if mime_type == "text/html" or mime_type == "text/plain":
                ...
                await self.sendMessage(
                    chat_id, "Тип файлов {0} не поддерживается. Повторите ввод:".format(mime_type))
        else:
