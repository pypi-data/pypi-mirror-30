...
Run a query against a collection or a view
...
examples:
  ...
  This example assumes you have a view named "logins" that includes two
  collections "bad_ips" and "login_attempts":
  ...
  $ python3 -c 'from rockset import Q,F; \\
        print(Q(F[":collection"] == "bad_ips")\
.select(F["ip_address"])\
.apply(F["login_ip"]))' | rock query logins
    ...
    def go(self):
        ...
        # lets do this
        results = self.client.query(q=q, name=self.name,
