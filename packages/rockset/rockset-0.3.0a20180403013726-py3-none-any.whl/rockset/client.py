...
to retrieve collections and views.
::
    ...
    # list all views in the account that I have access to
    all_views = rs.View.list()

    # create a new view ; returns a view object
    hotness = rs.View.create('hotness')

    # get details of an existing view as a view object
    search_app = rs.View.retrieve('prod_search')
...
class Client(object):
    ...
    #: Maximum allowed length of a collection or view name
    ...
    #: Maximum allowed length of ``:id`` field value
    ...
    def __init__(self,
        ...
        self.View = ClientResource(resource=View,
            client=self, model=self.swagger_client.All)
    ...
    def retrieve(self, name):
        func = self.swagger_client.All.describe
        resource = self.apicall(method=func, name=name, all=False)['data']
        ...
        if rtype == 'COLLECTION':
            ...
            return View(client=self, model=self.swagger_client.All,
                name=rname)
        raise TypeError('cannot recognize type {} for resource {}'.format(
            rtype, name))

    #TODO(veeve): add this method to docs
    def query(self, q, name=None, **kwargs):
        ...
        if not name:
            name = q.get_source()
            if not name:
                raise ValueError('please specify target collection or view to '
