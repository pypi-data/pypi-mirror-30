        ...
        try:
            with self.conn.cursor() as cur:
                _logger.debug('Search path query. sql: %r',
                    self.show_schemas_query)
                cur.execute(self.show_schemas_query)
                return [x[0] for x in cur.fetchall()] + [self.default_schema_name]
        except (rs.exception.ProgrammingError, rs.exception.OperationalError):
            pass
        return []

    def _relations(self, kinds=['all']):
        """Get table or view name metadata
        :param kinds: 'all' or 'collection' or 'view'
        :return: (schema_name, rel_name) tuples
        """
        kwargs = {}
        for kind in kinds:
            kwargs['type'] = kind
            for resource in self.conn._client.list(**kwargs):
                for workspace in self.schemata():
                    yield (workspace, resource['name'])

    def tables(self):
        """Yields (schema_name, table_name) tuples"""
        for row in self._relations(kinds=['collection']):
            yield row

    def views(self):
        """Yields (schema_name, view_name) tuples.
        """
        for row in self._relations(kinds=['view']):
            yield row
