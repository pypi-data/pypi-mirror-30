...
class HTTPError(Error):
    ...
    def __init__(self, **kwargs):
        ...
        if kwargs['response'] is not None:
            r = kwargs['response']
            kwargs['code'] = getattr(r, 'status_code', '?')
...
class ResourceSuspendedError(HTTPError):
    """ Target collection or view was suspended due to inactivity. A query
    issued against the resource will automatically trigger reactivation of
    the target collection or view. Please retry the query once the resource
