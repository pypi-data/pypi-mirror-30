...
class LocalData:
    ...
    def default_workspace( cls ) -> str:
        ...
        if not LocalData.is_ready():
            raise NotReadyError( "Attempt to the obtain default workspace without setting the application name. Possible cause: The programmer has attempted to get or set an application setting stored in the local data store before setting the application name. Maybe they tried to register plugins setting before calling MENV.name? Make sure MENV.name is set before importing the plugins in the package's __init__.py." )
