...
This transformation is parameterized by BS[2] (theta) which determines the beam splitter reflectivity, and by BS[3] (phi).
...
class Interferometer:
    ...
    def add_BS(self,BS):    #Use this to manually add a beam splitter at the output of the current interferometer
        self.BS_list.append(BS)
        
    def add_phase(self,phase):    #Use this to manually add a phase shift to a selected mode at the output of the interferometer
        while phase[0] > np.size(self.output_phases) :
            self.output_phases.append(0)
        self.output_phases[phase[0]-1] = phase[1]
    
    def n_modes(self):          #Calculate number of modes involved in the transformation. This is required for unitary_transformation and draw_interferometer
        list_modes = []
        for BS in self.BS_list:
            list_modes.append([BS[0],BS[1]])
        return np.max(list_modes)
    
    def unitary_transformation(self):       #Calculate unitary matrix describing overall unitary transformation
        N = int(self.n_modes())
    ...
    def draw_interferometer(self):  #Use matplotlib to make a drawing of the interferometer
        plt.cla()
        N = self.n_modes()
        mode_tracker = np.zeros(N)

        for ii in range(N):
            line = plt.Line2D((-1, 0), (ii, ii), lw=1)
            plt.gca().add_line(line)
        
        for BS in self.BS_list:
            x = np.max([mode_tracker[BS[0]-1],mode_tracker[BS[1]-1]])
            line = plt.Line2D((x+0.3, x+1), (N-BS[0], N-BS[1]), lw=1)
            plt.gca().add_line(line)
            line = plt.Line2D((x, x+0.3), (N-BS[0], N-BS[0]), lw=1)
            plt.gca().add_line(line)
            line = plt.Line2D((x, x+0.3), (N-BS[1], N-BS[1]), lw=1)
            plt.gca().add_line(line)
            line = plt.Line2D((x+0.3, x+1), (N-BS[1], N-BS[0]), lw=1)
            plt.gca().add_line(line)
            line = plt.Line2D((x+0.4, x+0.9), (N-0.5-(BS[1]+BS[0])/2, N-0.5-(BS[1]+BS[0])/2), lw=1)
            plt.gca().add_line(line)
            reflectivity = str(np.cos(BS[2])**2)
            plt.text(x+0.9, N-0.45-(BS[1]+BS[0])/2, reflectivity[0:3], color="green", fontsize=7)
            
            line = plt.Line2D((x+0.15, x+0.15), (N-0.2-(BS[1]+BS[0])/2, N+0.2-(BS[1]+BS[0])/2), lw=1)
            plt.gca().add_line(line)
            circle = plt.Circle((x+0.15, N-(BS[1]+BS[0])/2), 0.1, fill=False)
            plt.gca().add_patch(circle)
            phase = str(BS[3])
            if BS[3] > 0:
                plt.text(x+0.2, N+0.2-(BS[1]+BS[0])/2, phase[0:3], color="red", fontsize=7)
            else:
                plt.text(x+0.2, N+0.2-(BS[1]+BS[0])/2, phase[0:4], color="red", fontsize=7)
            if x > mode_tracker[BS[0]-1]:
                line = plt.Line2D((mode_tracker[BS[0]-1], x), (N-BS[0], N-BS[0]), lw=1)
                plt.gca().add_line(line)
            if x > mode_tracker[BS[1]-1]:
                line = plt.Line2D((mode_tracker[BS[1]-1], x), (N-BS[1], N-BS[1]), lw=1) 
                plt.gca().add_line(line)
        ...
        for ii in range(N):
            line = plt.Line2D((mode_tracker[ii], max_x+1), (N-ii-1, N-ii-1), lw=1)
            plt.gca().add_line(line)
            while np.size(self.output_phases) < N:      #Autofill for users who don't want to bother with output phases
                self.output_phases.append(0)
            if self.output_phases[ii] != 0:
                line = plt.Line2D((max_x+0.5, max_x+0.5), (N-ii-1.2, N-ii-0.8), lw=1)
                plt.gca().add_line(line)
...
"""The following code implements the decomposition algorithm in:
...
"""
The following code is a python implementation of Matlab code written by Toby Cubitt:
