...
def unmarshal(
    ...
    # if isinstance(data, Model):
    #     if serial.meta.pointer(data) is None:
    #         serial.meta.pointer(data, '#')
    #     if serial.meta.xpath(data) is None:
    #         serial.meta.xpath(data, '')
    ...
    def __setattr__(self, property_name, value):
        ...
        if property_name[0] != '_':
            ...
            if isinstance(value, Model):
                key = property_definition.name or property_name
                ...
                if pointer is not None:
                    serial.meta.pointer(
                        value,
                        '%s/%s' % (pointer, key.replace('~', '~0').replace('/', '~1'))
                    )
...
class Dictionary(OrderedDict, Model):
    ...
    def __setitem__(
        ...
        if isinstance(value, Model):
            ...
            url = serial.meta.url(self)
            if pointer is not None:
                serial.meta.pointer(value, '%s/%s' % (pointer, key.replace('~', '~0').replace('/', '~1')))
            if xpath is not None:
                serial.meta.xpath(value, '%s/%s' % (xpath, key))
            if url is not None:
                serial.meta.url(value, url)
        super().__setitem__(
            key,
            value
        )
