...
def structure(struct, projection='XY', color='auto', scale=1, 
    ...
    if color == 'auto':
        if hasattr(struct, "struct"):
            if hasattr(struct.struct.material, '__iter__'):
                ...
                material = np.array(struct.struct.material)[materials_idx]
                different_materials = np.unique(material)
                indices_subsets = []
    ...
    if color != 'auto':
        ...
        for i, idx in enumerate(indices_subsets):
            col = colors[i]
            if plot_legend:
                label = different_materials[i].__name__
            else:
                label = ''
...
def animate_vectorfield(NF, Nframes=50, projection='XY', doQuiver=True, doColor=False, 
                     ...
                     kwargs={'cmin':0.3}, kwargsColor={'fieldComp':'Ex'}, ax=None, show=True):
