...
class AmazonDaxClient(object):
    ...
    def resource(session=None,
        ...
        '''
        # Local import so it's only used if boto3 is available
        import boto3
        ...
        # Directly use the AmazonDaxClient
        dax_client = AmazonDaxClient(session,
            region_name=region_name, api_version=api_version,
            use_ssl=use_ssl, verify=verify, endpoint_url=endpoint_url,
            aws_access_key_id=aws_access_key_id,
            aws_secret_access_key=aws_secret_access_key,
            aws_session_token=aws_session_token, config=config, endpoints=endpoints)

        # Create the boto DynamoDB resource
        # This will use the default client, but we're going to replace it so it shouldn't
        # cause any issues.
        res = session.resource('dynamodb', region_name=region_name)

        # Swap out the default client for the DAX client
        res.meta.client = dax_client

        # Add the resouce manager APIs to ensure the client is closed
        res.__enter__ = _resource_enter
        res.__exit__ = _resource_exit
    ...
    def create_backup(self, **kwargs):
        raise NotImplementedError("Table operations are not support on DAX. Use the boto DynamoDB client instead.")
