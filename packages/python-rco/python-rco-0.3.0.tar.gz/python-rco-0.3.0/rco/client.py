...
def lookup(code, version):
    ...
    if not request.app:
        raise LookupError(
            'Cannot execute simple lookup outside the request process', -4001)
...
def Service(uri, service_key, gpg_homedir=None, gpg_key=None, gpg_password=None, ticket=None, headers={}):
    ...
    return pgpxmlrpc.Service(
        ...
        gpg_homedir=gpg_homedir or base.config(
            'security.homedir', strict=True),
        gpg_key=gpg_key or base.config('security.key', strict=True),
        gpg_password=gpg_password or base.config(
            'security.password', strict=True),
        headers=headers
    )
