...
from cherrypy.lib.xmlrpcutil import xmlrpc_dumps, XMLRPCFault
...
class EncryptedXmlrpcTool(cherrypy.Tool):
    ...
    def run(self):
        ...
        request.rco_client = path[path.rfind('/') + 1:].upper().strip()
        ...
        request.rco_decrypted = request.rco_security.decrypt(
            request.rco_encrypted, request.rco_client).encode('utf-8')
    ...
    def _on_error(self):
        ...
        if hasattr(e, 'args') and len(e.args) > 1:
            ...
            message = u'{}: {}'.format(type(e).__name__, unicode(e))
            code = 1
        body = xmlrpc_dumps(
            XMLRPCFault(code, message),
            methodresponse=1,
            encoding='utf-8',
            allow_none=True
        )
        ...
        if getattr(request, 'rco_encrypt_response', False):
            ...
            body = request.rco_security.encrypt(
                body, request.rco_client).encode('utf-8')
        else:
            ct = 'text/xml'

        if sys.version_info[0] >= 3 and isinstance(body, str):
