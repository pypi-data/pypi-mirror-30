                ...
                for row in self.players_data:
                        for listfilerow in listfile_rows:
                                if listfilerow['IDFIDE'] != ' ' and listfilerow['IDFIDE'] != '' and listfilerow['IDFIDE'] != '0' and listfilerow['IDFIDE']==row['IDFIDE']:
                                        ...
                                        if len(listfilerow['IDNAT']) > 2 and idfeda == 1:
                                                if len(row['IDNAT']) <=2:
                                                    newidnat.append(listfilerow['NAME']+" (by fidecode) ") 
                                        ...
                                        if listfilerow['TITLE'] != ' ':
                                                row['TITLE']=listfilerow['TITLE']
                                        updated += 1
                                ...
                                elif listfilerow['NAME']==row['NAME']:
# and len(row['ELOFIDE']) <= 3 and len(row['IDNAT']) <= 3:
                                        #print('Updating data from: '+row['NAME']+'...')
                                        ...
                                        if len(listfilerow['IDNAT']) > 2 and idfeda == 1:
                                                if len(row['IDNAT']) <= 2:
                                                    newidnat.append(listfilerow['NAME']+" (byname) ")
                                                row['IDNAT']=listfilerow['IDNAT']
                                                if alreadyupdated == False:
                                                    updated += 1
                                                    alreadyupdated = True
                ...
                with open(os.path.join(os.getcwd(), "arbitools-report.log"), 'a') as logfile:
                        ...
                        logfile.write("Be careful with:"+str(becareful)+". They have been updated by their names. There may be errors.")
                        logfile.write("\nWrote new IDNAT fields:"+str(newidnat)+". They did not have an IDNAT before.")
                        logfile.write("\n"+str(errors))
                        #print(row)#testing
