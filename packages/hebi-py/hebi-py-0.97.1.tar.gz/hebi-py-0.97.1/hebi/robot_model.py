...
class RobotModel(__UnmanagedObject):
  ...
  def get_frame_count(self, frame_type):
    ...
    """
    from ._internal.kinematics import parse_frame_type
    return self.__get_frame_count(parse_frame_type(frame_type))
  ...
  def add_joint(self, joint_type, combine=False):
    ...
    """
    from ._internal.kinematics import parse_joint_type
    self.__add_joint(parse_joint_type(joint_type), combine)
  ...
  def get_end_effector(self, positions):
    ...
    *Currently, kinematic trees are not fully supported -- only kinematic
    ...
    num_frames = self.__get_frame_count(FrameTypeOutput)
    if (num_frames == 0):
  ...
  def solve_inverse_kinematics(self, initial_positions, *objectives):
    ...
    if not is_finite(initial_positions):
  ...
  def masses(self):
    ...
    count = self.__get_frame_count(FrameTypeCenterOfMass)
    # edge case
    if (count == 0):
      return np.empty(0, np.float64)

    masses = np.empty(count, dtype=np.float64)
    hebiRobotModelGetMasses(self, masses.ctypes.data_as(POINTER(c_double)))
    return masses
...
def custom_objective(num_errors, func, user_data=None, weight=1.0):
  ...
  class Objective(_ObjectiveBase):
    ...
    def __callback(self, user_data, num_positions, c_positions, c_errors):
      ...
      with ffi calls.
      """
      positions = np.ctypeslib.as_array(c_positions, (num_positions,))
    ...
    def __init__(self, num_errors, func, user_data, weight):
      ...
      from ctypes import CFUNCTYPE, c_size_t, c_void_p, c_double, POINTER
      self._c_func = CFUNCTYPE(None, c_void_p, c_size_t, POINTER(c_double), POINTER(c_double))(self.__callback)

      def impl(internal):
        from ._internal.raw import (hebiIKAddObjectiveCustom,
                                    StatusSuccess)
        res = hebiIKAddObjectiveCustom(internal, self._weight,
                                       self._num_errors,
