...
from keyup.script_utils import stdout_message, bool_assignment, debug_mode
...
class UpdateConfig():
    ...
    def update(self, cfg, debug=False):
        ...
        if bool_assignment(response_q0):
            ...
            if self.parameters['enable_logging'] not in (True, False):
                self.parameters['enable_logging'] = converge_answer(question1, [True, False], answer1)
            ...
            if self.parameters['enable_logging']:
                ...
                if self.parameters['log_mode'] == 'FILE':
                    default = self.parameters['log_path'] or user_home + '/logs/keyup.log'
                    msg = ('\n\t' + ACCENT + 'Log file location?' + Colors.RESET + ' [%s] ' % default)
                    ...
                    if '.' in self.parameters['log_path'].split('/')[-1]:
                        self.parameters['log_filename'] = self.parameters['log_path'].split('/')[-1]
                    else:
                        stdout_message(
                            message='Log file set to: %s/keyup.log' % self.parameters['log_path'],
                            tabspaces=16
                            )
                        self.parameters['log_path'] = self.parameters['log_path'] + '/keyup.log'
            ...
            q4a = self.print_header('q4'); print(q4a)
            ...
            if self.parameters['enable_keybackup'] not in (True, False):
                self.parameters['enable_keybackup'] = converge_answer(q4a + q4b, response_q4, [True, False])
            print(Colors.RESET + '\n\tEnable key backup set to: %s\n' % self.parameters['enable_keybackup'])
            if self.parameters['enable_keybackup']:
                default = self.parameters['backup_location'] or user_home + '/Backup'
                msg = ('\n\t' + ACCENT + 'Backup directory location?' + Colors.RESET + ' [%s] ' % default)
                # test location to ensure write compatible
                ...
                if '.' in self.parameters['backup_location'].split('/')[-1]:
                    extraneous_file = self.parameters['backup_location'].split('/')[-1]
                    self.parameters['backup_location'] = '/'.join(self.parameters['backup_location'].split('/')[:-1])
                    stdout_message(
            ...
            print(self.print_header('closing_footer'))
            sys.exit(exit_codes['E_USER_CANCEL']['Code'])
        # process update
    ...
    def config_directory(self, cfg):
        ...
        user_home = seed_config['PROJECT']['HOME']
        L1_path = user_home + '/' + seed_config['PROJECT']['CONFIG_DIR']
        L2_path = L1_path + '/' + seed_config['PROJECT']['CONFIG_SUBDIR']
    ...
    def print_header(self, header):
        ...
        title = ('|    ' + PACKAGE + ' Local Configuration Setup      |')
        ...
        ____________________  """ + closing_title + """  ______________________
