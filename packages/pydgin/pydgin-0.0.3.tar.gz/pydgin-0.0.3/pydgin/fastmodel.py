...
class FastPopulation:
    def __init__(self, *, races: np.ndarray, native_langs: np.ndarray, n_langs: int, lr_coordinated: float,
                 lr_uncoordinated: float,
                 select: typing.Callable[['FastPopulation'], typing.Tuple[np.ndarray, np.ndarray]], racists: bool,
                 promotion_age=730):
        self.promotion_age = promotion_age  # Default to two years because right now I can't be arsed to think of a better time
        self.racists = racists
        ...
        self.select = select
        self.ages = np.zeros_like(self.races, dtype=np.int)
    ...
    def merge(self, other: 'FastPopulation'):
    ...
    def replace_merge(self, p1, p2):
        ...
        self.ages = np.concatenate((p1.ages, p2.ages))
        assert self.langs.shape[0] == self.races.shape[0] == self.native_languages.shape[0] == self.ages.shape[0]

    def pass_the_time(self):
        assert self.langs.shape[0] == self.races.shape[0] == self.native_languages.shape[0] == self.ages.shape[0]
        self.ages += 1
        if self.promotion_age is None:
            return
        old_idx = self.ages > self.promotion_age
        black_idx = self.races == black
        self.races[np.logical_and(old_idx, black_idx)] = mixed
    ...
    def kill(self, lang_index, lang_delta):
        ...
        self.ages = np.delete(self.ages, kill_idx, axis=0)
...
@jit(nopython=True)
def update(population, races, idx, my_lang, your_lang, lrc, lru, racists):
    """ Fast update a single individual a-la the java version. """
    if racists and races[idx] == european:
        return
...
@jit(nopython=True)
def chatter(population, races, us_idx, them_idx, lr_coordinated, lr_uncoordinated, racists):
    for me_idx, you_idx in zip(us_idx, them_idx):
        my_lang = speak(population[me_idx])
        your_lang = speak(population[you_idx])
        update(population, races, me_idx, my_lang, your_lang, lr_coordinated, lr_uncoordinated, racists)
        update(population, races, you_idx, your_lang, my_lang, lr_coordinated, lr_uncoordinated, racists)


class Factory:
    def __init__(self, *, lr_coordinated: float, lr_uncoordinated: float, select, racists, features, non_white,
                 rise_ranks, promotion_age):
        self.racists = racists
        self.select = select
        self.lr_coordinated = lr_coordinated
        self.lr_uncoordinated = lr_uncoordinated
    ...
    def __call__(self, *, races, native_langs) -> FastPopulation:
        return FastPopulation(
            ...
            select=self.select,
            racists=self.racists,
            promotion_age=self.promotion_age,
        )

    def get_empty(self) -> FastPopulation:
        return self(
            races=np.empty((0,), dtype=np.int),
            native_langs=np.empty((0,), dtype=np.int),
        )

    def make_n(self, n, group_index) -> FastPopulation:
...
class FastModel:
    ...
    def run(self, feature_columns):
        ...
        rise_ranks = self.plantation_epoch is None
        agent_factory = Factory(lr_coordinated=self.lr_coordinated,
                                lr_uncoordinated=self.lr_uncoordinated,
                                select=self.select,
                                racists=self.racists,
                                features=feature_columns,
                                non_white=black if rise_ranks else mixed,
                                rise_ranks=rise_ranks,
                                promotion_age=self.promotion_age,
                                )
        ...
        for epoch_popdeltas, days_in_epoch in zip(self.population_deltas, self.epoch_lengths):
            ...
            if not rise_ranks and agent_factory.non_white != mixed and days_passed >= self.plantation_epoch:
                if self.verbosity >= Verbosity.debug:
                    print(f'Switching to generating non-whites as black after {days_passed} days')
            ...
            if self.verbosity >= Verbosity.debug:
                print(
                    f'Races: {np.count_nonzero(population.races == european):5d} white, '
                    f'{np.count_nonzero(population.races == black):5d} black, '
                    f'{np.count_nonzero(population.races == mixed):5d} mixed'
                )
            ...
            for _ in range(days_in_epoch):
                population.pass_the_time()
                population.chat_amongst_yourselves()
        return analysis.Distribution(population.langs)


test_times = np.asarray([10, 20, 30, 20, 10])
test_popdeltas = np.asarray([[10, 0, 0],
                             [-5, 1, 3],
                             [5, 9, 7],
                             [-1, -1, -1],
                             [0, 0, 0]])
