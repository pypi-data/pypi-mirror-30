...
//
  ...
  assign_stmt : testlist ("=" (yield_expr|testlist))+ ;
  ...
  print_stmt : PRINT (print_into ",")? test ("," test)+? ","?
    | PRINT print_into?
    ;
...
// compound flow statements
  ...
  with_stmt : WITH test (AS expr)? ("," test (AS expr)?)* COLON suite;    // Too permissive
  ...
  except_stmt : (EXCEPT (test ((AS|",") test)?)? COLON suite);
...
// XXX Overly permissive? (maybe gen_for belongs in arglist)
  ...
  @testlist_star : testlist_star? "," test ;

  varargslist : vararg ("," vararg)+?
            (("," args_decl)? ("," kwargs_decl)?
            |","
            )
    | args_decl ("," kwargs_decl)?
    | kwargs_decl
...
@listmaker : test (gen_for | ("," test)+? ","?) ;
stunted_testlist : stunted_test (("," stunted_test)+ ","?)? ;

dict : LBRACE dictmaker? RBRACE ;
@dictmaker : dict_item (gen_for | ("," dict_item)+? ","?) ;
