...
def check_upload_file(file_path):
    ...
    -------
    bool
        True if correct extension, False othersise.
    """
...
class Project:
    ...
    def download_file(self, container_id, file_name, local_filename=False,
        ...
        content = self._account._send_request("file_manager/download_file",
                                              params, stream=True,
    ...
    def delete_subject(self, subject_name):
        ...
        -------
        bool
            True if correctly delted, False otherwise.
        """
        logger = logging.getLogger(logger_name)
        if not self.check_subject_name(subject_name):
            logger.error(
                "Attempt to delete an inexisting subject: '{}'.".format(
                    subject_name))
            return False
        ...
        if content.get("success", False):
            logger.info("Subject '{}' successfully deleted.".format(
                subject_name))
            return True
        else:
            logger.error(
                "Subject '{}' could not be deleted.".format(subject_name))
            return False
    ...
    def upload_file(self, file_path, subject_name, ssid="", date_of_scan="",
                    ...
                    input_data_type="mri_brain_data:1.0",
                    add_to_container_id=0, chunk_size=2 ** 8):
        """
        ...
        chunk_size : int
            ...
            a power of 2: 2**x. Default value of x is 8 (chunk_size = 256)
