...
class CanvasMainWindow(QMainWindow):
    ...
    def setup_actions(self):
        ...
        self.reload_last_action = \
            QAction(self.tr("重新加载最后的工作流"), self,
    ...
    def setup_menu(self):
        ...
        self.recent_menu = QMenu(self.tr("打开最近的"), file_menu)
        ...
        for title, filename in self.recent_schemes:
            action = QAction(title or self.tr("未命名"), self,
                             toolTip=filename)
        ...
        if sys.platform == "darwin":
            ...
            self.window_menu = QMenu(self.tr("窗口"), self)
    ...
    def open_scheme(self):
        ...
        filename, _ = QFileDialog.getOpenFileName(
            self, self.tr("打开工作流文件"),
            start_dir, self.tr("工作流 (*.ows)"),
        )
    ...
    def ask_clear_report(self):
        ...
        msgbox = QMessageBox(QMessageBox.Question,
                             "报告", "报表窗口中有未保存的更改。",
                             parent=self,
                             informativeText="保存报告？")
        # Cancel must have AcceptRole, otherwise Os X reorders the buttons
        save = msgbox.addButton("保存并清除", QMessageBox.AcceptRole)
        msgbox.addButton("清除", QMessageBox.DestructiveRole)
        keep = msgbox.addButton("保持", QMessageBox.AcceptRole)
        cancel = msgbox.addButton("取消", QMessageBox.RejectRole)
        msgbox.exec()
    ...
    def ask_save_changes(self):
        ...
        selected = message_question(
            self.tr('您想保存对工作流"%s"所做的更改吗？')
            % title,
            self.tr("保存更改？"),
            self.tr("如果您不保存它们，您的更改将丢失。"),
    ...
    def save_scheme_to(self, scheme, filename):
        ...
        try:
            ...
            if ex.errno == 2:
                ...
                message_warning(
                    self.tr('无法保存工作流"%s"。路径不存在。') % title,
                    title="",
                    informative_text=self.tr("选择另一个位置。"),
                    parent=self
                )
            elif ex.errno == 13:
                message_warning(
                    self.tr('无法保存工作流"%s"。您没有写权限。') % title,
                    title="",
                    informative_text=self.tr(
                        "更改文件系统权限或选择其他位置。"),
                    parent=self
            ...
            log.error("Error saving %r to %r", scheme, filename, exc_info=True)
            message_critical(
                self.tr('试图保存工作流"%s"到"%s"时出错。') % (title, basename),
                title=self.tr("%s保存错误") % basename,
                exc_info=True,
                parent=self
            )
            return False
    ...
    def welcome_dialog(self):
        ...
        dialog.setWindowTitle(self.tr("欢迎使用BIS-Miner“雅典娜”数据挖掘系统"))
    ...
    def scheme_properties_dialog(self):
        ...
        dialog.setWindowTitle(self.tr("工作流信息"))
    ...
    def open_addons(self):
        ...
        if not have_install_permissions():
            QMessageBox(QMessageBox.Warning,
                        ...
                        parent=self).exec_()
        dlg = AddonManagerDialog(self, windowTitle=self.tr("扩展"))
