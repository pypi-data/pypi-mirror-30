...
class SchemeEditWidget(QWidget):
    ...
    def __init__(self, parent=None, ):
        ...
        self.__widgetMenu = QMenu(self.tr("&小组件"), self)
    ...
    def __onSelectionChanged(self):
        ...
        if len(nodes) > 1:
            self.__openSelectedAction.setText(self.tr("打开全部"))
        else:
            self.__openSelectedAction.setText(self.tr("打开"))

        if len(nodes) + len(annotations) > 1:
            self.__removeSelectedAction.setText(self.tr("移除全部"))
        else:
            self.__removeSelectedAction.setText(self.tr("移除"))
