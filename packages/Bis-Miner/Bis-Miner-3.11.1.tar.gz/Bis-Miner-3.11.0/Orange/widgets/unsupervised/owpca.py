...
class OWPCA(widget.OWWidget):
    ...
    class Outputs:
        transformed_data = Output("转换后数据", Table)
        components = Output("组件", Table)
        pca = Output("主成分分析", PCA, dynamic=False)
    ...
    class Error(widget.OWWidget.Error):
        no_features = widget.Msg("至少需要1个特性。")
        no_instances = widget.Msg("至少需要1个数据实例。")
        sparse_data = widget.Msg("不支持稀疏数据。")
