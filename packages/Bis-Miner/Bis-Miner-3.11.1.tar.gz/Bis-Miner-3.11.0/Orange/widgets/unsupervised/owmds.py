...
class OWMDS(OWWidget):
    name = "多维尺度（MDS）"
    description = "把由距离矩阵生成的多维尺度投射成二维数据"
    icon = "icons/MDS.svg"

    class Inputs:
        data = Input("数据", Orange.data.Table, default=True)
        distances = Input("距离", Orange.misc.DistMatrix)
        data_subset = Input("数据子集", Orange.data.Table)
    ...
    class Error(OWWidget.Error):
        not_enough_rows = Msg("输入数据至少需要2行。")
        matrix_too_small = Msg("输入矩阵必须至少2x2")
        no_attributes = Msg("数据没有属性")
        mismatching_dimensions = \
            Msg("数据和距离尺寸不匹配。")
        out_of_memory = Msg("内存不足")
        optimization_error = Msg("优化过程出错\n{}")
