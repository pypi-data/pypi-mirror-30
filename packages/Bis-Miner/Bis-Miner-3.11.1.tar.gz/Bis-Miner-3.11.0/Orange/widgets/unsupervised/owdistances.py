...
class OWDistances(OWWidget):
    ...
    class Error(OWWidget.Error):
        no_continuous_features = Msg("没有数字特性")
        dense_metric_sparse_data = Msg("{} 需要稠密数据。")
        distances_memory_error = Msg("没有足够的内存")
        distances_value_error = Msg("计算出错：\n{}")

    class Warning(OWWidget.Warning):
        ignoring_discrete = Msg("忽略分类特性")
        imputing_data = Msg("缺失值被估算")
