...
class OWLinearProjection(widget.OWWidget):
    ...
    class Warning(widget.OWWidget.Warning):
        no_cont_features = widget.Msg("绘图需要数字特征")
        not_enough_components = widget.Msg("输入预测的组件少于2个。")
        trivial_components = widget.Msg(
            "PCA的所有组件都是不重要的（解释0方差）。 "
            "输入数据是常量（或接近常数）。")

    class Error(widget.OWWidget.Error):
        proj_and_domain_match = widget.Msg("预测和数据域不匹配")
        no_valid_data = widget.Msg("由于无效数据而没有预测")
