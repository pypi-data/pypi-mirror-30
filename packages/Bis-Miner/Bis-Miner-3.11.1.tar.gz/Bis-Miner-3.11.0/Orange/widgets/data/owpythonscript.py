...
class OWPythonScript(widget.OWWidget):
    name = "Python脚本"
    description = "编写python脚本，在输入数据或模型上运行"
    ...
    class Inputs:
        ...
        learner = Input("学习器", Learner, replaces=["in_learner"],
                        default=True, multiple=True)
        classifier = Input("分类器", Model, replaces=["in_classifier"],
                           default=True, multiple=True)
        object = Input("对象", object, replaces=["in_object"],
                       default=False, multiple=True)

    class Outputs:
        data = Output("数据", Table, replaces=["out_data"])
        learner = Output("学习器", Learner, replaces=["out_learner"])
        classifier = Output("分类器", Model, replaces=["out_classifier"])
        object = Output("对象", object, replaces=["out_object"])
