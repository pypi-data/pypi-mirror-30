...
import itertools  # module for iteractive routines
from .helpers import *   
...
def data_classification(data_frame, wavelength_tag = 'wlength', frequency_tag = 'freq', 
                        ...
                        class_tags = ['polarization', 'ring_bus', 'transverse_mode'], pml_thre = 0.5,
                        tm_thre = 1.0, bus_thre = 1.0):
...
def dispersion_calculation(data_frame, frequency_tag = 'freq', wavelength_tag = 'wlength', 
                           neff_tag = 'neff', wlength0 = None, Pn = 1e0):
...
#    wlength0 = 0.9e-6
    ...
    norm_gain = calculate_gain(Dbeta, Pn)
    ## outputs
    ...
    return data_frame
###
def dispersion_analysis(data_frame, list0, frequency_tag = 'freq', wlength0 = None, Pn = 1e0):
    ## list of columns
    ...
    data_frame = data_frame.groupby(list0, as_index = False).apply(dispersion_calculation, wlength0 = wlength0, Pn = 1e0)
    return data_frame
##
...
def create_data_frame(file_list, dict_sub, Nhline = 5, sep = ','):
    ...
    for filename in file_list:
        with open(filename) as myfile:
            head = list(itertools.islice(myfile, Nhline))  # read only the first Nhline lines
