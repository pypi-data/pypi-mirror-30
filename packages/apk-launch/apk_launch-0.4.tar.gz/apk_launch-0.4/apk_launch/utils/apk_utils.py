...
def init_directory(args, rootdir, data, count=0, gradleversion=GRADLE_VERSION):
    """
    ...
    Args:
        ...
        gradleversion: str
            gradle version to use (defaults to '4.6')
    ...
    if args['dirtype'] in ['gradle', 'ant', 'maven', 'android']:
        ...
        cmd = 'gradle wrapper --gradle-version={0}'.format(gradleversion)
        ...
        androidcmd = 'android'
        cmd = ('{0} create project --gradle --gradle-version 0.11.+ --path . '
               '--activity {1} -t 2 -n {2} -k {3}'
...
def build(args, rootdir, gradlecache='~/Desktop/gradle-cache'):
    """
    Build project.
    ...
    if args['dirtype'] == 'gradle':
        cmd = 'gradle --project-cache-dir {0} assembleDebug'.format(gradlecache)
...
def install(args, rootdir, gradlecache='~/Desktop/gradle-cache'):
    """
    Install project.
    ...
    if apkpaths:
        ...
        cmd = './gradlew --project-cache-dir {0} installDebug'.format(gradlecache)
    elif args['dirtype'] == 'ant':
        cmd = 'ant install'
    elif args['dirtype'] == 'maven':
...
def test(args, rootdir, gradlecache='~/Desktop/gradle-cache'):
    """
    Test project.
    ...
    """
    cmd = './gradlew --project-cache-dir {0} cAT'.format(gradlecache)
...
def clean(args, rootdir, gradlecache='~/Desktop/gradle-cache'):
    """
    Clean project.
    ...
    """
    if args['dirtype'] == 'gradle':
        cmd = './gradlew --project-cache-dir {0} clean'.format(gradlecache)
    elif args['dirtype'] == 'ant':
        cmd = 'ant clean'
    elif args['dirtype'] == 'maven':
        cmd = 'mvn clean'
    ...
    gradledir = '{0}/.gradle'.format(rootdir)

    if os.path.isdir(gradledir):
        shutil.rmtree(gradledir)


def uninstall(args, rootdir, gradlecache='~/Desktop/gradle-cache'):
    """
    Uninstall project.
    ...
    """
    if args['dirtype'] == 'gradle':
        cmd = './gradlew --project-cache-dir {0} uninstallAll'.format(gradlecache)
    elif args['dirtype'] == 'ant':
        cmd = 'ant uninstall'
    elif args['dirtype'] == 'maven':
        cmd = 'mvn uninstall'
