...
from apk_launch.utils import hbase_utils
...
CMD_IDS = [
    ('summary', apkanalyzer.APK_SUMMARY),
    ('file_size', apkanalyzer.APK_FILESIZE),
    ('download_size', apkanalyzer.APK_DOWNLOADSIZE),
    ('min_sdk', apkanalyzer.APK_MINSDK),
    ('target_sdk', apkanalyzer.APK_TARGETSDK),
    ('permissions', apkanalyzer.APK_PERMISSIONS),
    ('packages', apkanalyzer.APK_DEXPACKAGES),
]
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        prog=__file__, description='Analyze Android apk file',
    )
    ...
    parser.add_argument('-o', '--outdir', help='Directory to output apk data into',
                        default='.')

    parser.add_argument('-d', '--dryrun', help='Do not save trees to file',
                        action='store_true')

    parser.add_argument('-v', '--verbose', help='Print debug information',
                        action='store_true')
...
def count_states(items):
    ...
    """
    defined = apkanalyzer.match_state(items, 'd')
    referenced = apkanalyzer.match_state(items, 'r')
...
def apk_stats(apkfile, category):
    ...
    for cmd_str, cmd_id in CMD_IDS:
        cmd = apkanalyzer.apk_command(cmd_id, {'apk': apkfile})
        cmd.start()
    ...
    for cmd_str, cmd in tqdm(cmds, leave=False):
        ...
        if cmd_str == 'packages':
            ...
            classes, fields, methods, _ = apkanalyzer.parse_dex_packages(log.split('\n'))
