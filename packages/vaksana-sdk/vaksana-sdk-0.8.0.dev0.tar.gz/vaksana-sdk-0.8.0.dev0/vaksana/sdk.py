...
class vaksana(object):
    ...
    '''
    def __init__(self, configfile, force_clean=False):
        self._proto_urls = {}
        ...
        ''' Registers the domain of the client & gets token in return for
            subsequent calls '''
        token = self.register_domain(force_clean)
    ...
    '''
    def register_domain(self, force_clean):
    ...
    def cleanup(self, proto_url=None):
        global send_keepalive_signal
        if proto_url:
            (_t, _a, _stub) = self._proto_urls[proto_url]
            _stub.Cleanup(self.void)
            ...
            del self._proto_urls[proto_url]
        else:
            while len(self._proto_urls):
                for proto_url, (_t, _a, _stub) in self._proto_urls.iteritems():
                    self.cleanup(proto_url=proto_url)
                    break

        if len(self._proto_urls) == 0:
            send_keepalive_signal = False
