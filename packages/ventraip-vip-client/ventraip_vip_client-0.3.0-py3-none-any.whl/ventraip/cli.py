...
@click.option('--allow-updates/--no-allow-updates', default=True)
@click.pass_obj
def add(vip_client: VipClient, hostname, domain_name, destination, ttl, record_type, allow_updates):
    domain = vip_client.domain(domain_name=domain_name)
    dns_record = vip_client.dns_record(hostname, domain_name, record_type)

    if dns_record and allow_updates:
