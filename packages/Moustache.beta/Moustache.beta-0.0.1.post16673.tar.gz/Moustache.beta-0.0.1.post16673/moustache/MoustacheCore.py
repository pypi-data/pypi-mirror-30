...
class MoustacheCore:
    ...
    def fusion(self, app, template_file_path, the_json, gabarit_file_mapping, annexe_file_mapping):
        try:
        ...
        if the_json.get(APIDefinition.ANNEXE_MAPING_JSON_KEY):
            ...
            for annexe_key, annexe_value in the_json[APIDefinition.ANNEXE_MAPING_JSON_KEY].items():
                ...
                if helper.search_and_select(annexe_key):
                    annexe_filepath = self.transformAnnexeInPDF(annexe_file_mapping[annexe_value])
    ...
    def transformAnnexeInPDF(self, annexe_filepath):
