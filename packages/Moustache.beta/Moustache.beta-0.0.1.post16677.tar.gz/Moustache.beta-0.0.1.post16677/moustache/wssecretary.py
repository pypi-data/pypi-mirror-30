...
def validate():
    ...
    temp_template_file = FileRetriever.retrieve(app, APIDefinition.TEMPLATE_FILE_FORM_NAME)
...
def parse():
    ...
    gabarit_file_mapping = FileRetriever.retrieve_multiple(app, APIDefinition.GABARIT_FORM_NAME, '.odt')

    annexe_file_mapping = FileRetriever.retrieve_multiple(app, APIDefinition.ANNEXE_FORM_NAME, '.pdf')
    ...
    if the_json.get(APIDefinition.GABARIT_MAPING_JSON_KEY):
        for gabarit_key, gabarit_value in the_json[APIDefinition.GABARIT_MAPING_JSON_KEY].items():
            if gabarit_value not in gabarit_file_mapping:
                raise InvalidUsage("The file %s defined in %s is not present"%(gabarit_value,APIDefinition.GABARIT_MAPING_JSON_KEY))
    ...
    if the_json.get(APIDefinition.ANNEXE_MAPING_JSON_KEY):
        ...
        for annexe_key, annexe_value in the_json[APIDefinition.ANNEXE_MAPING_JSON_KEY].items():
            if annexe_value not in annexe_file_mapping:
                raise InvalidUsage("The file %s defined in %s is not present"%(annexe_value,APIDefinition.ANNEXE_MAPING_JSON_KEY))
