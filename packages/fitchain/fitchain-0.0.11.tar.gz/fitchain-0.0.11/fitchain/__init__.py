...
class Runtime:
    ...
    def load(self, dataset_id):
        ...
        """
        return self.project.load(dataset_id)

    def store_train_params(self, **kwargs):
        if self.result_dir.is_dir():
            for key, val in kwargs.items():
                joblib.dump(val, '/data/out/train_param_' + key + '.pkl', True)

    def store_validate_params(self, **kwargs):
        if self.result_dir.is_dir():
            for key, val in kwargs.items():
                joblib.dump(val, '/data/out/validate_param_' + key + '.pkl', True)

    def store_model(self, model):
        if self.result_dir.is_dir():
            joblib.dump(model, '/data/out/model.pkl', True)
