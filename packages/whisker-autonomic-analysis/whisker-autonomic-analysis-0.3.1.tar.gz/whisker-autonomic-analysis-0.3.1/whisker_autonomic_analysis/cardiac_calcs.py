...
-------------------------------------------------------------------------------
(1) Re standard deviations: us versus Kubios
-------------------------------------------------------------------------------

R

    x <- c(1, 2, 3, 4, 5)
    var(x)  # 2.5  -- SAMPLE VARIANCE
    sd(x)  # 1.581139  -- SAMPLE SD

    sum((x - mean(x))^2) / length(x)  # population variance
...
Python
    ...
    x.var()  # 2.0 -- POPULATION VARIANCE
    x.var(ddof=0)  # 2.0  -- POPULATION VARIANCE
    x.var(ddof=1)  # 2.5  -- SAMPLE VARIANCE
    x.std()  # 1.4142135623730951  -- POPULATION SD
    x.std(ddof=0)  # 1.4142135623730951  -- POPULATION SD
    x.std(ddof=1)  # 1.5811388300841898  -- SAMPLE SD

https://stackoverflow.com/questions/26269512/python-numpy-var-returning-wrong-values  # noqa
https://docs.scipy.org/doc/numpy/reference/generated/numpy.var.html

-------------------------------------------------------------------------------
(2) Re discrepancy with HRV toolkit: is the HRV toolkit missing a beat?
-------------------------------------------------------------------------------

    My 'SDNN_ms': 14.285714285714286,  # matches Kubios SDNN value of 14.3
    ... my code with "--calcmethod POP_SD" code gives 14.142135623730951
    ... but also: 'HRVToolkit_SDNN_s': 0.0144338
        ... which matches neither.
    In R, sd(ibi) gives 14.28571.
    The HRV toolkit code is
        https://physionet.org/tutorials/hrv-toolkit/HRV.src/statnn.c
    which uses:
        printf("SDNN = %g\n", sqrt((sum2 - sum*sum/totnn) / (totnn-1)));
    Right. So assuming totnn is the number of IBIs, this is trying for
    the sample variance, and the assumed equivalence is that
        [canonical R code] sum((x - mean(x))^2)
    is calculated by
        [HRV C code] sum2 - sum*sum/totnn
    Now we find, earlier,
        sum = rr;
        sum2 += rr*rr;
    so their calculation for variance
        (sum_of_squared_x - sum_of_x ^ 2 / n) / (n - 1)
    which we can show
        = [(n - 1) / n] * (sum_of_squared_x / n - (sum_of_x / n) ^ 2)
        = [(n - 1) / n] * [(1 / n) sum_of_squared_x - mean_of_x ^ 2]
    which is fair; see https://en.wikipedia.org/wiki/Variance .
    So perhaps the discrepancy is in totnn.

Compiling HRV toolkit quickly from source:

    cd ~/dev
    git clone https://github.com/kees/physionet-wfdb
    git clone https://github.com/kees/physionet-hrv
...
Now edit get_hrv (it's a shell script) to add the lines marked "RNC":

    ...
    NNSTATS=`cat foo.nnstat`
    echo "SHOWING INPUT TO statnn:"  # RNC
    cat foo.frr
    echo "HOW MANY LINES IN INPUT TO statnn?"  # RNC
    cat foo.frr | wc --lines  # RNC
    echo "SHOWING OUTPUT FROM statnn:"  # RNC
    cat foo.nnstat  # RNC
    ...
...
and edit statnn.c:
    ...
    ...
    printf("SDNN = %g\n", sqrt((sum2 - sum*sum/totnn) / (totnn-1)));
    printf("totnn = %d\n", totnn);  // RNC: ADD THIS LINE to see working
    ...

and re-run the make command, then create an RR list file and use

    ./get_hrv -R ~/tmp/my_rr_list.txt

What happens? We see that get_hrv takes a file like
...
and makes a temporary file (foo.frr) like this:
...
where:
    the first column is cumulative time (t),
    the second is RR interval (rr) FROM THE PREVIOUS TIME,
    and the third is an annotation (ann) meaning 'N' for ?normal beat.

Yes. The problem is that the HRV Toolkit ignores the first RR interval, and
calculates its NN intervals from that first one onwards only (see statnn.c);
so we will add a dummy beat to our output to compensate.
See
...
pNN50    = 0.00210704
    ...
    with tempfile.NamedTemporaryFile(mode='wt', dir=cfg.tempdir.name,
        ...
        if cfg.hrvtk_dummy_initial_beat:
            dummy_rr_interval = 9999
            print(dummy_rr_interval, file=rr_file)
...
class CardiacIndices(object):
    ...
    def __init__(self,
        ...
        if cfg.calcmethod == CalcMethod.POP_SD:  # wrong; older method
            ddof = 0  # "delta degrees of freedom"; 0 gives population variances/SDs  # noqa
        ...
        if cfg.hrv_builtin:
            ...
            # and it's a short name. More specifically, N-N intervals are
            # normal sinus beat to normal sinus beat (NN) interbeat intervals
            # [10].
        ...
        if test_verbose:
            ...
            log.critical(
                "CardiacIndices: IBI file for HRV toolkit (ms):\n"
                "{}".format("\n".join(str(x) for x in ibi_values_ms)))
            log.critical("CardiacIndices: IBI file (ms):\n" +
                         repr(ibi_values_ms))
            log.critical(
...
TEST_IBI_DATA = [
    ...
    # List of test dictionaries.
    {
        ...
        # ... my code in "--calcmethod POP_SD" code gives 14.142135623730951
        # ... but also: 'HRVToolkit_SDNN_s': 0.0144338
        #     ... which matches neither.
        # In R, sd(ibi) gives 14.28571.
        # The HRV toolkit code is
