...
class Service_DB():
    ...
    def find(self):
        ...
        if finded_count == 0:
            ...
            message = self.__data_types.toStr(filtered_count) + " documentos encontrados."
        ...
        if len(data) > 0:
            ...
            if len(insert_many_result) > 0:
                ...
                if inserted_count == 1:
                    ...
                    message = self.__data_types.toStr(inserted_count) + " documentos inseridos com sucesso."
    ...
    def update(self):
        ...
        if update_count == 0:
            ...
            message = self.__data_types.toStr(update_count) + " documentos atualizados com sucesso."
    ...
    def remove(self):
        ...
        if deleted_count == 0:
            ...
            message = self.__data_types.toStr(deleted_count) + " documentos removidos com sucesso."
    ...
    def __prepare_data_changes(self, data_update, data_active):
        ...
        for id_doc in documents:
            ...
            if len(doc_active.keys()) > 0:
                if self.__data_types.toStr(id_doc) not in data:
                    data[self.__data_types.toStr(id_doc)] = {}
                data[self.__data_types.toStr(id_doc)]["active"] = doc_active
            ...
            if len(doc_update.keys()) > 0:
                if self.__data_types.toStr(id_doc) not in data:
                    data[self.__data_types.toStr(id_doc)] = {}
                data[self.__data_types.toStr(id_doc)]["update"] = doc_update

        save_data_update = {
            "filter": {},
            "data": data,
            "created_time": self.__data_types.toStr(datetime.datetime.utcnow()),
    ...
    def __prepare_data_trash(self, data):
        ...
        for item in data:
            deleted_ids.append(self.__data_types.toStr(item["_id"]))
        ...
        def action(item_result):
            ...
            for item_field in item_result:
                ...
                if type(item_result[item_field]) == bson.ObjectId:
                    if "_id" == item_field:
                        result['id'] = self.__data_types.toStr(item_result[item_field])
                    else:
                        result[item_field] = self.__data_types.toStr(item_result[item_field])
                ...
                # Converte Decimal128 para Float
                    ...
                    result[item_field] = float(self.__data_types.toStr(item_result[item_field]))
