...
class Filter():
    ...
    # --------------------------------
    def __str__(self):
        return self.__data_types.toStr(self.__params)
    ...
    def __conflict(self):
        ...
        if len(fields_errors) > 0:
            self.__validate.add_error(3363569, "ERROR:: CAMPOS EM CONFLITOS" + self.__data_types.toStr(fields_errors))
    ...
    def add(self, field, value):
        if type(value) == str:
            filter = self.__data_types.toStr(field + "(" + value + ")")
        else:
        ...
        for param in list_params:
            ...
            if re.match(r"(?:([\s\S]*|\.(eq|gt|gte|lt|lte|ne|exists|not|in|nin|type))\(([\s\S]+)\))\Z", param, re.IGNORECASE):
                ...
                if re.match(r"^([a-z0-9_]+\.?)+$", param_match.group(1), re.IGNORECASE):
                    ...
                    self.__validate.add_error(87541545, "ERROR - Filtro inválido" + self.__data_types.toStr(param_match.group(1)))
                ...
                if not re.match(r"(?:[a-z0-9]+\([\s\S]+\)[^\+])\Z", param_match.group(2), re.IGNORECASE):
                    if re.match(r"^([a-z0-9_]+\.?)+$", param_match.group(1), re.IGNORECASE):
                        ...
                        self.__validate.add_error(523152, "ERROR FILTRO- de separador '|' nao encontrado" + self.__data_types.toStr(param_match.group(1)))
                        # print "ERROR - Filtro inválido " , param_match.group(1)
                else:
                    self.__validate.add_error(523152, "ERROR FILTRO- de separador '|' nao encontrado" + self.__data_types.toStr(param_match.group(1)))
                ...
                if not re.match(r"(?:[a-z0-9]+\([\s\S]+\)[^|])\Z", param_match.group(2), re.IGNORECASE):
                    if re.match(r"^([a-z0-9_]+\.?)+$", param_match.group(1), re.IGNORECASE):
                        ...
                        self.__validate.add_error(5454545454, "Filtro inválido" + self.__data_types.toStr(param_match.group(1)))
                        # print "ERROR - Filtro inválido ",  param_match.group(1)
                else:
                    self.__validate.add_error(523152, "ERROR FILTRO- de separador '|' nao encontrado" + self.__data_types.toStr(param_match.group(1)))
    ...
    def __values(self, value):
        ...
        # Verifica se é do tipo objectID
            ...
            try:
                value = bson.objectid.ObjectId(self.__data_types.toStr(value))
            except:
                pass
            ...
            for item in value:
                ...
                if re.match("^[0-9a-f]{24}$", item):
                    try:
                        ...
                        value[count] = bson.objectid.ObjectId(self.__data_types.toStr(item))
                        count = count + 1
                    except:
                        pass
