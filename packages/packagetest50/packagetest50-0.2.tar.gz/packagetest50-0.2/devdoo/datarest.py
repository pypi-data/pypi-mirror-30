...
class DataRest():
    ...
    def limit(self, max_limit=0):
        ...
        if self.__limit > 0:
            ...
            if limit > self.__max_limit:
                ...
                self.add_error(85545454, "O limite máximo (" + self.__data_types.toStr(error_max_limit) + ") foi excedido. Limite solicitado (" + self.__data_types.toStr(error_limit) + ").")
    ...
    def changed_time(self):
        self.__changed_time = self.__data_types.toStr(datetime.datetime.utcnow())
    ...
    def created_time(self):
        self.__created_time = self.__data_types.toStr(datetime.datetime.utcnow())
    ...
    # validateService
    # --------------------------------
    def validateService(self, service, service_id):
        print "SERVICE", service
        print "SERVICE_ID", service_id
        # TODO:: Implementar verificação de possivel conflito de unicode em situações não claramente definidas

        if self.__service != service or self.__data_types.toStr(self.__service_id) != self.__data_types.toStr(service_id):
            self.__validate.add_error("FALHA DE SERVICE", "Identificador do serviço nao valido:: " + self.__service + "!=" + service + "::" + self.__data_types.toStr(
                self.__service_id) + "!=" + self.__data_types.toStr(service_id))
            return False
        return True
    ...
    def decode(self, message):
        ...
        self.__database = "db" + self.__data_types.toStr(self.__database_id) + self.__service
