...
class Service(object):
    ...
    def action(self):
        ...
        if source in self.actions[method].keys():
            ...
            if hasattr(self, action["database_action"]) and self.__datarest.validateService(self.name, self.id):
                ...
                if action["database_action"] == "update":
                    ...
                    method(action["collection"], self.__getScheme(action["scheme"], self.schemes), self.__get_max_limit(action))
            else:
                ...
                self.__datarest.add_error_required("AAA ACTION INVALIDA",
                                                   source + " :: action nao existe no servico '" + self.name + "' da versao::" + " " + self.__data_types.toStr(
                                                       service_api["action"]) + "metodo::" + method)
        else:
    ...
    def __getScheme(self, scheme, list_schemes):
        result = {}
    ...
    def update(self, collection, scheme, list_scheme):
        ...
        if self.__is_valid_scheme(scheme):
            ...
            for item_scheme in scheme:
                obj_item_scheme = self.__getScheme(scheme[item_scheme], list_scheme)
