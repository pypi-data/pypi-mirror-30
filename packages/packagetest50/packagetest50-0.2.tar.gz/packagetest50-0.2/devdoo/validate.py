...
class Validate(object):
    ...
    def array(self, field, value):
        original_value = self.__data_types.toStr(value)

        # Converte para array
        value = self.__data_types.toArray(value)

        # Verifica se o resultado do tipo convertido é um array válido
        if self.__data_types.isArray(value):
            return value
        else:
            # Registra mensagem de erro no serviço de log e retona ao cliente
            self.add_error(4505845, "O valor do campo '" + field["field"] + "' nao pode ser '" + original_value + "', vazio ou null, utilize um valor do tipo array [\"a\", \"b\", 1, 2]")
    ...
    def boolean(self, field, value):
        ...
        value = self.__data_types.toBoolean(value)

        # Verifica se o resultado do tipo convertido é um boleano válido
        if self.__data_types.isBoolean(value):
            return value
        else:
            # Registra mensagem de erro no serviço de log e retona ao cliente
            self.add_error(4505845, "O valor do campo '" + field["field"] + "' nao pode ser '" + original_value + "', vazio ou null, utilize um valor entre true | false | 0 | 1")
            value = self.__check_value(value)
    ...
    def date(self, field, value):
        ...
        try:
            ...
            value = self.__register_error(field, value, "DATE")
        '''
        return value
    ...
    def enum(self, item_scheme, value):
        ...
        value = self.__data_types.toStr(value)

        if value == '__empty__' or value == '__none__':
            value = self.__clean_value(value)
            self.add_error(4505845, "O valor do campo '" + item_scheme[
                "field"] + "' nao pode ser '" + value + "', vazio ou null, utilize um valor entre: " + str(options))
            value = '__error__'

        if value not in options:
            self.add_error(4505845, "O valor do campo '" + item_scheme[
                "field"] + "' :: '" + value + "', deve conter na lista de opcoes: " + str(options))
            value = '__error__'
    ...
    # mixed
    # --------------------------------
    # Valida campo do tipo mixed, aceita qualquer valor no campo
    #
    # type: mixed
    #
    def mixed(self, field, value):
    ...
    def number(self, field, value):
        ...
        value = self.__data_types.toNumber(value)

        # Verifica se o resultado do tipo convertido é um boleano válido
        if self.__data_types.isNumber(value):
            # min_value / max_value
            ...
            # Registra mensagem de erro no serviço de log e retona ao cliente
            self.add_error(8545255, "O campo '" + field["field"] + "' nao pode ser '" + original_value + "', vazio ou null, utilize um valor entre -x até x.")
            value = self.__check_value(value)
    ...
    # number_decimal
    # --------------------------------
    # Valida campo do tipo númro decimal
    def decimal(self, field, value):
        original_value = self.__data_types.toStr(value)

        # Converte valor para número
        value = self.__data_types.toDecimal(value)

        # Verifica se o resultado do tipo convertido é um boleano válido
        if self.__data_types.isDecimal(value):
            pass
    ...
    def object(self, field, value):
        ...
        # Converte valor para número
        value = self.__data_types.toObject(value)

        # Verifica se o resultado do tipo convertido é objeto válido
        if not self.__data_types.isObject(value):
    ...
    def object_id(self, field, value):
        try:
            ...
            self.add_error(4505845, "O valor do campo '" + field[
                "field"] + "' nao pode ser '" + str(value) + "', vazio ou null, utilize um valor do tipo object_id")
            value = '__error__'
    ...
    def string(self, field, value):
        ...
        value = self.__modifier(field, value)
    ...
    def timestamp(self, field, value):
        if str(value) == 'now':
            value = bson.timestamp.Timestamp(datetime.datetime.utcnow(), 0)
            return value
        try:
            value = re.sub(r'[^0-9]+', '', str(value))
            value = bson.timestamp.Timestamp(int(value), 0)
        except:
            value = self.__clean_value(value)

            self.add_error(4505845, "O valor do campo '" + field[
                "field"] + "' nao pode ser '" + str(value) + "', vazio ou null, utilize um valor do tipo timestamp")
            value = '__error__'
        return value
    ...
    def __max_length(self, field, value):
        ...
        if max_length != None and len(value) > max_length:
            message = "O tamanho máximo permitido é (" + self.__data_types.toStr(max_length) + "), foi recebido (" + self.__data_types.toStr(
                len(value)) + ")."
    ...
    def __min_length(self, field, value):
        ...
        if min_length != None and len(value) < min_length:
            message = "O tamanho mínimo permitido é (" + self.__data_types.toStr(min_length) + "), foi recebido (" + self.__data_types.toStr(
                len(value)) + ")."
    ...
    def __max_value(self, field, value):
        ...
        if max_value and value > bson.Int64(long(max_value)):
            message = "O valor máximo permitido é (" + self.__data_types.toStr(max_value) + "), foi recebido (" + self.__data_types.toStr(
                value) + ")."
    ...
    def __min_value(self, field, value):
        ...
        if min_value and value < bson.Int64(long(min_value)):
            message = "O valor mínimo permitido é (" + self.__data_types.toStr(min_value) + "), foi recebido (" + self.__data_types.toStr(
                value) + ")."
