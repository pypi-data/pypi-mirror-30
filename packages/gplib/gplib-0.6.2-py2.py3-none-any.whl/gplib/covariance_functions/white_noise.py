...
class WhiteNoise(CovarianceFunction):
    ...
    def __init__(self, data):
        hyperparams = [
            OptimizableParameter(
                ...
                default_value=np.std(data['Y']) * 1e-03,
    ...
    def covariance(self, mat_a, mat_b=None, only_diagonal=False):
        ...
        if only_diagonal:
            return self.get_param_value('output_variance2') * \
                np.ones((len_a, 1))

        return self.get_param_value('output_variance2') * np.eye(len_a)
    ...
    def dk_dtheta(self, mat_a, mat_b=None, trans=False):
        ...
        if trans:
            dk_dn = self.get_hyperparam('output_variance2').grad_trans(dk_dn)
