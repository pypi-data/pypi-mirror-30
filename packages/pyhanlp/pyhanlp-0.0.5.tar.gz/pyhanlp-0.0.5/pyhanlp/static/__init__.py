...
PATH_DATA = os.path.join(STATIC_ROOT, 'data')
PATH_DATA_VERSION = os.path.join(PATH_DATA, 'version.txt')
...
def hanlp_installed_data_version():
    try:
        with open(PATH_DATA_VERSION) as f:
...
def update_hanlp():
    releases = hanlp_releases()
    if update_hanlp_jar(releases):
        print('HanLP jar has been updated to the latest version {}'.format(hanlp_installed_jar_versions()[0]))
    else:
        print('HanLP jar is already the latest version {}'.format(hanlp_installed_jar_versions()[0]))

    root = read_config()
    if root == STATIC_ROOT:
        if install_hanlp_data(hanlp_installed_jar_versions()[0], releases):
            print('HanLP data has been updated to the latest version {}'.format(releases[0][1]))
        else:
            print('HanLP data is already the latest version {}'.format(releases[0][1]))


def update_hanlp_jar(releases=None):
    if not releases:
        releases = hanlp_releases()
    if releases[0][0] in hanlp_installed_jar_versions():
        return False
    install_hanlp_jar()
    uninstall_hanlp_jar()
    return True
...
def install_hanlp_data(version, releases=None):
    if not releases:
        releases = hanlp_releases()
    for jar_version, data_version, data_url in releases:
...
if not hanlp_installed_data_path():
    install_hanlp_data(hanlp_installed_jar_versions()[0])

HANLP_JAR_PATH = hanlp_jar_path(hanlp_installed_jar_versions()[0])
HANLP_DATA_PATH = hanlp_installed_data_path()
