...
class MLRepositoryApi(RepositoryApi):
    ...
    def upload_pipeline_model_version(self, model_id, version_id, content_stream, query_param=None, req_id=None):
        if query_param is not None and req_id is not None:
            for key in query_param:
                if key == 'async' and query_param[key] == 'true':
                    query_parameters = {'async': 'true', 'requestId': req_id}
                else:
                    raise ValueError("Got an unexpected value '%s' for keyword argument '%s'" %(query_param[key], key))
        else:
            query_parameters = {}
    ...
    def download_artifact_content(self, artifact_content_href, presigned_url, accept='application/octet-stream'):
        ...
        try:
            ...
            raise ApiException(500, 'Internal server error during download content for: {}. Error message: {}'.format(artifact_content_href, ex.message))
        #else:
