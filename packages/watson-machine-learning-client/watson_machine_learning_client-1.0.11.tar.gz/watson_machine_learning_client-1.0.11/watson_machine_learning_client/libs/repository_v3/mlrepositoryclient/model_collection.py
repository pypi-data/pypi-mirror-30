...
from repository_v3.swagger_client.models import MlAssetsAsyncGetStatusOutput, MlAssetsModelSizeOutput
...
class ModelCollection:
    ...
    def _create_pipeline_model_version(self, model_artifact, query_param=None):
        print("in _create_pipeline_model_version")
    ...
    def _upload_pipeline_model_content(self, model_artifact, query_param=None):
        ...
        if(query_param is not None and asyncValue == 'true'):
                req_id = uid_generate(36)
                logger.info("Uploading content with async set to true with requestId: '%s'"%req_id)
                def upload_call():
                    self.repository_api.upload_pipeline_model_version(model_id, version_id, content_stream, query_param, req_id)
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                    ...
                    try:
                        future.result()
                        logger.info("After future.result future completed:%s for req_id '%s'"%(str(future.done()),req_id))
                ...
                status_url = '%s/content/status?requestId=%s' %(model_version_url, req_id)
    ...
    def get_status(self, url):
        req_id = url.split('?')[1].split('=')[1]
        polling_status = self.repository_api.get_async_status(url)
        if polling_status is not None:
            if(polling_status.statusMessage=="Running" or polling_status.statusMessage == "Completed"):
                return polling_status
            else:
                error_msg=("'Status' = 'ERROR', 'Message' = '%s'" %polling_status.statusMessage)
                return error_msg

        else:
            raise ValueError("Request with req_id ='%s' not found" %req_id)
