...
class TestWMLClientWithExperiment(unittest.TestCase):
    ...
    def test_09_1_monitor_logs(self):
        ...
        with CaptureOutput() as capturer:
            process = Process(target=run_monitor, args=(self.client, TestWMLClientWithExperiment.experiment_run_uid))
            process.start()
            process.join(timeout=90)
            process.terminate()
            self.assertTrue('training-' in capturer.get_text())
    ...
    def test_09_2_monitor_training_logs(self):
        ...
        with CaptureOutput() as capturer:
            process = Process(target=run_monitor_training, args=(self.client, training_uids[1]))
            process.start()
            process.join(timeout=90)
            process.terminate()
            self.assertTrue('training-' in capturer.get_text())
