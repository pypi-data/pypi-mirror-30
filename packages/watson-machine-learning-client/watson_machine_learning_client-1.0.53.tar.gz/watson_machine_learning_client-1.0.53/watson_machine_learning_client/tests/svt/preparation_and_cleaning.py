...
def clean_models(client, threashold_date=yesterday):
    ...
    for model_details in details['resources']: # TODO preserve model for Dinoop
        if model_details['metadata']['created_at'] < threashold_date.isoformat() and model_details['metadata']['guid'] not in '5cbbe4b2-d60d-4ad2-aa77-b65bbafe5c38':
...
def run_monitor(client, experiment_run_uid):
    client.experiments.monitor_logs(experiment_run_uid)


def run_monitor_metrics(client, experiment_run_uid):
    client.experiments.monitor_metrics(experiment_run_uid)


def run_monitor_training(client, training_run_uid):
