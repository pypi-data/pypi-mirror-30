...
def get_ml_token(watson_ml_creds):
    ...
    if response.status_code == 200:
        token = json.loads(response.text).get(u'token')
    else:
...
def refresh_ml_token(watson_ml_creds, token):
    import requests
    import json
    import base64
    from datetime import datetime

    refresh_margin = 30*60.0  # 30 minutes converted to seconds
    ...
    time_to_expire = (datetime.fromtimestamp(token_expire) - datetime.now()).total_seconds()

    if (time_to_expire < refresh_margin):
        response = requests.put(HrefDefinitions(watson_ml_creds).get_token_endpoint_href(),
                                json={'token': token},
