...
from watson_machine_learning_client.utils import get_type_of_details, INSTANCE_DETAILS_TYPE, STR_TYPE, str_type_conv, get_ml_token, refresh_ml_token
...
class WMLResource:
    def __init__(self, name, client):
        ...
        WMLResource._validate_type(client.instance_details, u'instance_details', dict, True)
        WMLResource._validate_type_of_details(client.instance_details, INSTANCE_DETAILS_TYPE)
        self._client = client
        self._wml_credentials = client.wml_credentials
        self._wml_token = wml_token
        self._instance_details = client.instance_details
        self._href_definitions = HrefDefinitions(client.wml_credentials)

    def _refresh_token(self):
        # TODO if self._wml_token expired
        self._wml_token = refresh_ml_token(self._wml_credentials, self._wml_token)
        self._client.wml_token = self._wml_token
