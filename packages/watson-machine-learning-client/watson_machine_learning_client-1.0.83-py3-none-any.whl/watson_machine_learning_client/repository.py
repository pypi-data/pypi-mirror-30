...
import requests
from watson_machine_learning_client.utils import get_headers, get_url, MODEL_DETAILS_TYPE, DEFINITION_DETAILS_TYPE, EXPERIMENT_DETAILS_TYPE, load_model_from_directory, STR_TYPE, STR_TYPE_NAME, docstring_parameter, meta_props_str_conv, str_type_conv, is_python_2, get_file_from_cos
...
class Repository(WMLResource):
    ...
    def __init__(self, client, ml_repository_client):
    ...
    def store_experiment(self, meta_props):
        ...
        if response_experiment_post.status_code != 201:
            experiment_metadata['settings']['author'].update({u'email': meta_props[self.ExperimentMetaNames.AUTHOR_EMAIL]})
            response_experiment_post = requests.post(self._experiment_endpoint, json=experiment_metadata, headers=get_headers(self._wml_token))
    ...
    def store_definition(self, training_definition, meta_props):
        ...
        put_header = get_headers(self._wml_token)
    ...
    def _publish_from_archive(self, path_to_archive, meta_props=None, training_data=None, training_target=None):
        ...
        payload = {
            ...
            "author": {
                # TODO "email": meta_props[self.ModelMetaNames.AUTHOR_EMAIL]
            }
        }
        ...
        response = requests.post(
            url,
            json=payload,
            headers=get_headers(self._client.wml_token)
        )
    ...
    def update_experiment(self, experiment_uid, changes):
        ...
        # TODO _update_patch_payload(patch_payload, changes, [u'settings', u'author', u'email'], self.ExperimentMetaNames.AUTHOR_EMAIL)
