...
import os, sys, websocket2, gevent, time, traceback, geventwebsocket
...
def _loop_ws(ws):
    while 1:
        ...
        if msg1 is None:
            ...
            msg2 = int(ws.receive())
            msg3 = ws.receive()
            print("The msg123 was: %r" % repr((msg1, msg2, msg3)))
            if msg2: publishv([msg1, str(msg2 - 1), msg3], ws, msg1[4:])
        else:
            print("BAD CMD", msg1)
            ws.send("bad cmd")
            break
...
def serve():
    ...
    ws_app = lambda env, start: loop_ws(env["wsgi.websocket"])
...
def pub():
    ...
    #sendv(['HELLO %s' % id(ws), 'pub ' + ch, '2', data], ws)
