...
def dctiv(f, L=None, dim=None):
    ...
    ``dctiv(f,L)`` zero-pads or truncates **f** to length **L** before doing
    the transformation.

    ``dctiv(f,dim=dim)`` or ``dctiv(f,L,dim)`` applies the transformation
    along dimension **dim**.

    The transform is real (output is real if input is real) and
    it is orthonormal. It is its own inverse.

    Let f be a signal of length **L** and let :math:`c=dctiv(f)` then:
    ...
    .. image:: images/dctiv.png
       :width: 700px
       :alt: dctiv image
       :align: center
    .. seealso::  :func:`~ltfatpy.fourier.dctii`,
        :func:`~ltfatpy.fourier.dctiii`, :func:`~ltfatpy.fourier.dstiv`
