        ...
        for shape in shapes:
            ...
            for dim in range(len(shape)):
                ...
                for N in N_val:
                    ...
                    msg = ('Wrong shape in the ouput of idft with ' +
                           'inputs ' + str(inputs))
                    self.assertEqual(f.shape, expected_shape, msg)

    def test_inversion(self):
        """Check that idft is the inverse of dft
        """
        K = 16
            ...
            for dim in range(len(shape)):
                ...
                for N in N_val:
                    ...
                    msg = ('Result of inversion is not close enough to the ' +
                           'original when composing dft and idft ' +
                           'with inputs ' + str(inputs))
                    assert_allclose(f, f_ref, rtol=1e-15, atol=1e-14,
                                    err_msg=msg)

    def test_known(self):
        """Checking idft on some known results taken from Octave
        """
        filename = get_dataset_path('idft_ref.mat')
        data = read_ref_mat(filename)

        for inputs, outputs in data:
            h = idft(**inputs)
            msg = ('Wrong value in output of idft with inputs ' +
                   str(inputs))
            assert_allclose(h, outputs[0], rtol=1e-15, atol=1e-14, err_msg=msg)


if __name__ == '__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(TestIdft)
