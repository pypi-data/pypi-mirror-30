...
import pathlib
import re
from typing import Optional, List, Pattern, MutableMapping, Union, Tuple  # pylint: disable=unused-import
...
def __tokens_as_regex(tokens: List[lexery.Token]) -> PatternSegment:
    ...
    for token in tokens:
        if token.identifier == '*':
            ...
            regex_parts.append('(0[1-9]|1[1-2])')
            patseg.group_map[group] = token.identifier
            group += 1
        elif token.identifier == '%-m':
            regex_parts.append('(1[1-2]|[1-9])')
            patseg.group_map[group] = token.identifier
            group += 1
        elif token.identifier == '%y':
