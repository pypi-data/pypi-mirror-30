...
def log_dict_types(a_dict, prefix="", types=None, use_logger=logger):
    was_logged = False
    for key in a_dict.keys():
        ...
        if not types or value_type in types:
            use_logger.info("'{0}' is a '{1}'".format(fq_key, value_type))
            was_logged = True
        if type(value) is dict:
            log_dict_types(value, prefix=fq_key, types=types)
    if not was_logged:
        if prefix:
            self_prefix = "self."
        else:
