...
#		global spiBoards, ldac, clr, cnv, cnv_board
		...
		#TEC+LED unit setup
		self.led_on = 17 #this will have to change to say 22. Remaining status LEDs will have to be BCM 0,5,6,26?
...
#		GPIO.output(self.ldac,GPIO.LOW)
	...
	#I think this doesn't actually directly correspond to current, need to work out from circuit.
	#I guess for now think of it as voltage instead for now, but in gradations up to 120.
	def set_LEDCurrent(self,current):
		if(current > 120):
			raise ValueError("Current too high, stay under 120mA")
