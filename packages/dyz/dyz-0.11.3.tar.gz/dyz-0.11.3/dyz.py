...
def db_backup(ctx, output, zip):
    ...
    if zip:
        ...
        filename = '%s_%s.backup' % (database, time.strftime('%Y%m%d_%H%M%S'))
        _format = 'backup'
...
def db_backup_all(ctx, output, dev, test, prod, host, port, database, user, password, protocol, local, remote,
    ...
    for section in sections:
        t = threading.Thread(target=__db_backup_by_section, args=(ctx, section, output, manifest, zip))
        t.start()
        threads.append(t)
    for t in threads:
        t.join()
...
def __db_backup_by_section(ctx, section, output, manifest, zip):
    kwargs = __get_kwargs_from_section(ctx, section)
    database = kwargs['database']
    host = kwargs['host']
    ...
    try:
        ...
        if zip:
            try:
                ...
                not_zip = True
                filename = '%s_%s.backup' % (database, now_str)
        if not_zip:
