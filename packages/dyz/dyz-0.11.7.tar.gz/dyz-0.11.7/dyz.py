...
def db_backup_all(ctx, output, dev, test, prod, host, port, database, user, password, protocol, local, remote,
    ...
    for section in sections:
        t = threading.Thread(target=__db_backup_by_section, args=(ctx, section, output, manifest, zip, key))
        t.start()
        threads.append(t)
    for t in threads:
        t.join()
...
def __db_backup_by_section(ctx, section, output, manifest, zip, key):
    kwargs = __get_kwargs_from_section(ctx, section)
    database = kwargs['database']
    host = kwargs['host']
