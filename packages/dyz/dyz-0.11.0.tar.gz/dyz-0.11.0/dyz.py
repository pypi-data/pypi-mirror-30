...
def __section_filtered(ctx, **kwargs):
    ...
    f_user, f_password, f_protocol = kwargs.get('user', None), kwargs.get('password', None), kwargs.get('protocol',
                                                                                                        None)
    modes = []
    ...
    for index, section in enumerate(sections):
        ...
        if connect:
            try:
                ...
                data.append('*')
                odoo_version = odoo_cnx.version or ''
            except:
                data.append(' ')
        if login:
            try:
...
@click.option('--password', type=click.STRING, default=False, required=False)
@click.option('--protocol', type=click.STRING, default=False, required=False)
def db_backup_all(ctx, output, dev, test, prod, host, port, database, user, password, protocol):
    ...
    for section in sections:
        __db_backup_by_section(ctx, section, output)
