...
def __list(ctx, arg, connect, login, short, version, dev, test, prod):
    """Show section list"""
    ...
    if not modes:
        modes = ['dev', 'test', 'prod']
    __section_list(ctx, arg, connect, login, short, version, modes)


def __section_list(ctx, arg, connect, login, short, version, modes):
    """Show section list"""
    config = ctx.obj['config_obj']
    config_path = ctx.obj['config_path']
    ...
    for index, section in enumerate(config.sections()):
        ...
        if login:
            try:
                odoo_cnx = odoo_cnx.login(database, user, password)
                data.append('*')
            except:
                data.append(' ')
        if version:
