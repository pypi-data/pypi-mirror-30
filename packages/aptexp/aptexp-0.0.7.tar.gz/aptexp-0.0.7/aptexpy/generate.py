...
class GenerateXls:
    ...
    def appendSingleLine(self, values=['文件名','文件SHA1','发布年份','发布日期','发布方','文件二进制'], cellstyle=xlwt.easyxf(
             'font: name 微软雅黑;'
        ...
        for i in range(len(values)):
            ws.write(self.__nrows, i, values[i], cellstyle)
    ...
    def appendHeadLine(self):
        self.configStyle()
        return self.appendSingleLine(cellstyle=xlwt.easyxf(
             'font: name 微软雅黑;'
             'borders: left thin, right thin, top thin, bottom thin;'
             'alignment: horizontal center, vertical center;'
