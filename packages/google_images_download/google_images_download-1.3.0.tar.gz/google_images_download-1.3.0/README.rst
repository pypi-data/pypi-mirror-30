...
On **Windows** if for some reason the chromedriver gives you trouble, download it under the current directory and run the command.

On **Linux** if you are having issues installing google chrome browser, refer to this `Link <https://intoli.com/blog/installing-google-chrome-on-centos/>`__

On **MAC** simply place the chromedriver in a directory and reference the file.

For **All the operating systems** you will have to use '--chromedriver' or '-cd' argument to specify the path of chromedriver in your machine.
