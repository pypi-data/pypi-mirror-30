...
class UETAnalytics:
    ...
    def loadModel(self, modelname, type):
        ...
        filename = os.path.dirname(os.path.realpath(__file__)) + '/model/' + modelname + '_' + type + '_' + str(version)
    ...
    def regression(self, modelname, columnX, columnY):
        ...
        uet = UETRegressor(modelname=modelname, X=X, y=y)
        uet.fit()
        uet.saveModel()
    ...
    def classifcation(self, modelname, columnX, columnY, traindata=[]):
        X = traindata[columnX]
        y = traindata[columnY]
        uet = UETClassification(modelname=modelname, X=X, y=y)
        uet.fit()
        uet.saveModel()

    def clustering(self, modelname, columnXs):
        X = self.dataset[columnXs]
        uet = UETClustering(n_cluster=5, X=X, modelname=modelname)
        uet.saveModel()
        best = uet.labelGrade()
        return best
