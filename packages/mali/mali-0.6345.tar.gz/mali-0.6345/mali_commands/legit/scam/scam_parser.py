...
class SQLQueryBuilder(MLQueryVisitor, MLQueryMixin):
    ...
    def visit_group(self, node, parents=None, context=None):
        self._sub_visit(node.expr, context=context, is_group=True)
    ...
    def _sub_visit(self, child, context=None, is_group=False):
        ...
        for bucket, wheres in combine_where.items():
            ...
            self.__where.setdefault(bucket, []).append((context.get('op'), sql))
