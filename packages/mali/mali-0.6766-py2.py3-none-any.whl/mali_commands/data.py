...
@click.option('--batchSize', required=False, default=1000)
@click.option('--asDict/--asList', is_flag=True, required=False, default=False)
@click.pass_context
def query_metadata(ctx, volumeid, query, batchsize, asdict):
    def get_all_results():
        ...
        download_iter = data_sync.create_download_iter(query, batchsize)

        for results in download_iter:
            if asdict:
                for item in results:
                    total_results_dict[item['@path']] = item
            else:
                total_results_list.extend(results)

        return total_results_dict if asdict else total_results_list
