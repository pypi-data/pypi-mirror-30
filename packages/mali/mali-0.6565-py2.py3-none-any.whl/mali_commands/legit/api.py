...
def __refresh_token_id_tone(config):
    try:
...
def __api_call(config, url, http_method, data):
    id_token = config.id_token if config.id_token else __refresh_token_id_tone(config)

    for _ in range(3):
        headers = {'Authorization': 'Bearer {}'.format(id_token)}
        r = http_method(url, headers=headers, json=data)
        if r.status_code == 401:
            id_token = __refresh_token_id_tone(config)
            continue
