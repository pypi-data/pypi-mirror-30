...
def setup_cloud_template(ctx, org, arn, ssh_key_path):
    ...
    manager_image = ctx.obj.rm_manager_image
    container_name = ctx.obj.rm_container_name
    template = dict(cloud_type='aws', key=arn, ssh=ssh, prefix=prefix, mali=mali,
                    mali_image=mali_image, socket_server=socket_server,
                    config_volume=config_volume, container_name=container_name, rm_image=manager_image)
...
def pop_key_or_prompt_if(kws, key, src_default=None, **kwargs):
    ...
    while cur_val == src_default:
        if 'default' in kwargs and kws.get('silent', False):
            print('{}: {} (silent)'.format(kwargs.get('text'), kwargs.get('default')))
...
def azure_auth(ctx, kwargs, org):
    ...
    config_name = pop_key_or_prompt_if(kwargs, 'azure_connector', text='configuration name [--azure-connector]:', default='%s-%s' % ('azure', 'default'))
    prefix, config_data = get_config_prefix_and_file(ctx.obj)
    mali_image = 'missinglinkai/mali:UNSTABLE'
    socket_server = ctx.obj.rm_socket_server
    config_volume = ctx.obj.rm_config_volume
    manager_image = ctx.obj.rm_manager_image
    container_name = ctx.obj.rm_container_name
    template = dict(cloud_type='azure', prefix=prefix, name=config_name,
                    mali_image=mali_image, socket_server=socket_server,
                    config_volume=config_volume, container_name=container_name, rm_image=manager_image)
    ...
    cloud_data = [
        ...
        # {'key': 'auth_url', 'value': auth_url_path},
    ...
    if authed_request['ok']:
        print('Azure Authorisation for %s completed' % template['name'])
    else:
        print('Azure Authorisation for %s failed' % template['name'])
...
def _get_cloud_connectors(ctx, org, cloud_type):
    print("Checking for available cloud connectors for %s" % cloud_type)
...
def get_cloud_connector(ctx, org, kwargs, cloud_type, connect_dict=None):
    if connect_dict is None:
        connect_dict = {'azure': lambda: azure_auth(ctx, kwargs, org)}
    ...
    print('List of %s available connectors for %s' % (cloud_type, org))
    output_result(ctx, available_connectors)
    print('You can use easting connector by specifying its name or create a new one by using a name from the list')
    connector_param = '%s_connector' % cloud_type
    connector_cli_param = '--%s-connector' % cloud_type
    ...
    for x in available_connectors:
        if connector_name == x['name']:
            print('Reusing `%s` connector' % connector_name)
        return x
...
def create_static_group(ctx, kws, org):
    group_name = pop_key_or_prompt_if(kws, 'group_name', text='Group name [--group-name]', default='static-default')
    print('Creating static group `%s` ' % group_name)
    ...
    request = {
        ...
        'terminate_on_stop': pop_key_or_prompt_if(kws, 'terminate_on_stop', text='Terminate instances (should be False for static groups) [--group-terminate-on-stop]', default=False, type=bool),
        'region': None if cloud_type == 'azure' else pop_key_or_prompt_if(kws, 'region', text='Region [--group-region]'),
        'priority': pop_key_or_prompt_if(kws, 'priority', text='Priority (higher is better) [--group-priority]', default=100, type=int),
        'idle_timeout': pop_key_or_prompt_if(kws, 'idle_timeout', text='Idle timeout  [--group-idle-timeout]', default=5, type=int),
    }
    url = '{org}/resource_group/{name}'.format(org=org, name=group_name)
    response = ctx.obj.handle_api(ctx.obj, requests.post, url, request)
    print('Static Group %s created.' % group_name)
    return response['result']


def select_static_resource_group(ctx, available_groups, kws, org):
    default_name = 'static-default'
    if available_groups:
        print('Here is a list of your current groups. You can use one of them or enter a name for new group')
        output_result(ctx, available_groups, fields=['name', 'cloud_type', 'capacity', 'gpu', 'priority'])
        default_name = available_groups[0]['name']
    group_name = pop_key_or_prompt_if(kws, 'group_name', text='Group name [--group-name]', default=default_name)
    for group in available_groups:
        if group_name == group['name']:
            print('Selected pre-existing group %s' % group_name)
            return group
    kws['group_name'] = group_name
    return create_static_group(ctx, kws, org)


def static_resource_groups(ctx, org, kwargs):
    print("Checking for available resource groups for %s" % org)
    url = '{org}/static_resource_groups'.format(org=org)
    response = ctx.obj.handle_api(ctx.obj, requests.get, url)
    available_groups = response.get('results', [])
    return select_static_resource_group(ctx, available_groups, kwargs, org)
...
def setup_static_template(ctx, org, **kwargs):
    ...
    if machine is None:
        print('All of your machines are manged.')
        return
...
def apply_aws_template(ctx, org, arn, ssh, mali, prefix, token, rm_socket_server, rm_manager_image, rm_config_volume, rm_container_name, region):
    ...
    print('The resource manager is configured and running %s' % inst.id)
