...
class ProfilerMiddleware(object):
    ...
    def check_conditions(self, request):
        """Checks conditions to run the profiler.

        :param request: Request object
        :type request: :class:`django.http.HttpRequest`
        :return: True if all conditions are meet
        :rtype: bool
        """
        return profiler.is_on and hasattr(request, 'user') and self.get_view_name(request) and \
    ...
    def process_view(self, request, *args, **kwargs):
        ...
        """
        self.is_active = self.check_conditions(request)
    ...
    def process_response(self, request, response):
        ...
        if self.is_active:
            ...
            # Saves profiler data
            profiler.save(view_name, request.method, is_anon_call, is_cache_hit, sql_time, sql_count, view_execution_time)
