...
class input(PybindBase):
  ...
  YANG type.
  """
  __slots__ = ('_pybind_generated_by', '_path_helper', '_yang_name', '_rest_name', '_extmethods', '__page_cursor','__node_id','__id','__mode','__gw_name','__src_ip','__dest_ip','__config_src','__site_name','__admin_state','__oper_state','__bfd_state',)
...
this value is not present or empty.
  ...
  def _get_node_id(self):
    """
    Getter method for node_id, mapped from YANG variable /brocade_tunnels_ext_rpc/get_tunnel_info/input/node_id (uint32)

    YANG Description: Node from which the tunnel information to be
...
all nodes.
  ...
  def _set_node_id(self, v, load=False):
    """
    Setter method for node_id, mapped from YANG variable /brocade_tunnels_ext_rpc/get_tunnel_info/input/node_id (uint32)
    If this variable is read-only (config: false) in the
...
address are supported in this release.
  ...
  _pyangbind_elements = {'page_cursor': page_cursor, 'node_id': node_id, 'id': id, 'mode': mode, 'gw_name': gw_name, 'src_ip': src_ip, 'dest_ip': dest_ip, 'config_src': config_src, 'site_name': site_name, 'admin_state': admin_state, 'oper_state': oper_state, 'bfd_state': bfd_state, }
