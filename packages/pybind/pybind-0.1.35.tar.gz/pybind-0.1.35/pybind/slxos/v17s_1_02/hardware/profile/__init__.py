...
class profile(PybindBase):
  ...
  def _get_overlay_visibility(self):
    ...
    """
    return self.__overlay_visibility
      
  def _set_overlay_visibility(self, v, load=False):
    """
    Setter method for overlay_visibility, mapped from YANG variable /hardware/profile/overlay_visibility (container)
    If this variable is read-only (config: false) in the
    source YANG file, then _set_overlay_visibility is considered as a private
    method. Backends looking to populate this variable should
    do so via calling thisObj._set_overlay_visibility() directly.
    """
    if hasattr(v, "_utype"):
      v = v._utype(v)
    try:
