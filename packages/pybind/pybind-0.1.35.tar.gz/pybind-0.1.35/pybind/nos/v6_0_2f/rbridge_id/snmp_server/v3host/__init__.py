...
class v3host(PybindBase):
  ...
  def _get_use_vrf(self):
    """
    Getter method for use_vrf, mapped from YANG variable /rbridge_id/snmp_server/v3host/use_vrf (use-vrf-option)
    """
    return self.__use_vrf
      
  def _set_use_vrf(self, v, load=False):
    """
    Setter method for use_vrf, mapped from YANG variable /rbridge_id/snmp_server/v3host/use_vrf (use-vrf-option)
    If this variable is read-only (config: false) in the
