...
class use_vrf(PybindBase):
  ...
  def _get_secure_and_plain(self):
    """
    Getter method for secure_and_plain, mapped from YANG variable /rbridge_id/http/server/http_vrf_cont/use_vrf/secure_and_plain (empty)
    """
    return self.__secure_and_plain
      
  def _set_secure_and_plain(self, v, load=False):
    """
    Setter method for secure_and_plain, mapped from YANG variable /rbridge_id/http/server/http_vrf_cont/use_vrf/secure_and_plain (empty)
    If this variable is read-only (config: false) in the
