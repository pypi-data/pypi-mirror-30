...
def api():
    ...
    principal_file = tempfile.NamedTemporaryFile(suffix='.pdf', delete = False).name
    f.save(principal_file)
    app.logger.info("Saving template file %s to %s" % (f.filename, principal_file))
    the_json["general"] = dict()
    ...
    for annexe_file in annexes_filelist:
        temp_annexe_file = tempfile.NamedTemporaryFile(suffix='.pdf', delete = False).name
    ...
    res = skittlesPY.skyttles(the_json, output_file, app.debug)
    if res is not True:
        raise InvalidUsage(res)

    rv = send_file(output_file, attachment_filename="result.pdf", as_attachment=True)
...
if __name__ == '__main__':
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hp:d", ["help", "port=", "debug"])
