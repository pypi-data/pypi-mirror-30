...
def test_account_registration():
    ...
    expected_keys = ['contact', 'createdAt', 'id', 'initialIp',
                     'key', 'status']
...
def test_order_pending_challenge_values_by_name():
    domain_names = [f'{generate_subdomain()}.{domain}',
                    f'{generate_subdomain()}.{domain}']
    domain_names.append(f'*.{domain_names[1]}')

    if acme_client.kid is None:
        acme_client.register()

    order = acme_client.new_order(domain_names)

    assert len(order.pending_challenge_values_by_name) == 2
    assert sum([len(v) for v
                in order.pending_challenge_values_by_name.values()]) == 3
