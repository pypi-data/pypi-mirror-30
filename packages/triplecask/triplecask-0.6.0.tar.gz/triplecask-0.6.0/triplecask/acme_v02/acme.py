...
class ACME(object):
    ...
    def __init__(self, account_key, directory_uri=None):
        """
        Creates a new ACME client instance for a given private key.
        """
        if directory_uri is None:
            if os.getenv('TRIPLECASK_LE_ACME_ENVIRONMENT', 'stage').lower() == 'production':
                directory_uri = PRODUCTION_DIRECTORY_URI
            else:
                directory_uri = STAGING_DIRECTORY_URI
    ...
    def __read_order(self, raw, domains, refresh_uri):
        authz = [self.__read_dns_challenge(a) for a in raw['authorizations']]
        pending_authz_by_dns_name = {}

        for auth in [a for a in authz if a.status == 'pending']:
            if auth.dns_record_name in pending_authz_by_dns_name:
                pending_authz_by_dns_name[auth.dns_record_name].append(
                    auth.dns_record_value
                )
            else:
                pending_authz_by_dns_name[auth.dns_record_name] = [
                    auth.dns_record_value
                ]
    ...
    def register(self, agree_to_tos=False):
        if agree_to_tos is False and \
           os.getenv('TRIPLECASK_LE_TOS_AGREED', '').lower() != 'true':
            raise Exception('Please agree to the Let\'s Encrypt Terms of Service')
