...
from ..utils.wsclient_utils import WsClient
...
class Instance(object):
    ...
    @staticmethod
    def log(auth, serviceid,instanceid):
        """实例日志"""
        short_auth_token, short_access_authority_token = auth.get_short_token()
        data = Instance._list(auth, serviceid)
        microservices = data.get("micro_services", [])
        domain_name_service = None
        credential_user = None
        credential_password = None
        for microservice in microservices:
            for instance in microservice.get("instances", []):
                if instance.get("id", "") == instanceid:
                    domain_name_service = instance.get("domain_name_service", "")
                    credential_user = instance.get("credential_user", "")
                    credential_password = instance.get("credential_password", "")
                    break
            if domain_name_service:
                break
        if not domain_name_service:
            raise Exception(u"domain name service异常")
        # wsurl = "ws://{}/webtty/{}:59998/{}/{}/ws".format(auth.end_point, domain_name_service, short_auth_token, short_access_authority_token)
        # wsurl = "ws://{}/webtty/{}:59998/{}/{}/ws".format(auth.end_point, "i477-mysql.a3.huayun.local", short_auth_token, short_access_authority_token)
        wsurl = "http://{}@{}/webtty/{}:59998/{}/{}/".format("{}:{}".format(credential_user, credential_password),
                                                             auth.address, domain_name_service, short_auth_token,
                                                             short_access_authority_token)
        try:
    ...
    def terminal(auth, serviceid, instanceid):
        ...
        # wsurl = "ws://{}/webtty/{}:59998/{}/{}/ws".format(auth.end_point, domain_name_service, short_auth_token, short_access_authority_token)
        # wsurl = "ws://{}/webtty/{}:59998/{}/{}/ws".format(auth.end_point, "i477-mysql.a3.huayun.local", short_auth_token, short_access_authority_token)
        wsurl = "http://{}@{}/webtty/{}:59999/{}/{}/".format("{}:{}".format(credential_user, credential_password),
                                                             auth.address, domain_name_service, short_auth_token,
                                                             short_access_authority_token)
        try:
            cmd = "{} {}".format(auth.ttyclient, wsurl)
