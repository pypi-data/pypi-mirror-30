...
def show_version(**kwargs):
    sys.stdout.write(_dump_response(RequiredAuth.NONE, AuthenticationApi, AuthenticationApi.get_server_version , kwargs))
...
SDKMS_API_KEY*: The API key of the application to use for application-
...
'''.format(VERSION)
    ...
    parser.add_argument("--verify-ssl", action="store_true", default=True, help="Verify server certificate")
...
""")
    ...
    parser_show_version = subparsers.add_parser("version", help="check sdkms server version")
    parser_show_version.set_defaults(func=show_version)

    # parser for show health
    parser_show_health = subparsers.add_parser("health", help="check sdkms server health")
