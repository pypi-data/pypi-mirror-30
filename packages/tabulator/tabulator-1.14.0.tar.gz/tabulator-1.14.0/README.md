...
-  ``skip_rows (int/str[])`` - list of rows to skip by row number or row
   comment. Example: ``skip_rows=[1, 2, -1, -3, '#', '//']`` - rows 1, 2
   and rows 1, 3 from the end and all rows started with ``#`` and ``//``
   will be skipped. See `skip
...
-  the ``skip_rows`` argument now supports negative numbers to skip rows
   from the end
