...
class DataClayObject(object):
    ...
    @dclayMethod(attribute="str", new_alias_val="anything")
    def _set_has_alias_internal(self, attribute, new_alias_val):
        # FIXME: Remove this import
        from dataclay.commonruntime.Runtime import getRuntime
        for exec_env in getRuntime().get_all_execution_environments_by_oid(self.get_object_id()):
            self.run_remote(exec_env, 'set_has_alias', new_alias_val)

    @dclayMethod(new_alias_val="anything")
    def set_has_alias(self, new_alias_val):
        """
        @postcondition: Set has_alias
        @param new_alias_val: true if has alias
        """
        self.has_alias = new_alias_val
