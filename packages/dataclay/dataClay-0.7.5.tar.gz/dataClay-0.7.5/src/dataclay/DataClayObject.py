...
from dataclay.serialization.lib.SerializationLibUtils import serialize_association, serialize_reference_counting, PersistentIdPicklerHelper
...
class DataClayObject(object):
    ...
    def has_alias(self):
        """
        @postcondition: Return true if the object has alias
        @return true if the object has alias
        """
        return self.__dclay_instance_extradata.has_alias
    
    def set_has_alias(self, new_alias_val):
        """
        @postcondition: Set has_alias
        @param new_alias_val: true if has alias
        """
        self.__dclay_instance_extradata.has_alias = new_alias_val
    ...
    def getID(self):
        ...
        string.
        """
        return str(self.__dclay_instance_extradata.object_id)
    ...
    def run_federated(self, operation_name, params):
        result = dict()
        federation = getRuntime().ready_clients["@LM"].get_federation_of_object(self.get_object_id())
        for dc_info in federation:
            try:
                r = getRuntime().call_execute_to_external_DC(self, params, operation_name, dc_info)
                result[dc_info.dcID] = r
            except Exception as e:
    ...
    def serialize(self, io_file, ignore_user_types, iface_bitmaps,
        ...
        serialize_reference_counting(self.get_object_id(), io_file, reference_counting)
    ...
    def deserialize(self, io_file, iface_bitmaps,
        ...
        self.set_has_alias(BooleanWrapper().read(io_file))
