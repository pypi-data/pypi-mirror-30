...
def serialize_association(
    ...
    """ if hint is none, key is None, means no location specified. """ 
    if not reference_counting.has_key(hint): 
        references_per_hint = dict()
        reference_counting[hint] = references_per_hint
    else:
        references_per_hint = reference_counting.get(hint)
    
    if references_per_hint.has_key(associated_oid):
        num_refs = references_per_hint.get(associated_oid) + 1
    else:
        num_refs = 1
    references_per_hint[associated_oid] = num_refs   
...
def serialize_reference_counting(referrer_oid, io_file, reference_counting):
    ...
    logger.debug("*********** Serialized reference counting for %s : length of map: %s", referrer_oid, len(reference_counting))
    
    IntegerWrapper().write(io_file, len(reference_counting))
    for location, ref_counting_in_loc in reference_counting.iteritems():
        if location is None:
            logger.debug("*********** Serialized reference counting for %s : none location", referrer_oid)
            BooleanWrapper().write(io_file, True)
        else:
            logger.debug("*********** Serialized reference counting for %s : location %s", referrer_oid, location)
            BooleanWrapper().write(io_file, False)
            StringWrapper().write(io_file, str(location))
        
        logger.debug("*********** Serialized reference counting for %s : associations map size: %s", referrer_oid, len(ref_counting_in_loc))
        IntegerWrapper().write(io_file, len(ref_counting_in_loc))
        for oid, counter in ref_counting_in_loc.iteritems():
            logger.debug("*********** Serialized reference counting for %s : association: %s -> %s", referrer_oid, oid, counter)
...
class PersistentIdPicklerHelper(object):
    ...
    def __call__(self, obj):
        if isinstance(obj, dataclay.DataClayObject):
            ...
            hint = obj.get_hint()  
            """ if hint is none, key is None, means no location specified. """ 
            if not self._reference_counting.has_key(hint): 
                references_per_hint = dict()
                self._reference_counting[hint] = references_per_hint
            else:
                references_per_hint = self._reference_counting.get(hint)
