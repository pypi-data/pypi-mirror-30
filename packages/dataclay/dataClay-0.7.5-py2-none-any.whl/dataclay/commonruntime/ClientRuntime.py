...
class ClientRuntime(DataClayRuntime):
    ...
    Refusing the temptation to guess.""")
        ...
        if instance.is_persistent():
            if alias is not None:
                ...
                self.logger.debug("Is Persistent but we will add alias with session %s, metaclassID %s, backendID %s",
                settings.current_session_id, instance.get_class_extradata().class_id, backend_id)
                ...
                self.alias_cache[alias] = (instance.get_object_id(), backend_id)
                instance.set_has_alias(True)
                return backend_id

            else:
                self.logger.verbose("Trying to make persistent %r, which already is persistent. Returning backendID", self)
                return backend_id
        ...
        if alias is not None:
            ...
            self.alias_cache[alias] = (object_id_to_have_alias, hint)
            instance.set_has_alias(True)
    ...
    def execute_implementation_aux(self, operation_name, instance, parameters, exeenv_id=None):
        ...
        for k in range(max_retry):
            ...
            except DataclayException as dce: 
                last_exception = dce
    ...
    def call_execute_to_external_DC(self, instance, params, operation_name, dc_info):
        ...
        if serialized_params is not None and serialized_params[3] is not None:
            for param in serialized_params[3]:
                # TODO: CHECK
                self.logger.info("param[0] is object_id??: %s", param[0])
        ...
        if num_misses>0 and serialized_params is not None:
            for volatil in serialized_params[3]:
                # TODO: CHECK
                self.logger.info("volatil[0] is object_id??: %s", volatil[0])
