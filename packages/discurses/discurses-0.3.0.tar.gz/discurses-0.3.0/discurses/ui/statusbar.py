...
class Statusbar(urwid.WidgetWrap):
    ...
    def __init__(self, chat_widget):
        ...
        # Public widgets
        self.w_echo = urwid.Text(self.message)
    ...
    def echo(self, message, *args, **kwargs):
        ...
        self.w_echo.set_text(self.message)

    def clear(self, message=None):
        """
        Clear the echo area.
        If `message` is not `None`, clear only if `message` is equal to the
        currently displayed message
        """
        if message is None or message == self.w_echo.text:
            self.message = ''
            self.w_echo.set_text('')
