...
class MessageListWidget(urwid.WidgetWrap):
    ...
    def __init__(self, discord_client, chat_widget):
        ...
        self.first_unread_message = None
        self.__super.__init__(self.listbox)

    def add_message(self, message):
        if self.first_unread_message is None:
            self.set_first_unread(message)
    ...
    def set_first_unread(self, message):
        """Set the first unread message and add the correct ReadLineWidget"""
        self.first_unread_message = message
        read_line = next((x for x in self.list_walker
                          if isinstance(x, ReadLineWidget)), None)
        if read_line is not None:
            read_line.timestamp = message.timestamp
            if message is None:
                self.list_walker.remove(read_line)
            else:
                self.list_walker.sort_messages()
        elif message is not None:
            self.list_walker.append(
                ReadLineWidget(self.chat_widget, message.timestamp))
...
class MessageListWalker(urwid.MonitoredFocusList, urwid.ListWalker):
    ...
    def get_logs(self, before=None, callback=lambda: None):
        ...
        async def _callback():
            ...
            first_message = None
            # for channel in self.list_widget.chat_widget.channels:
            #     read_state = self.list_widget.discord.read_states.get(channel.id, None)
            #     if read_state is not None:
            #         if first_message is None or \
            #         read_state.last_message.timestamp < first_message.timestamp:
            #             first_message = read_state.last_message

            # self.list_widget.set_first_unread(first_message)
...
class MessageWidget(urwid.WidgetWrap):
    ...
    def quote_message(self):
        self.chat_widget.w_message_edit.reply_to(self.message)
        self.chat_widget.set_focus('MESSAGE_EDIT')

    @keymaps.MESSAGE_LIST_ITEM.command
    def mention_author(self):
        self.chat_widget.w_message_edit.edit.insert_text("<@!{0}>".format(
    ...
    def aknowledge(self):
        pass
        # self.discord.async(f)
        # self.chat_widget.w_message_list.set_first_unread(self);
...
class ReadLineWidget(urwid.WidgetWrap):
    """An indicator below the last read message"""
    ...
    def __init__(self, chat_widget, date):
        ...
        self.message = FakeMessage(
            datetime.datetime.combine(date,
                                      datetime.datetime.min.time()))
        self._selectable = False
        txt = urwid.Text(("dateline", 'New messages'),
                         align=urwid.LEFT)
        self.__super.__init__(txt)
