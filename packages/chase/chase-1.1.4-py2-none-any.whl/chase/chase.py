...
class Endpoint(object):
    ...
    def add_order_trace_number(self, order_id, message_type):
        message_type_ints = {
                "A": 1000,
                "AC": 2000,
                "FC": 3000,
        ...
        message_type_int = message_type_ints.get(message_type)
        if message_type_int:
            filtered_order_id = re.sub("[^0-9]", "", str(order_id))
            self.headers['Trace-number'] = "%d%s" % (message_type_int, filtered_order_id)
...
class Order(Endpoint):
    ...
    def __init__(self, **kwargs):
        ...
        self.add_order_trace_number(self.order_id, self.message_type)
