...
try:
    ...
    MsgBoxErr("Error creating TEMP path")
    sys.exit(0)
...
class App(object):
    ...
    def __showMain(self):            
        def OnImageChosen(examType, examNumber, showImages, path):
            ...
            self.__processAnswerSheet(examType = examType, examNo = examNumber, file = path, showImages = showImages)
        
        self.__frmMain = CODGUI.frmMain(PATH_TEMP)
        self.__frmMain.OnImageChosen += OnImageChosen
        WinForms.Application.Run(self.__frmMain)

    def __getExamTypeString(self, examType):
        if examType == 0:
            return "Internals"
        elif examType == 1:
            return "Externals"
    ...
    def __getExamNumberString(self, examType, examNo):
        if examType == 0:
            if examNo == 0:
                return "First internal"
            elif examNo == 1:
                return "Second internal"
            else:
                return "Undefined"
        elif examType == 1:
            if examNo == 0:
    ...
    def __processAnswerSheet(self, examType = 0, examNo = 0, file = None, showImages = True):
        ...
        print("Exam type: %s (%d)" % (self.__getExamTypeString(examType), examType))
        print("Evaluation: %s (%d)" % (self.__getExamNumberString(examType, examNo), examNo))
        asp = PaperProcessor(temp = PATH_TEMP, file = file, roi = roi, scale = 1, examType = examType, examNo = examNo, showImages = showImages, frmHelp = CODGUI.frmPerspectiveHelp(), roiPickerMod = CODGUI.RoiPickerMod) 
        output = asp.process()
