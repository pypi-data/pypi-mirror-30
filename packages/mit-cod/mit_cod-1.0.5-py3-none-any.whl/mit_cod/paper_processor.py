...
from .marks_model import Exam, Internal, External
from .marks_detection import getBoundsOfMarks
from .predictor import predictor
from .msgbox import MsgBoxErr, MsgBoxInfo
...
class PaperProcessor(object):
    ...
    def __init__(self, file, roi, scale = 1, examType = 0, examNo = 0, showImages = True, temp = None, frmHelp = None, roiPickerMod = None): # logLevel = LOG_VERBOSE):
        self.__file = file
        self.__roi = roi
        self.__scale = scale
        ...
        self.__roiPickerMod = roiPickerMod
    ...
    def __saveMarksImage(self, img, s):
        if self.__temp != None and isdir(self.__temp):
            ...
            path = os.path.join(self.__temp, 'marks.jpg')
    ...
    def process(self):
        ...
        if not isfile(self.__file):
            MsgBoxErr("Image file doesn't exist")
            return PaperProcessor.ERROR
        ...
        if self.__roi is None:
            roi = roi_selector(imgOriginal, "RoI Picker", self.__frmHelp, self.__roiPickerMod).get()
        else:
            roi = np.array(self.__roi)
            
        if (roi is None):
            MsgBoxErr("RoI not selected")
            return PaperProcessor.ERROR
        ...
        # Predict scores
        detector = predictor()
        if self.__examType == 0:
            exam = Internal(imgGrayscale.shape, self.__examNo)
        elif self.__examType == 1:
            exam = External(imgGrayscale.shape, self.__examNo)
        else:
            MsgBoxErr("Incorrect exam type")
            return PaperProcessor.ERROR
        
        digitBounds = getBoundsOfMarks(imgRedPen.copy(), exam.getBoundsLimitForDetection())
        
        if digitBounds is None:
            MsgBoxErr("No digits detected")
            return PaperProcessor.ERROR
        
        table = self.__getBlankDataTable()
        imgMarksPath = self.__saveMarksImage(imgOriginalCorrected, exam.getTableShapeForExam())
        
        if (imgMarksPath is None or not isfile(imgMarksPath)):
            MsgBoxErr("RoI image could not be created")
            return PaperProcessor.ERROR
