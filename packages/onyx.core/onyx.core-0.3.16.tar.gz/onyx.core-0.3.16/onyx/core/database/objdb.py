...
DROP TABLE IF EXISTS ClassDefinitions CASCADE;""")
    ...
    def get_dated(self, name, attr, date, strict=False):
        ...
        try:
            ...
            self[record_id] = row.value, arc_date

            if arc_date != date:
                # --- cache the value for the actual archive date too
                self[(name, attr, arc_date)] = row.value, arc_date

            return arc_date, json.loads(row.value, cls=custom_decoder)
