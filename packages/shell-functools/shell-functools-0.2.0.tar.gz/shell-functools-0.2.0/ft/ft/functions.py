...
def typed(type_in, type_out):
    def wrap(fn):
        def fn_typecheck(*args):
            ...
            if len(args) > 2:
                arg_array = list(args[0:-1])
                result = fn(arg_array, inp=inp.value)
            elif len(args) == 2:
...
def run(command, inp):
    command = list(map(lambda v: T_STRING.create_from(v).value, command))
    args = list(map(lambda v: T_STRING.create_from(v).value, inp))

    print("Running '{}' with arguments '{}'".format(" ".join(command), ", ".join(args)))
