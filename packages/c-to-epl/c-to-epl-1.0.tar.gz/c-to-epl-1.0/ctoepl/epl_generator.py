...
class EPLGenerator(object):
    ...
    def visit_ArrayRef(self, n, mappings):
        ...
        gh = arrref + '[' + self.visit(n.subscript) + ']'
    ...
    def visit_Return(self, n, mappings):
        ...
        if current_fct=="main":
            ...
            return "verify_pow("+assignments[-1]+", "+assignments[-2]+", "+assignments[-3]+", "+assignments[-4]+"); verify_bty(" + self.visit(n.expr) + ");"
        else:
    ...
    def visit_TernaryOp(self, n, mappings):
        s  = '(' + self._visit_expr(n.cond) + ', mappings) ? '
        s += '(' + self._visit_expr(n.iftrue) + ', mappings) : '
        s += '(' + self._visit_expr(n.iffalse) + ', mappings)'
        return s
