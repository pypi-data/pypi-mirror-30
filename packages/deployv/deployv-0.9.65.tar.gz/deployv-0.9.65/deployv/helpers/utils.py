...
def byte_converter(from_str, to_unit='B', return_float=False):
    ...
    if to_unit not in units:
        return False
    from_str = from_str.replace(' ', '').upper()
    exp = r'(?P<size>[\d]*([\.][\d]*)?)(?P<unit>{units})'.format(units="|".join(units))
    match = re.match(exp, from_str)
    if not match:
        return False
    from_size = float(match.group('size'))
    from_unit = match.group('unit')
    if from_unit not in units:
        return False
    base_size = 1024
    steps = units.index(from_unit) - units.index(to_unit)
    if steps > 0:
        to_size = from_size * (base_size ** steps)
    else:
        to_size = from_size / (base_size ** abs(steps))
    to_str = '{size:.2f}{unit}'.format(size=to_size, unit=to_unit)
    return to_size if return_float else to_str


def verify_free_disk_space(expected_free_space):
