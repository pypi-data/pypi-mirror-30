...
class Element(object):
    ...
    def _set_drilldown(self, options):
        ...
        drilldownLink = options.pop('drilldownLink', None)
        explicitGridId = options.pop('drilldownGridId', None)
        if drilldownLink is not None:
            self._drilldownLink = drilldownLink
        elif explicitGridId is not None:
            self._drilldownLink = '/#RiskRequest:DV01:gridId='+str(explicitGridId)
        elif isinstance(self.response, ApiResponse):
            gridId = self.response.stats.get('GridId')
            if gridId is not None:
                self._drilldownLink = '/#RiskRequest:DV01:gridId='+str(gridId)
