...
import clusterone.clusterone_client

# Pypi converts "-alpha" -> "a", and "-beta" to "b"
PRERELEASES_SUFFIXES = ['dev', 'a', 'b', 'rc']
...
def is_stable(version): return not any(suffix in version for suffix in PRERELEASES_SUFFIXES)
# TODO: Move this to function signature after removing 2.7 compliance
# type: str -> bool
...
def get_pypi_versions():
    ...
    response_data = requests.get(clusterone.clusterone_client.PYPI_VERSIONS_URL).json()["releases"]
