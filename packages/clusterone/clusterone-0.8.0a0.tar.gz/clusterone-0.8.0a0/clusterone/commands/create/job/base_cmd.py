...
#TODO: Test this
#TODO: Import this here
def append_to_docstring(text):
    #TODO: Add info about potential use cases withing this project
    def wrapper(function):
...
job_base_options = combine_options([
    ...
    # Why click.UNPROCESSED really?
    click.argument('custom-arguments', nargs=-1, type=click.UNPROCESSED),
...
def base(context, kwargs, module_arguments=None):
    ...
    module_arguments = extract_user_arguments(module_arguments)
...
def extract_user_arguments(click_raw_input):
    # type ((str)) -> {str: str}
    #TODO: Python 2.7 compliance legacy -> move this to function signature
    """
    Translates click unprocessed options into API consumable
    """

    arguments = [argument.lstrip('-') for argument in click_raw_input[::2]]
    values = click_raw_input[1::2]

    if len(arguments) != len(values):
        raise MissingParameter("", param_type="one or more value for custom arguments")
