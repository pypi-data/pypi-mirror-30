...
from clusterone.utilities import lazy_property, lazify
#TODO: Redo this to top level import
...
#TODO: Account for dev urls - do something about it
#TODO: Do we still need dev envvars?
config = Config()
API_BASE = config.endpoint or 'https://clusterone.com/api'
API_URL, API_SCHEMA, API_TOKEN = map(lambda x: "".join(API_BASE + x), ["/", "/schema/", "/token/"])


#TODO: Get rid of LOCAL_DEV while refactoring this - use something more generic - ex. API target
#Overwriting local dev value insted of variable, because it used across this module
LOCAL_DEV = True if os.environ.get('JUST_LOCAL', 'False').upper() == "TRUE" else False

#TODO: Document this!!!
DEBUG = True if os.environ.get('JUST_DEBUG', 'False').upper() == "TRUE" else False
