...
# Update tests for S3 and add Github Support
...
def test_passing(mocker):
    ...
    ClusteroneClient.create_dataset = mocker.Mock(return_value=testing_dataset)
    cmd.time = mocker.Mock()

    CliRunner().invoke(cli, ['create', 'dataset', 'someDatasetName', '--source', 'gitlab', '--description', 'This is a sample project description'])

    ClusteroneClient.create_dataset.assert_called_with('someDatasetName', 'gitlab', 'This is a sample project description')
