...
@click.option(
    '--once',
    is_flag=True,
    help='See latest events. Just once.'
    )
@click.pass_obj
@authenticate()
def command(context, once):
    ...
    if once:
        ...
        while True:
            ...
            sleep(config.events_refresh_rate)
...
def output_events(client, *, before_printing=None):
    ...
    try:
        ...
        raise SoftInternalServiceError from exception

    if before_printing:
        before_printing()

    echo(make_table(events_data, header=HEADER))
