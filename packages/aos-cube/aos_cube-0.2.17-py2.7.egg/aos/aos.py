...
def __run_setup():
    ...
    if host_os.startswith('Linux'):
        ...
        if which('apt-get') != None:
            ...
            print('Warning: not yet supported linux distribution')
    ...
    if os.path.isdir('./build/compiler') == False:
        toolchains = {
            ...
            'OSX': 'http://alios-things-public.oss-cn-hangzhou.aliyuncs.com/aos-toolchain-osx.zip'
            }
    ...
    if len(packages) > 0:
        ...
        print('installing: {0}...'.format(packages), end='')
...
def _run_upload(image):
    """ we only support stlink tools for now """
    host_os = get_host_os()
    if host_os == None:
        error('Unsupported Operating System!')
    upload_cmds = {
        'Win32': 'cmd/win32/st-flash.exe',
        'Linux64': 'cmd/linux64/st-flash',
        'Linux32': 'cmd/linux32/st-flash',
        'OSX': 'cmd/osx/st-flash'
    }
    tools_dir = os.path.join(os.getcwd(), 'build').replace('\\', '/')
    upload_cmd = os.path.join(tools_dir, upload_cmds[host_os])
    log('upload_cmd: %s\n' % upload_cmd)
    addr = '0x08000000'
    subprocess.call([upload_cmd, 'write', image, '0x08000000'], stdout=sys.stdout, stderr=sys.stderr)
...
def upload():
    ...
    image_path = os.path.join(out_path, target, 'binary/', target + '.bin')
    log("image_path: %s\n" % image_path)

    _run_upload(image_path)
