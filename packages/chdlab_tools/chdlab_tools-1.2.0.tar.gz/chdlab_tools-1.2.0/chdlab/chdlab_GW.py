...
class GW(object):
        ...
        def __init__(self, board_number, AP_24G_SID, AP_24G_PSW, AP_5G_SID, AP_5G_PSW):
                ...
                self.caltest = {
                        "2.4G": ["1", self.AP_24G_SID, self.AP_24G_PSW],
                        "5G": ["2", self.AP_5G_SID, self.AP_5G_PSW],
                        "2.4G, 5G": ["3", self.AP_24G_SID, self.AP_24G_PSW, self.AP_5G_SID, self.AP_5G_PSW]}
...
### GW configuration changes ###
	...
	def BeerDis(self):
		...
		try:
			...
			pass
        # def login(self, usr = "admin", psw = "admin"):
        #         self.usr = usr
        #         self.psw = psw
	# 	time.sleep(180)
	# 	try:
	...
	# 	finally:
	# 		pass
        #         Print.INFO("Device succesfully loged-in!")

        def write_cal(self, cmd):

                self.telnet.write_raw('echo -en \"%s\n\" >> /tmp/calwrite\n' % (cmd))

        def config_GW(self, backhaul="2.4G, 5G" , AP_24G_SID="test_ssid", AP_24G_PSW="test_passphrase", AP_5G_SID="LSDK_5G", AP_5G_PSW="test_passphrase"):
		...
		# executecaltest command
                ...
                ###    TO DO:   ###
# Optimize it with method of write CAL to wlan interface # 
                ###################
                if backhaul == "1":
			...
			Print.INFO("backhaul: %s, ssid: %s, password: %s" % (backhaul, AP_5G_SID, AP_5G_PSW))
                # ["3", self.AP_24G_SID, self.AP_24G_PSW, self.AP_5G_SID, self.AP_5G_PSW]
                elif backhaul == "3":
			AP_5G_SID = caltest_backhaul[CALTEST_SSID + 2]
			AP_5G_PSW = caltest_backhaul[CALTEST_PASSWORD + 2]
                        backhaul = str(int(backhaul) -1)
			#Print.INFO("backhaul: %s, ssid: %s, password: %s" % (backhaul, AP_5G_SID, AP_5G_PSW))
			self.write_cal("object:Device.WiFi.SSID.%s: :MODIFY" % backhaul)
			self.write_cal("param:SSID: :%s" % AP_5G_SID)
			self.write_cal("object:Device.WiFi.AccessPoint.%s.Security.: :MODIFY" % backhaul)
			...
			Print.INFO("backhaul: %s, ssid: %s, password: %s" % (backhaul, AP_24G_SID, AP_24G_PSW))
                        ...
                        raise("Backhaul Value is Ilegal!!!")
                ###################
...
### checking DUT's status ###
        ...
        def run_test():
                ...
                config_GW_1 = GW("48", "Far-AP_24G", "12345678", "Far-AP_5G", "12345678")
                config_GW_1.BeerDis()
                config_GW_1.config_GW()
