...
config.read(os.getcwd() + "/trans/config.ini")
...
class Translate:
    ...
    def __init__(self, args):
        ...
        self.key = config.get("youdao", "key")
        self.secret = config.get("youdao", "secret")
        word = args[0]
        option = '-e'
        if len(args) > 1:
    ...
    def translate(self, word, option):
        ...
        try:
            ...
            if res_json.get('basic'):
                print("美音：[" + res_json['basic']['us-phonetic'] + ']   英音：[' + res_json['basic']['uk-phonetic'] + '] \n')
